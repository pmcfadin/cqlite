...
## Server-emitted Arrow golden (drift guard, issue #2234)

`ArrowToTrinoTest` builds `VectorSchemaRoot` vectors by hand, so it cannot catch
drift between what the Rust Flight server ACTUALLY emits (field order, Arrow
types, timestamp unit, `Date32`, the `arrow.uuid` extension, nullability) and
what this connector assumes. `ArrowToTrinoGoldenTest` closes that gap: it decodes
a committed Arrow IPC blob **produced by the real server emission path** and runs
...
- The blob is generated by the Rust `emit_arrow_golden` example
  ...
  wire Arrow schema — the exact bytes `do_get` would stream.
- **Regenerate deliberately** (never hand-edit) whenever the server's Arrow
  schema/type mapping changes:
