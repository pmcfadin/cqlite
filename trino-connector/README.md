...
| `cqlite.split-pruning-enabled` | `true` | Plan-time split pruning (issue #2679): when the pushed-down constraint fully binds the partition key, emit splits only for the covering token range(s) instead of one per range. Set `false` to force the full fan-out (the differential baseline). Always fail-safe — any doubt falls back to the full fan-out. |
| `cqlite.sub-splits-per-range` | `4` | Weight-balanced split→pod assignment (issue #2680): deterministically sub-split each Sidecar read-replica token range into K equal-token-span slices and rotate the slices across the range's replica owners, so per-owner assigned weight converges to ~1/N of the total even when per-range weights are very unequal. Each split also reports a `SplitWeight` proportional to its slice's token span. Must be in `[1, 64]`; `1` reproduces the pre-#2680 one-split-per-range behavior exactly. Larger K reduces per-pod CPU skew at the cost of more DoGet fan-out per full scan (~48 ranges → ~48×K slices). A pushed-down `LIMIT` or a fully-bound partition-key point read automatically plans at **K=1** regardless of this value (no benefit, and it keeps the `LIMIT` shape out of the multi-stream path — issue #2782). |
