...
observe_snapshot_listing() {
  ...
  local cassandra_cid
  cassandra_cid="$("${compose[@]}" ps -q cassandra)"
  ...
  local put_status
  put_status="$(docker run --rm --network "container:$cassandra_cid" curlimages/curl:latest \
  ...
  docker run --rm --network "container:$cassandra_cid" curlimages/curl:latest \
...
observe_degraded_schema_warn() {
  ...
  "${compose[@]}" logs cqlite-flight --no-color > "$logfile" 2>&1 || true
