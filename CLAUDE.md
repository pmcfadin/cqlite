...
- `test_parity.py`: Validates all 33 tables against JSONL golden files
  - `TestRowCountParity`: Row count validation per keyspace (33/33 passing)
...
**Known Issues** (tracked as XFail): none. Phase 4 (epic #471) closed the previous
two xfail entries (`static_columns_table` via #480 and `typed_collections_table`
via #481). Issue #493 tracks an out-of-scope follow-up (set element tombstones)
for v0.9.1.
