...
| **Recertify** (#4268) | `scripts/agent-gate.sh --recertify <anchor-summary-file> --components <c1[,c2]>` | Host-fault re-certification: rerun ≤2 components against a SAME-tree-digest full-gate anchor (never a new commit — that's `--delta`) when their anchor failure is proven the host's, not the code's. Requires: every OTHER component already PASS/OPT-OUT in the anchor, the current tree matching the anchor's identity exactly (`dirty: no`), the anchor ≤24h old, and none of the named components in the PR's own diff domain (`scripts/lib/recert-component-domains.sh`, a declared best-effort coarse classifier — unmapped ⇒ always ineligible). A recert can never chain off another recert (structural: a RECERT SUMMARY never satisfies the full-header check a valid anchor needs). Emits a DISTINCT `AGENT-GATE RECERT SUMMARY` with `recert-verdict: CERTIFIED` only if every named component is exactly PASS; record BOTH it AND the anchor's full SUMMARY in the PR. NOT the gate of record — see `docs/development/gate-ops.md`. |
