...
impl SSTableValidator {
            ...
            match self.detector.detect_file_deviations(path) {
                Ok(deviations) => {
                    for deviation in deviations {
                        result.warnings.push(format!("Format deviation: {}", deviation));
                    }
                },
                Err(e) => {
    ...
    pub fn validate_batch(&self, paths: &[&Path]) -> Vec<Result<ValidationResult, ValidationError>> {
        paths.iter()
    ...
    pub fn validate_memory(&self, data: &[u8], file_type: SSTableFileType) -> Result<ValidationResult, ValidationError> {
        let start_time = Instant::now();
    ...
    fn test_report_generation() {
        ...
        let results = vec![
            ...
            ValidationResult {
                ...
                format_version: None,
                is_valid: false,
                errors: vec![ValidationError::InvalidMagic { expected: 0x6F610000, found: 0xFFFFFFFF }],
                warnings: vec![],
                statistics: Default::default(),
            }
        ];
