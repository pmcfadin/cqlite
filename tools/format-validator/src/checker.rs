...
use crate::{ValidationError, ValidationResult, SSTableFileType, FormatValidator};
use std::path::Path;
...
impl FormatChecker {
            ...
            _ => {
                if self.strict_mode {
                    result.errors.push(ValidationError::StructureViolation {
                        reason: format!("Unsupported file type for strict compliance: {:?}", file_type)
                    });
                } else {
                    result.warnings.push(format!("Skipping compliance check for file type: {:?}", file_type));
                }
            }
        }
    ...
    fn check_data_compliance(&self, data: &[u8], result: &mut ValidationResult) -> Result<(), ValidationError> {
        ...
        // Verify magic number
        if let Err(e) = crate::utils::verify_magic(data, crate::format_constants::BIG_FORMAT_OA_MAGIC) {
            if self.strict_mode {
                result.errors.push(e);
            } else {
                result.warnings.push(format!("Magic number verification failed: {}", e));
            }
        } else {
        ...
        if data.len() >= 6 {
            ...
            if version != crate::format_constants::SUPPORTED_VERSION {
                if self.strict_mode {
                    ...
                    result.warnings.push(format!("Unsupported version: 0x{:04x}", version));
                }
            }
        }
    ...
    fn check_index_compliance(&self, data: &[u8], result: &mut ValidationResult) -> Result<(), ValidationError> {
        if data.is_empty() {
            result.errors.push(ValidationError::StructureViolation {
        ...
        if self.validate_structure && data.len() < 16 {
            result.warnings.push("Index file appears to be very small".to_string());
        }

        Ok(())
    }

    fn check_statistics_compliance(&self, data: &[u8], result: &mut ValidationResult) -> Result<(), ValidationError> {
        // Check Statistics.db format
        if data.len() >= 4 {
            if let Err(e) = crate::utils::verify_magic(data, crate::format_constants::STATISTICS_MAGIC) {
                if self.strict_mode {
                    result.errors.push(e);
                } else {
                    result.warnings.push("Statistics magic number not found".to_string());
                }
            } else {
    ...
    fn check_partitions_compliance(&self, data: &[u8], result: &mut ValidationResult) -> Result<(), ValidationError> {
        // Check BTI Partitions.db format
        if data.len() >= 4 {
            if let Err(e) = crate::utils::verify_magic(data, crate::format_constants::BTI_FORMAT_DA_MAGIC) {
                if self.strict_mode {
                    result.errors.push(e);
                } else {
                    result.warnings.push("BTI 'da' magic number not found".to_string());
                }
            } else {
        ...
        if self.validate_structure && data.len() >= 8 {
            ...
            if block_size == 0 || block_size > 1024 * 1024 {
                result.warnings.push(format!("Unusual BTI block size: {}", block_size));
            }
        }
    ...
    fn check_rows_compliance(&self, data: &[u8], result: &mut ValidationResult) -> Result<(), ValidationError> {
        ...
        if data.len() >= 4 {
            if let Err(e) = crate::utils::verify_magic(data, crate::format_constants::BTI_FORMAT_DA_MAGIC) {
                if self.strict_mode {
                    ...
                    result.warnings.push("BTI 'da' magic number not found in Rows.db".to_string());
                }
            } else {
                result.format_version = Some("BTI 'da'".to_string());
            }
    ...
    fn check_filter_compliance(&self, data: &[u8], result: &mut ValidationResult) -> Result<(), ValidationError> {
        ...
        if self.validate_structure && data.len() % 8 != 0 {
            result.warnings.push("Filter file size is not aligned to 8-byte boundary".to_string());
        }
...
impl FormatValidator for FormatChecker {
    ...
    fn validate_bytes(&self, data: &[u8], file_type: SSTableFileType) -> Result<ValidationResult, ValidationError> {
        ...
        match file_type {
            ...
            _ => {
                result.warnings.push(format!("Validation not implemented for file type: {:?}", file_type));
            }
        }
