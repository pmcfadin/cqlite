...
impl MemorySafetyRunner {
    ...
    pub fn run_miri_tests(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Set Miri flags for better error detection via Command::env() (safe approach)
    ...
    pub fn run_asan_tests(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Set environment variables for AddressSanitizer via Command::env() (safe approach)
        let output = Command::new("cargo")
            .args([
                "+nightly",
                "test",
            ...
            .env("ASAN_OPTIONS", "detect_odr_violation=0:abort_on_error=1")
            .output()?;
    ...
    pub fn print_available_tools(&self) {
        ...
        println!("  Miri: {}", if self.miri_available { "YES" } else { "NO" });
        println!(
            "  Valgrind: {}",
            if self.valgrind_available { "YES" } else { "NO" }
        );
        println!(
            "  AddressSanitizer: {}",
            if self.asan_available { "YES" } else { "NO" }
        );
