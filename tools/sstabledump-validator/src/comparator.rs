...
use crate::parser::{ParsedData, ParsedPartition, ParsedRow, ParsedCell, CellValue};
...
impl CellByCell {
    ...
    pub async fn compare_cell_by_cell(
        ...
        for (partition_key, cassandra_partition) in &cassandra_lookup {
            if let Some(cqlite_partition) = cqlite_lookup.get(partition_key) {
                let partition_diffs = self.compare_partitions(
                    partition_key,
                    cassandra_partition,
                    cqlite_partition,
                    &mut total_cells,
        ...
        let result = ComparisonResult {
            summary: ComparisonSummary {
                ...
                matching_cells,
                different_cells,
                missing_in_cassandra: differences.iter()
                    .filter(|d| matches!(d.difference_type, DifferenceType::MissingInCassandra))
                    .count() as u64,
                missing_in_cqlite: differences.iter()
            ...
            statistics: ComparisonStatistics {
                ...
                cassandra_cells: cassandra_lookup.values()
                    .flat_map(|p| &p.rows)
                    .map(|r| r.cells.len())
                    .sum(),
                cqlite_cells: cqlite_lookup.values()
                    .flat_map(|p| &p.rows)
                    .map(|r| r.cells.len())
                    .sum(),
        ...
        if result.has_differences() {
            warn!("Cell-by-cell comparison found {} differences", result.difference_count());
    ...
    async fn compare_partitions(
        ...
        for (clustering_key, cassandra_row) in &cassandra_rows {
            if let Some(cqlite_row) = cqlite_rows.get(clustering_key) {
                let row_diffs = self.compare_rows(
                    partition_key,
                    clustering_key,
                    cassandra_row,
                    cqlite_row,
    ...
    fn create_partition_lookup<'a>(&self, partitions: &'a [ParsedPartition]) -> HashMap<String, &'a ParsedPartition> {
        partitions.iter()
    ...
    fn create_row_lookup<'a>(&self, rows: &'a [ParsedRow]) -> HashMap<String, &'a ParsedRow> {
        rows.iter()
            .enumerate()
            .map(|(idx, row)| {
                let key = row.clustering_key.clone()
...
mod tests {
    ...
    async fn test_different_values_comparison() {
        ...
        let result = comparator.compare_cell_by_cell(&data1, &data2).await.unwrap();
