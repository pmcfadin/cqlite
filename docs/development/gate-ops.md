...
- Self-tests, both wired into `tooling-tests`: `scripts/tests/test_agent_gate_tree_integrity.sh` (the behavioural guard) and `scripts/tests/test_agent_gate_tree_portability.sh` (the same guard under BSD/macOS `sed`/`stat`/`sort` shims, plus a lint that FAILs on any GNU-only construct in the tree-integrity code — macOS is a first-class gate host).
