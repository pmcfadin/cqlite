...
- **A capture that cannot be validated is a FAIL, not a SKIP.** If the hash tool fails or a manifest write is truncated (e.g. `$TMPDIR` full during a long run), the identity is rejected and the run reports `tree-integrity: FAIL (tree-capture-failed; …)`. Only "there is no git worktree at all" produces a `SKIP`.
