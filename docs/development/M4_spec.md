...
lists** — 16 shapes, unchanged by #3505, which removed a *source* (`str`) without removing a shape.
One outcome is deliberately not a row: since #3505 a JSON number that no host type can represent
exactly raises rather than returning a value, and a refusal is not a host shape. Contrast with family (a), whose *instance* list ranges over an unbounded
