...
<<<<<<< HEAD
| **JSON object** | JSON keys | **OPEN.** A JSON object cell reaching Python as a `dict` (`json_to_py`) can carry a literal `"_type"` key of its own, so it is read as a UDT — the same collision with a third source. Instance **b-5**; for reachability see the single statement at the `Value::Json` arm of `value_to_hashable_key` (`bindings/python/src/value.rs`) — this row asserts nothing about it, and in particular does not claim a fixture would reach it. |
