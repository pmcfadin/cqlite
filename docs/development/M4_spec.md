...
|---|---|---|---|---|
| **a-1** | lossy projection | **UDT as a map KEY** (`map<frozen<udt>, v>`) | **CLOSED by #3504.** `value_to_hashable_key` used to project the key to a `frozenset` of `(field_name, value)` pairs (incl. `_type`/`_keyspace`), which canonicalized to a sorted array of `[name, value]` pairs while Node and the CLI render the same key as a UDT **object** — different in kind. The arm now projects to a **`cqlite.Udt` instance** (identity out of band, field values recursively projected), so the projection is type-PRESERVING and the key canonicalizes to the same UDT object the other two produce. Note the fix was NOT the declared type this row predicted: the loss was in the projection, not in the normalizer. **Reachability caveat — NO LONGER A CAVEAT, and the correction is the record.** It read: a NON-frozen `map<frozen<udt>,v>` is multicell, so its key lives in the cell path and `parse_cell_path_key` decodes it to `Value::Blob`, so only a `frozen<map<frozen<udt>,v>>` reaches this arm. That decode gap was instance A of #3631 and was **FIXED by #3612 / PR #3736** (`cell_path_key.rs` now delegates to the structural decoder); instance B — a collection-typed field INSIDE a frozen UDT — was fixed by **#3631**. RE-MEASURED on `test-data/fixtures/issue_3504` through the Python binding built from this tree: `cm` (non-frozen) and `fcm` (frozen) BOTH yield a `cqlite.Udt` key with identical fields, so both spellings reach this arm. | Nothing special. A UDT map key canonicalizes as a UDT object, under BOTH the frozen and the non-frozen spelling (#3612 closed the non-frozen decode gap; #3631 closed the collection-field-inside-a-UDT one). |
