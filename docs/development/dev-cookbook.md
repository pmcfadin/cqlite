...
| `CQLITE_EGRESS_MIN_CAP` | `8` | Forward-progress floor for the above (clamped to `[1, 256]`; budget forced `≥ min_cap`). The floor engages only at very high concurrency (`budget / min_cap` ≈ 256 concurrent merges at defaults). **Setting this ≥ 256 (or a budget ≥ 256 × min_cap) DISABLES the throttle** — the per-channel cap becomes constant (never shrinks with concurrency); a one-time `tracing::warn!` is emitted when the resolved knobs leave the throttle inert. Fresh loadgen validation tracked in #2895. |
