...
| Host floor (**precondition on the documented run command**) | The instrument as configured requires a host with **≥ 7 cores**, and fails **closed** below that. A published `Flat` sample is `measurement_time / sample_size` = 250 ms, which advances `0.25 s × 100 Hz × cores = 25 × cores` `/proc/stat` ticks; clearing the 160-tick resolution floor therefore needs `cores ≥ 6.4`. The "~400 ticks" figure quoted above is the **16-core** value, not a general one. On a ≤6-core host every published sample is classified unresolvable, `arm_gated` never reaches its minimum, and `end_arm` panics — correctly refusing to publish, but with a message (*"only 0 of N timed intervals could be gated"*) that reads like a sampling-loop bug rather than "this host is too small". Consequence: the two small-host **ceiling floors** (`LOAD_CEILING_FLOOR` 0.5, `FOREIGN_CPU_CEILING_FLOOR` 0.25), documented as serving 1–4-core hosts, are **unreachable** in this configuration — the resolution gate voids such a run first. Neither fact affects any number in this record (all of it measured at 16 cores, 27/27 arms gated per run). An up-front core-count assertion and the dead-floor cleanup are tracked in **issue #2930** (bench validity-guard hardening), deliberately **not** applied here so that the merged instrument stays byte-identical to the one that produced these numbers. |
