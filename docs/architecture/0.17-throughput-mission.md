...
> on a 2 GiB random pointer chase — ~20× the guest's 105 MiB L3, `ns_per_access=253` confirming DRAM
> by wall clock — while `cycle_activity.stalls_l2_miss` on the same run reads **8.23e9**, i.e. 70% of
> all cycles. Unmultiplexed, `enabled%=100.00`. Not a `perf` event-table defect either: raw
> `event=0xa3,umask=0x6,cmask=0x6` (#3224 §5.2's own verified encoding) reads `0` where `umask=0x5`
> reads 8.40e9 on the same event select. `offcore_requests_outstanding.*` is stuck at zero the same
> way. Evidence: `docs/reports/ws0-3287-artifacts/negative-control-c7i-guest.md`.
>
