...
> guest (Xeon 8488C, 8 physical cores, KVM), that counter **programs cleanly and returns a hard `0`**
> on a 2 GiB random pointer chase — ~20× the guest's 105 MiB L3, `ns_per_access=255` confirming DRAM
> by wall clock — while `cycle_activity.stalls_l2_miss` on the same run reads **6.17e9**, i.e. **91%
> of all cycles**. Unmultiplexed (`enabled%=100.00`), window gated to the chase, counted `:u`. Not a
> `perf` event-table defect either: raw `event=0xa3,umask=0x6,cmask=0x6` (#3224 §5.2's own verified
> encoding) reads `0` where `umask=0x5` reads **6.22e9** on the same event select.
> `offcore_requests_outstanding.*` is stuck at zero the same way, and so is `cache-misses`. The
> workload's behaviour is fixed by **wall clock** — 6.0 ns/access L2-resident vs 242–268 ns in DRAM,
