...
| Issue | P | Finding |
|---|---|---|
| **#3028** (WS2) | P1 | Allocator churn 19.6%. **RE-PRICE against post-#3058 profile first** (#3248). Level lever — live against gap A; no gap-B effect unless it shrinks the footprint. |
...
correctness gate is discharged; throughput has not moved since — but the program now has a
correctly-scoped map (owner session 2026-08-04): the bar is BOX-LEVEL AGGREGATE rows/s, whose
denominator #3299 must measure; gap A (per-core 1.62×, ~79% unlocated) is #3248's to attribute and
LEVEL levers close it; gap B (~29% scaling discount) is NAMED as LLC capacity contention (#3224,
DRAM at only 21.7% of peak) and only #3288 (bytes touched per row) moves it — all of it after
#3272 hardens the rig it rests on, and anchored to the field by one through-Trino re-probe (#2866)
after the next lever lands.**
