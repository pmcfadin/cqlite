...
**So the ~29% discount is specific to the `do_get`/Flight path, not to the bytes on disk.** Bare
scan's per-row cost rises ~4% across the whole range where `do_get`'s rises ~19%. The extra decay is
attributable to `do_get`'s *additional* footprint — Arrow encode buffers — which is exactly what
#3288 targets. **#3288's ceiling is therefore preserved, not lowered.**
