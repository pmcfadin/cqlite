...
**The bar (owner ruling 2026-08-04): BOX-LEVEL AGGREGATE rows/s.** The box-level target is
currently **underivable** — bare scan, the denominator, has only ever been measured at S=1.
**#3299** (bare-scan C(S) S=1..6) derives it as `1.3 × bare_scan_aggregate(S=6)` and reports
whether bare scan itself decays under LLC contention (which calibrates #3288's ceiling).
...
box-level bare scan.** The box denominator is unmeasured → **#3299** derives it. Until it lands, the
program tracks two component gaps — which are DIFFERENT problems with DIFFERENT lever classes, and
...
| **B. Scaling discount** — aggregate efficiency S=1→S=6 | 71% marginal at 6 cores (#3217) | ~29% of box throughput | **LLC capacity contention (#3224)** | **SLOPE lever**: #3288 (bytes touched/row) — once its footprint baseline exists |
...
| **#3299** | P1 | **Bare-scan C(S) S=1..6 — the missing box-level ceiling.** Derives the box target (`1.3 × bare_scan(S=6)`) and reports whether bare scan itself decays under LLC contention (calibrates #3288's maximum value). Filed 2026-08-04 on the owner's box-level ruling. Prefer the metal box #2818 holds — do not launch a second. |
