...
# discarded). Each cell's load is appended to machine-state.jsonl.
#
# THAT FILE IS NOT IN THIS ARTIFACTS DIRECTORY, and its absence is the honest
# state: the committed 45 cells predate the guard, so there is no load record for
# them (see 3.1(c)). A future run creates it; do not go looking for it here.
...
| `machine-state.jsonl` | **not present** — the per-cell load record the driver now writes. The committed cells predate that guard and carry no load field (§3.1(c)); a future run creates this file |
