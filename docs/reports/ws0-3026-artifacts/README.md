...
| `ws0-results/h2h-perf-summaries/` | **Truncated** perf/jstack reports (see exclusions). Backs the §5 percentages. |
...
**The truncation is a real limitation, not a formality.** The `top400` files carry the ranked frames the
report cites, but a reader cannot re-aggregate a different view (e.g. a per-thread or per-callee breakdown)
from them. To reproduce a full profile, re-run per the method doc.
