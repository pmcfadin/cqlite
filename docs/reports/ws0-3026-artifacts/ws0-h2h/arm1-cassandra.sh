...
#   * counters come from `perf stat -p <daemon-pid>`, so client wall-time and
...
sudo -n taskset -acp 2 "$PID" >/dev/null 2>&1 || taskset -acp 2 "$PID" >/dev/null
...
perf stat -x, -e cycles,instructions,l2_lines_in.all,l2_lines_out.non_silent \
     -p "$PID" -o "$STAT" &
PERF=$!
sleep 1   # let perf attach to all threads before the workload starts
...
kill -INT "$PERF" 2>/dev/null || true
wait "$PERF" 2>/dev/null || true

CPU1=$(awk '{print $14+$15}' /proc/$PID/stat)
IO1=$(cat /proc/$PID/io)
HZ=$(getconf CLK_TCK)

python3 - "$STAT" "$OUT/scan-$LABEL.json" "$CPU0" "$CPU1" "$HZ" "$TEMP" \
  <<PYEOF > "$OUT/summary-$LABEL.txt"
import json,sys
stat,scan,c0,c1,hz,temp=sys.argv[1:]
io0="""$IO0"""; io1="""$IO1"""
def parse_io(s):
    d={}
...
print(json.dumps({
 "engine":"cassandra-5.0.8","label":d['label'],"temp":temp,
