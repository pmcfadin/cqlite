...
| `cycle_activity.stalls_l2_miss` on the 2 GiB arm | **> 80% of all cycles** (billions; 82.3–90.0% over ten captures) | `arm-hostile-2g-stalls.csv` |
...
| `ns_per_access` | **~5–6 ns** L2-resident vs **~165–315 ns** DRAM (range over ten captures) | `arm-*.txt` |
...
L3. (The bound is stated loosely on purpose: it measured between 82.3% and 90.0% over ten captures on
...
arms across ten captures — an access-latency spread of roughly **28× to 65×** (28.4–64.2× measured), produced purely by
...
explain a *wrong magnitude*; it cannot manufacture a hard `0` in ten consecutive captures of a
...
exists. Over this branch's review rounds that happened **ten times** on the same box
(`host/capability-probe.txt` stamps each one, and the superseded captures are in this branch's git
history). Ten independent reps is more than the finding needed, and the contrast they draw is the
file's whole argument:

| `hostile-2g` arm | across all ten captures |
|---|---|
...
A counter that varies with load is measuring something. A counter that returns **exactly 0** in ten
