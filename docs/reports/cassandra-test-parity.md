...
- `cass.compaction.CompactionColumnDeleteAndPurgeTest.cell_delete_purge` (planned): No dedicated fixture mirrors this specific cell-delete-then-gc-purge case; the general purge path is covered by the compaction_parity_tombstone_ttl suite. → _Commission a Cassandra fixture for cell deletion + gc purge and add per-case sstabledump goldens to the compaction parity suite. Re-parked to open tracker #1387._
- `cass.compaction.CompactionColumnTest.cell_merge_lww` (planned): No dedicated fixture mirrors this specific per-column LWW merge case. → _Commission a fixture for per-column LWW merge and assert per-case goldens. Re-parked to open tracker #1387._
- `cass.compaction.CompactionControllerTest.purgeable_timestamp_boundary` (planned): No dedicated fixture mirrors the CompactionController purgeable-timestamp boundary cases. → _Commission fixtures spanning the purgeable-timestamp boundary and assert per-case goldens. Re-parked to open tracker #1387._
- `cass.compaction.CompactionDeleteAndPurgePKTest.partition_delete_purge` (planned): No dedicated fixture mirrors this partition-delete-then-purge case. → _Commission a partition-delete + gc-purge fixture and assert per-case goldens. Re-parked to open tracker #1387._
...
- `cass.compaction.CompactionDeletePKTest.partition_delete_preserved` (planned): No dedicated fixture mirrors this partition-delete-preservation case. → _Commission a fixture asserting partition deletions survive compaction and add goldens. Re-parked to open tracker #1387._
...
- `cass.compaction.CompactionDeleteRowTest.row_deletion_marker_preserved` (planned): No dedicated fixture mirrors this row-deletion-marker-preservation case. → _Commission a row-deletion fixture and assert markers survive compaction. Re-parked to open tracker #1387._
- `cass.compaction.CompactionDiskSpaceTest.disk_space_constrained_merge` (planned): CQLite has no disk-space budgeting guardrail; a merge that would exhaust free space is not detected before writing partial output. → _Under the compaction fixture family (#1387), commission a space-constrained scenario and add a pre-write disk-space estimate/guardrail to the compactor._
- `cass.compaction.CompactionOverlappingSSTableTest.partial_space_constrained_merge` (planned): No dedicated fixture mirrors the disk-space-constrained partial-compaction case. → _Commission overlapping-SSTable fixtures under a space constraint and assert live-row preservation. Re-parked to open tracker #1387._
- `cass.compaction.CompactionsCQLTest.negative_ldts_suspect_table` (planned): No dedicated fixture mirrors the negative-LDTS suspect-table cases. → _Commission fixtures with negative local-deletion-time metadata and assert detection/goldens. Re-parked to open tracker #1387._
...
- `cass.compaction.LeveledCompactionStrategyTest.non_overlapping_levels` (planned): CQLite does not yet run a leveled compaction strategy; merge output parity for LCS-produced inputs is unmirrored. → _Implement/mirror leveled-strategy merge ordering or assert read parity over LCS-produced fixtures. Re-parked to open tracker #1387._
- `cass.compaction.LeveledGenerationsTest.level_generation_merge_order` (planned): CQLite does not yet model leveled generations; merge-order parity is unmirrored. → _Mirror leveled-generation merge ordering or assert read parity over generation-tagged fixtures. Re-parked to open tracker #1387._
- `cass.compaction.LongLeveledCompactionStrategyTest.concurrent_level_merge_safety` (planned): CQLite does not yet run concurrent leveled compaction; overlapping-range merge safety is unmirrored. → _Mirror concurrent leveled-compaction merge safety or assert read parity over the produced fixtures. Re-parked to open tracker #1387._
- `cass.compaction.ParallelizedTasksTest.sharded_row_partition_integrity` (planned): CQLite does not run parallelized compaction tasks; sharded row/partition integrity over the produced output is unmirrored. → _Mirror sharded row/partition integrity or assert read parity over parallelized-compaction fixtures. Re-parked to open tracker #1387._
- `cass.compaction.PartialCompactionsTest.tombstone_not_purged_excluded_sources` (planned): No dedicated fixture mirrors this excluded-source tombstone-retention case. → _Commission a partial-compaction fixture excluding sources and assert tombstones are retained. Re-parked to open tracker #1387._
...
- `cass.compaction.SSTablesIteratedTest.merge_no_shadow_without_timestamp` (planned): No dedicated fixture mirrors the SSTablesIterated shadow-ordering case. → _Commission overlapping fixtures and assert no live cell is shadowed without timestamp ordering. Re-parked to open tracker #1387._
- `cass.compaction.ShardedCompactionWriterTest.shard_boundary_row_preservation` (planned): No dedicated fixture mirrors sharded-compaction-writer shard-boundary preservation. → _Commission sharded-writer fixtures and assert no partition is orphaned across shard boundaries. Re-parked to open tracker #1387._
- `cass.compaction.ShardedMultiWriterTest.flush_sharding_row_count` (planned): No dedicated fixture mirrors flush-triggered sharded multi-writer row counts. → _Commission flush-sharding fixtures and assert each partition yields the expected row count. Re-parked to open tracker #1387._
- `cass.compaction.SingleSSTableLCSTaskTest.single_table_uplevel_metadata_readable` (planned): CQLite does not run LCS uplevel tasks; the single-table uplevel metadata path is unmirrored. → _Mirror single-SSTable LCS uplevel metadata validation or assert metadata read parity. Re-parked to open tracker #1387._
...
- `cass.compaction.UnifiedCompactionDensitiesTest.sstable_density_distribution` (planned): CQLite does not implement UCS density distribution; output sizing parity is unmirrored. → _Mirror UCS density distribution or assert read parity over the produced SSTables. Re-parked to open tracker #1387._
- `cass.compaction.UnifiedCompactionStrategyTest.gc_grace_purge_boundary` (planned): CQLite does not yet run the unified compaction strategy; gc_grace purge-boundary parity is unmirrored. → _Mirror UCS gc_grace purge boundary or assert read parity over UCS-produced fixtures. Re-parked to open tracker #1387._
...
- `cass.compaction_merge.byte_for_byte_output` (planned): No gated byte-for-byte comparison of compaction output. → _Promote the debug byte tier in compaction-parity to a gated comparison once writer output is byte-stable. Re-parked to open tracker #1387._
...
- `cass.compression_info.deflate.real_fixture_chunks.strict` (planned): No real Cassandra Deflate-compressed fixture in the corpus. → _Generate a DeflateCompressor SSTable fixture via issue #996 (epic #970) and add it to the dataset; the strict lane will then decode and round-trip it. Re-parked to open tracker #1402._
- `cass.compression_info.zstd.real_fixture_chunks.strict` (planned): No real Cassandra Zstd-compressed fixture in the corpus (non-dictionary). → _Generate a non-dictionary ZstdCompressor SSTable fixture via issue #996 (epic #970); the strict lane will then decode and round-trip it. Re-parked to open tracker #1402._
...
- `cass.index_summary.column_index.range_tombstone_boundary_big_bti` (partial): BTI (da) range-tombstone-at-block-edge fixtures are not yet generated (no da tombstone generator). → _Add a da/BTI wide-partition range-tombstone fixture generator and assert BTI column-index boundary parity; file a follow-up issue. Re-parked to open tracker #1387._
...
- `cass.tombstone_ttl.RangeTombstoneBurnTest.range_partition_column_deletes_readable` (planned): No dedicated burn-scale fixture mirrors the RangeTombstoneBurn cases. → _Commission a range-tombstone burn fixture and assert deletes serialize and read back vs Cassandra. Re-parked to open tracker #1387._
...
- `cass.tombstone_ttl.SSTableExpiredBlockersTest.fully_expired_drop_blockers` (planned): Fully-expired-SSTable drop (and the overlapping-SSTable blocker check that gates it) is not implemented; expired data is filtered on read but not dropped. → _Implement fully-expired-SSTable drop + blocker detection under #1388 and mirror the expired-blocker semantics._
- `cass.tombstone_ttl.ViewComplexTombstoneTest.cell_tombstone_not_purged` (planned): No dedicated fixture mirrors the view complex cell-tombstone cases; MV maintenance is coordinator-side. → _Commission a fixture with view-style cell tombstones and assert they are not prematurely purged on merge. Re-parked to open tracker #1387._
- `cass.tombstone_ttl.range_tombstone_boundaries` (partial): test_deltas dataset asset not published/enforced in CI (#701). → _Publish the test_deltas dataset and enforce scan_delta parity in CI. Re-parked to open tracker #1387._
...
|---|---|---|
| `cass.bti_big_version_matrix.big_nb_oa_read` | nb, oa | test-data/datasets/sstables/test_oa/simple_table-5ead3f007b2311f1ac0b1facdfdff4e7/oa-2-big-Data.db.jsonl |
| `cass.bti_big_version_matrix.bti_da_write_read` | da | test-data/datasets/sstables/test_da/simple_table-67be1ba07b2311f1a7ea1783683013a6/da-2-bti-Data.db.jsonl |
...
| `cass.compaction.CompactionAwareWriterTest.row_count_and_order_preservation` | nb | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl |
...
| `cass.compaction.CompactionIteratorTest.differential_compaction_loop` | nb | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl |
...
| `cass.compaction.harness_logical_tier` | nb | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl |
...
| `cass.compaction_merge.tombstone_ttl_shadowing` | nb | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl |
...
| `cass.compression_checksum.checksum_trailer_detection` | da | test-data/datasets/sstables/test_da/simple_table-67be1ba07b2311f1a7ea1783683013a6/da-2-bti-Digest.crc32<br>_fail:_ artifact.fast_pr.checksum_diff |
| `cass.compression_checksum.chunk_offsets_and_crc` | nb | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>test-data/datasets/sstables/test_collections/collection_table-4bf70df07b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl |
| `cass.compression_info.CompressedInputStreamTest.truncated_chunk_detection` | nb | test-data/datasets/sstables/test_basic/compression_test_table-4b7904507b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.CompressedInputStreamTest.truncated_chunk_detection.strict` | nb, oa, da | test-data/datasets/sstables/test_basic/composite_key_table-4b5f88e07b2311f1898d156e0c21aefb/nb-1-big-TOC.txt<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.CompressedRandomAccessReaderTest.chunk_offsets` | nb | test-data/datasets/sstables/test_basic/compression_test_table-4b7904507b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.CompressedRandomAccessReaderTest.chunk_offsets.strict` | nb, oa, da | test-data/datasets/sstables/test_basic/composite_key_table-4b5f88e07b2311f1898d156e0c21aefb/nb-1-big-TOC.txt<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.CompressedSequentialWriterTest.chunk_boundaries` | nb | test-data/datasets/sstables/test_basic/compression_test_table-4b7904507b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.CompressedSequentialWriterTest.chunk_boundaries.strict` | nb, oa, da | test-data/datasets/sstables/test_basic/compression_test_table-4b7904507b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.CompressionMetadataTest.metadata_serialization` | nb | test-data/datasets/sstables/test_basic/compression_test_table-4b7904507b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.CompressionMetadataTest.metadata_serialization.strict` | nb, oa, da | test-data/datasets/sstables/test_basic/composite_key_table-4b5f88e07b2311f1898d156e0c21aefb/nb-1-big-TOC.txt<br>test-data/datasets/sstables/test_oa/collection_table-5eba5e607b2311f1ac0b1facdfdff4e7/oa-2-big-TOC.txt<br>test-data/datasets/sstables/test_da/collection_table-67cf32a07b2311f1a7ea1783683013a6/da-2-bti-TOC.txt<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.DirectCompressedChunkReaderTest.inline_crc_validation` | nb | test-data/datasets/sstables/test_basic/compression_test_table-4b7904507b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.DirectCompressedChunkReaderTest.inline_crc_validation.strict` | nb, oa, da | test-data/datasets/sstables/test_da/collection_table-67cf32a07b2311f1a7ea1783683013a6/da-2-bti-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.StandardCompressedChunkReaderTest.round_trip_chunk_bytes` | nb | test-data/datasets/sstables/test_basic/compression_test_table-4b7904507b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.compression_info.lz4.real_fixture_chunks` | nb | test-data/datasets/sstables/test_basic/compression_test_table-4b7904507b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.lz4.real_fixture_chunks.strict` | nb, oa, da | test-data/datasets/sstables/test_basic/composite_key_table-4b5f88e07b2311f1898d156e0c21aefb/nb-1-big-TOC.txt<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.snappy.real_fixture_chunks` | nb | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.snappy.real_fixture_chunks.strict` | nb, oa, da | test-data/datasets/sstables/test_basic/composite_key_table-4b5f88e07b2311f1898d156e0c21aefb/nb-1-big-TOC.txt<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.cql_types.jsonl.canonical_value_comparator` | — | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl |
| `cass.cql_types.jsonl.cell_path_timestamp_ttl_tombstone_compare` | — | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl |
| `cass.cql_types.jsonl.manifest_report_generation` | — | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl |
| `cass.cql_types.jsonl.no_placeholder_references` | — | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl |
| `cass.cql_types.jsonl.schema_aware_normalization` | — | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl |
...
| `cass.data_db_decode.clustering_bounds.desc_order` | nb, oa | test-data/datasets/sstables/test_basic/composite_key_table-4b5f88e07b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.clustering_bounds.multi_column_prefix` | nb, oa | test-data/datasets/sstables/test_wide_rows/wide_partition_table-4d4364107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.clustering_bounds.null_vs_empty` | nb, oa | test-data/datasets/sstables/test_basic/static_columns_table-4b9f29f07b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.data_db_decode.row_cell_flags_and_vint` | nb, oa | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl |
| `cass.data_db_decode.row_preamble_size_mismatch` | nb | — |
| `cass.data_db_decode.serialization_header.timestamp_ttl_ldt_deltas` | nb, oa | test-data/datasets/sstables/test_basic/uncompressed_table-4b8735207b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>test-data/datasets/sstables/test_basic/ttl_test_table-6af66a30a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.serialization_mirror.multi_clustering_column_order` | nb, oa | test-data/datasets/sstables/test_basic/composite_key_table-4b5f88e07b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.static_rows.static_only_partition` | nb, oa | test-data/datasets/sstables/test_basic/static_columns_table-4b9f29f07b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>test-data/datasets/sstables/test_deltas/static_with_rows-29a4cf80701f11f1b5d1d98b0640ec05/nb-1-big-Data.db<br>test-data/datasets/sstables/test_deltas/static_with_rows-29a4cf80701f11f1b5d1d98b0640ec05/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.data_db_decode.unfiltered_serializer.row_and_cell_flags` | nb, oa | test-data/datasets/sstables/test_basic/uncompressed_table-4b8735207b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.unfiltered_serializer.row_size_vints` | nb, oa | test-data/datasets/sstables/test_basic/uncompressed_table-4b8735207b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.filter_db.corruption_fails_closed` | nb, oa, da | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.filter_db.no_false_negative_membership` | nb, oa, da | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>test-data/datasets/sstables/test_basic/composite_key_table-4b5f88e07b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.filter_db.serialization_round_trip` | nb, oa, da | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>test-data/datasets/sstables/test_da/simple_table-67be1ba07b2311f1a7ea1783683013a6/da-2-bti-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.filter_db_bloom.serialization_no_false_negative` | nb, oa | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>test-data/datasets/sstables/test_basic/composite_key_table-4b5f88e07b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.index_db.CorruptPrimaryIndexTest.big_primary_index_corruption` | nb, oa | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.index_db.RowIndexEntryTest.partition_offsets` | nb, oa | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.index_db.SSTableReaderTest.point_lookup_offsets` | nb, oa | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.index_db.SSTableScannerTest.range_boundaries` | nb, oa | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.index_db.big.raw_partition_keys_and_offsets` | nb, oa | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>test-data/datasets/sstables/test_oa/collection_table-5eba5e607b2311f1ac0b1facdfdff4e7/oa-2-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.index_db.bti.index_component_discovery` | da | test-data/datasets/sstables/test_da/simple_table-67be1ba07b2311f1a7ea1783683013a6/da-2-bti-TOC.txt<br>_fail:_ artifact.manual_debug.reproduction_bundle |
...
| `cass.index_summary.big_index_offsets` | nb | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl |
...
| `cass.index_summary.summary_boundaries` | nb, oa, big | test-data/datasets/sstables/test_timeseries/app_metrics-4ca6aee07b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.repaired_metadata.statistics_db.pending_repair_uuid` | nb, oa, da | test-data/datasets/sstables/test_basic/composite_key_table-4b5f88e07b2311f1898d156e0c21aefb/nb-1-big-Statistics.db.txt |
| `cass.repaired_metadata.statistics_db.repaired_at_field` | nb, oa, da | test-data/datasets/sstables/test_basic/composite_key_table-4b5f88e07b2311f1898d156e0c21aefb/nb-1-big-Statistics.db.txt<br>test-data/datasets/sstables/test_oa/collection_table-5eba5e607b2311f1ac0b1facdfdff4e7/oa-2-big-Statistics.db.txt<br>test-data/datasets/sstables/test_da/simple_table-67be1ba07b2311f1a7ea1783683013a6/da-2-bti-Statistics.db.txt<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.repaired_metadata.statistics_db.transient_repair_flag` | nb, oa, da | test-data/datasets/sstables/test_da/simple_table-67be1ba07b2311f1a7ea1783683013a6/da-2-bti-Statistics.db.txt |
...
| `cass.sstable_format.toc_component_manifest` | nb, oa, da | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-TOC.txt<br>test-data/datasets/sstables/test_oa/collection_table-5eba5e607b2311f1ac0b1facdfdff4e7/oa-2-big-TOC.txt<br>test-data/datasets/sstables/test_da/simple_table-67be1ba07b2311f1a7ea1783683013a6/da-2-bti-TOC.txt<br>test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Digest.crc32<br>test-data/datasets/sstables/test_oa/simple_table-5ead3f007b2311f1ac0b1facdfdff4e7/oa-2-big-Digest.crc32<br>_fail:_ artifact.fast_pr.byte_diff |
...
| `cass.statistics_db.MetadataSerializerTest.metadata_components` | nb, oa, da | test-data/datasets/sstables/test_basic/ttl_test_table-6af66a30a25111f0a3fef1a551383fb9/nb-1-big-Statistics.db.txt<br>test-data/datasets/sstables/test_oa/collection_table-5eba5e607b2311f1ac0b1facdfdff4e7/oa-2-big-Statistics.db.txt<br>test-data/datasets/sstables/test_da/simple_table-67be1ba07b2311f1a7ea1783683013a6/da-2-bti-Statistics.db.txt<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.statistics_db.SerializationHeaderTest.schema_evolution_header` | nb, oa, da | test-data/datasets/sstables/test_basic/static_columns_table-4b9f29f07b2311f1898d156e0c21aefb/nb-1-big-Statistics.db.txt<br>test-data/datasets/sstables/test_oa/udt_table-5ed0a5807b2311f1ac0b1facdfdff4e7/oa-2-big-Statistics.db.txt<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.statistics_db.SerializationMirrorTest.column_ordering_metadata` | nb, oa, da | test-data/datasets/sstables/test_basic/composite_key_table-4b5f88e07b2311f1898d156e0c21aefb/nb-1-big-Statistics.db.txt<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.statistics_db.clustering_key_bounds` | — | — |
| `cass.statistics_db.core_metadata_checksums` | nb, oa, da | test-data/datasets/sstables/test_da/simple_table-67be1ba07b2311f1a7ea1783683013a6/da-2-bti-Statistics.db.txt<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.statistics_metadata.serialization_header` | nb | test-data/datasets/sstables/test_basic/simple_table-4b4ee7107b2311f1898d156e0c21aefb/nb-1-big-Statistics.db.txt |
...
| `cass.summary_db.IndexSummaryTest.first_last_key_boundaries` | nb, oa, big | test-data/datasets/sstables/test_timeseries/app_metrics-4ca6aee07b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.summary_db.IndexSummaryTest.offset_table_entries` | nb, oa, big | test-data/datasets/sstables/test_timeseries/app_metrics-4ca6aee07b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.summary_db.IndexSummaryTest.serialization_round_trip` | nb, oa, big | test-data/datasets/sstables/test_timeseries/app_metrics-4ca6aee07b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.summary_db.big.index_offset_references` | nb, oa, big | test-data/datasets/sstables/test_timeseries/app_metrics-4ca6aee07b2311f1898d156e0c21aefb/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.summary_db.bti.summary_discovery_classification` | nb, oa, da, big, bti | test-data/datasets/sstables/test_da/simple_table-67be1ba07b2311f1a7ea1783683013a6/da-2-bti-TOC.txt<br>_fail:_ artifact.required_parity.byte_diff |
