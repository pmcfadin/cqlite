...
| `cass.compaction.udt_frozen_person` | compaction_merge | mirrored | byte_for_byte | `compaction_parity_tombstone_ttl` | p0_data_loss |
| `cass.compaction.udt_nested` | compaction_merge | mirrored | byte_for_byte | `compaction_parity_tombstone_ttl` | p0_data_loss |
...
| `cass.data_db_decode.range_tombstone.boundary_markers` | tombstone_ttl | mirrored | byte_for_byte | `compaction_parity_tombstone_ttl` | p1_correctness |
...
| `cass.sstable_format.CQLSSTableWriterTest.frozen_udt_roundtrip` | compaction_merge | mirrored | byte_for_byte | `compaction_parity_tombstone_ttl` | p0_data_loss |
...
- `cass.compaction.udt_frozen_person` — Frozen-UDT (person) value compaction byte parity
  - Normalization: Data.db/Index.db/Summary.db/Digest.crc32 whole-file byte-for-byte; CRC.db prefix (compaction-only trailing empty-chunk CRC32=0 excluded); typed JSONL per-(pk,cell) canonical UDT JSON equality pins field order + null-field + empty-string-field decode of the LWW survivors.
- `cass.compaction.udt_nested` — Nested frozen-UDT (employee→address) compaction byte parity
  - Normalization: Data.db/Index.db/Summary.db/Digest.crc32 whole-file byte-for-byte; CRC.db prefix (compaction-only trailing empty-chunk CRC32=0 excluded); typed JSONL per-(pk,cell) canonical nested-UDT JSON equality pins inner+outer field order, nested field decode, and the inner null field.
...
- `cass.data_db_decode.range_tombstone.boundary_markers` — Data.db range-tombstone boundary-marker (two deletion times) parity
  - Normalization: The boundary-marker grammar is compared field-by-field at exact offsets: IS_MARKER, the kind-2/kind-5 ordinal, the u16 cluster count, the prefix clustering value, and BOTH deletion-time pairs (primary then secondary). The deterministic writer lane (`range_boundary_marker_exact_grammar_writer`) drives the public DataWriter and asserts the full boundary grammar at exact offsets for a coalesced kind-2 boundary (both deletion-time mfda+ldt deltas), and runs everywhere (no fixture dependency). The fixture lanes reconstruct both deletion-time µs against the golden end/start marked_deleted timestamps and fail-closed under `CQLITE_REQUIRE_FIXTURES=1` / `CQLITE_PARITY_REQUIRE_DATASETS=1`.
...
- `cass.sstable_format.CQLSSTableWriterTest.frozen_udt_roundtrip` — Frozen-UDT round-trip through compaction (byte-parity mirror)
  - Normalization: Cross-references cass.compaction.udt_frozen_person (byte-for-byte components + typed JSONL survivor map).
...
- `cass.compaction.SizeTieredCompactionStrategyTest.bucket_selection` — STCS size-bucket grouping and candidate selection
  - Normalization: Compared on the SET of selected SSTable sizes (order-independent) STCS picks for the next compaction under the Cassandra-default policy (min_threshold=4, max_threshold=32, bucket_low=0.5, bucket_high=1.5, min_sstable_size=50MB); file identity and on-disk layout ignored.
...
| `cqlite-core/tests/sstable_parity_compression_info_test.rs` | 8 |
...
| `cqlite-core/tests/issue_1020_udt_frozen_compaction_byte_parity.rs` | 6 |
...
- `cass.compaction.CompactionColumnDeleteAndPurgeTest.cell_delete_purge` (planned): No dedicated fixture mirrors this specific cell-delete-then-gc-purge case; the general purge path is covered by the compaction_parity_tombstone_ttl suite. → _Re-parked to open tracker #2303 (Commission the parity fixture library; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.CompactionColumnTest.cell_merge_lww` (planned): No dedicated fixture mirrors this specific per-column LWW merge case. → _Re-parked to open tracker #2303 (Commission the parity fixture library; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.CompactionControllerTest.purgeable_timestamp_boundary` (planned): No dedicated fixture mirrors the CompactionController purgeable-timestamp boundary cases. → _Re-parked to open tracker #2303 (Commission the parity fixture library; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.CompactionDeleteAndPurgePKTest.partition_delete_purge` (planned): No dedicated fixture mirrors this partition-delete-then-purge case. → _Re-parked to open tracker #2303 (Commission the parity fixture library; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.CompactionDeletePKTest.partition_delete_preserved` (planned): No dedicated fixture mirrors this partition-delete-preservation case. → _Re-parked to open tracker #2303 (Commission the parity fixture library; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.CompactionDeleteRowTest.row_deletion_marker_preserved` (planned): No dedicated fixture mirrors this row-deletion-marker-preservation case. → _Re-parked to open tracker #2303 (Commission the parity fixture library; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.CompactionDiskSpaceTest.disk_space_constrained_merge` (planned): CQLite has no disk-space budgeting guardrail; a merge that would exhaust free space is not detected before writing partial output. → _Under the compaction manager (#905), commission a space-constrained scenario and add a pre-write disk-space estimate/guardrail to the compactor. Re-parked to open tracker #905 (predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.CompactionOverlappingSSTableTest.partial_space_constrained_merge` (planned): No dedicated fixture mirrors the disk-space-constrained partial-compaction case. → _Re-parked to open tracker #2303 (Commission the parity fixture library; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.CompactionsCQLTest.negative_ldts_suspect_table` (planned): No dedicated fixture mirrors the negative-LDTS suspect-table cases. → _Re-parked to open tracker #2303 (Commission the parity fixture library; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.ForceCompactionTest.major_compaction_tombstone_purge` (partial): Empty-output purge contract only; a fully-purged compaction has no bytes to diff. → _Keep the semantic purge contract; byte-for-byte surviving-tombstone parity is now covered by the row_delete_purge and range_tombstone_merge byte_for_byte scenarios (fixed by #1410, PR #1726), with ttl_window_expiry_purge still pending. Re-parked to open tracker #2303 (predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.LeveledCompactionStrategyTest.non_overlapping_levels` (planned): CQLite does not yet run a leveled compaction strategy; merge output parity for LCS-produced inputs is unmirrored. → _Re-parked to open tracker #905 (Support a compaction manager; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.LeveledGenerationsTest.level_generation_merge_order` (planned): CQLite does not yet model leveled generations; merge-order parity is unmirrored. → _Re-parked to open tracker #905 (Support a compaction manager; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.LongLeveledCompactionStrategyTest.concurrent_level_merge_safety` (planned): CQLite does not yet run concurrent leveled compaction; overlapping-range merge safety is unmirrored. → _Re-parked to open tracker #905 (Support a compaction manager; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.ParallelizedTasksTest.sharded_row_partition_integrity` (planned): CQLite does not run parallelized compaction tasks; sharded row/partition integrity over the produced output is unmirrored. → _Re-parked to open tracker #905 (Support a compaction manager; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.PartialCompactionsTest.tombstone_not_purged_excluded_sources` (planned): No dedicated fixture mirrors this excluded-source tombstone-retention case. → _Re-parked to open tracker #2303 (Commission the parity fixture library; predecessor #1387 closed COMPLETED, PR #1429)._
...
- `cass.compaction.SSTablesIteratedTest.merge_no_shadow_without_timestamp` (planned): No dedicated fixture mirrors the SSTablesIterated shadow-ordering case. → _Re-parked to open tracker #2303 (Commission the parity fixture library; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.ShardedCompactionWriterTest.shard_boundary_row_preservation` (planned): No dedicated fixture mirrors sharded-compaction-writer shard-boundary preservation. → _Re-parked to open tracker #2303 (Commission the parity fixture library; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.ShardedMultiWriterTest.flush_sharding_row_count` (planned): No dedicated fixture mirrors flush-triggered sharded multi-writer row counts. → _Re-parked to open tracker #2303 (Commission the parity fixture library; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.SingleSSTableLCSTaskTest.single_table_uplevel_metadata_readable` (planned): CQLite does not run LCS uplevel tasks; the single-table uplevel metadata path is unmirrored. → _Re-parked to open tracker #905 (Support a compaction manager; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.TimeWindowCompactionStrategyTest.ttl_window_expiry_purge` (partial): Byte-for-byte TTL-expiry compaction parity blocked on #1410. → _Un-#[ignore] ttl_expired_live_compaction_byte_for_byte and upgrade to byte_for_byte. #1410 (baseline fix, PR #1726) un-ignored the sibling row/RT byte tests, but this expired-TTL case stayed #[ignore] on #1538 (WriteWithTtl authoritative localExpirationTime), which is now closed COMPLETED (PR #2056) without un-ignoring it. Re-parked to open tracker #2303 pending a successor tracker for the residual authoritative-expiration wiring._
- `cass.compaction.UnifiedCompactionDensitiesTest.sstable_density_distribution` (planned): CQLite does not implement UCS density distribution; output sizing parity is unmirrored. → _Re-parked to open tracker #905 (Support a compaction manager; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compaction.UnifiedCompactionStrategyTest.gc_grace_purge_boundary` (planned): CQLite does not yet run the unified compaction strategy; gc_grace purge-boundary parity is unmirrored. → _Re-parked to open tracker #905 (Support a compaction manager; predecessor #1387 closed COMPLETED, PR #1429)._
...
- `cass.compaction_merge.byte_for_byte_output` (planned): No gated byte-for-byte comparison of compaction output. → _Promote the debug byte tier in compaction-parity to a gated comparison once writer output is byte-stable; the first gated cases now exist (row_delete_purge / range_tombstone_merge byte_for_byte, #1410). Re-parked to open tracker #2303 (predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.compression.CompressionUpgradeTest.codec_compat_across_upgrade` (planned): No dedicated fixture mirrors compressed SSTables produced across an upgrade boundary. → _Commission pre/post-upgrade compressed fixtures and assert decompression read parity. Re-parked to open tracker #1509._
- `cass.compression_checksum.checksum_trailer_detection` (partial): No gated test injects corruption and asserts rejection; only inline-checksum validation on read is implemented. (Byte-compare of clean Digest.crc32 is already covered by cass.sstable_format.toc_component_manifest.) → _Byte-compare of clean Digest.crc32 is covered by cass.sstable_format.toc_component_manifest; corruption-detection / rejection remains open and is tracked by the carved-out scrub/verify issue #1236. Re-parked to open tracker #1509._
- `cass.compression_info.deflate.real_fixture_chunks.strict` (planned): No real Cassandra Deflate-compressed fixture in the corpus. → _Generate a DeflateCompressor SSTable fixture and add it to the dataset; the strict lane will then decode and round-trip it. Re-parked to open tracker #1509 (Commission compression + checksum parity fixtures; successor to closed #970/#996/#1236; predecessor #1402 closed COMPLETED, PR #1523)._
- `cass.compression_info.zstd.real_fixture_chunks.strict` (planned): No real Cassandra Zstd-compressed fixture in the corpus (non-dictionary). → _Generate a non-dictionary ZstdCompressor SSTable fixture; the strict lane will then decode and round-trip it. Re-parked to open tracker #1509 (Commission compression + checksum parity fixtures; successor to closed #970/#996/#1236; predecessor #1402 closed COMPLETED, PR #1523)._
...
- `cass.filter_db.bti_membership` (partial): No raw-partition-key source for BTI fixtures, so the no-false-negative probe cannot run against da Filter.db. → _Recover raw BTI partition keys (e.g. by decoding partitions during a Data.db scan) and extend the no-false-negative gate to cover da fixtures. Re-parked to open tracker #1511._
- `cass.filter_db_bloom.SerializationsTest.bloom_filter_deserialization` (planned): No dedicated fixture mirrors the bloom-filter serialization round-trip cases from SerializationsTest. → _Commission bloom-filter serialization fixtures and assert deserialized membership goldens. Re-parked to open tracker #1511._
- `cass.index_db.RowIndexTest.row_index_sort_order` (planned): No dedicated fixture mirrors the RowIndex out-of-order / duplicate-key cases. → _Commission RowIndex fixtures with out-of-order/duplicate keys and assert offset/order goldens. Re-parked to open tracker #1511._
- `cass.index_summary.column_index.range_tombstone_boundary_big_bti` (partial): BTI (da) range-tombstone-at-block-edge fixtures are not yet generated (no da tombstone generator). → _Re-parked to open tracker #2303 (Commission the parity fixture library; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.repair.PendingAntiCompactionTest.pending_repair_metadata` (partial): No SSTable carrying a real pendingRepair session UUID exists in the corpus; such a file is produced only by an in-flight incremental repair / PendingAntiCompaction on a live cluster. The UUID parse/preserve/reject path is proven via synthetic bytes + a writer round-trip. → _Commission a live-cluster PendingAntiCompaction fixture (an SSTable with a real pendingRepair UUID) + sstablemetadata goldens, then assert the decoded UUID against the reference to promote to mirrored. Re-parked to open tracker #1508._
- `cass.repaired_metadata.statistics_db.pending_repair_uuid` (planned): No Cassandra 5.0 pending-repair fixture available; the reference null (`Pending repair: --`) state is confirmed, and the read path reports the field as Unparsed (it is not walked from bytes) rather than a fabricated absent value. → _When a pending-repair fixture is generated, decode the pendingRepair UUID (type-aware skip past improvedMinMax + commitLogIntervals) — promoting the field from RepairField::Unparsed to Decoded — and assert it byte-for-byte against the `Pending repair: <uuid>` reference line. Re-parked to open tracker #1508._
- `cass.repaired_metadata.statistics_db.transient_repair_flag` (planned): No transiently-replicated fixture available; the reference `IsTransient: false` state is confirmed, and the read path reports the field as Unparsed (it is not walked from bytes) rather than a fabricated `false`. → _When a transient-replication fixture is generated, decode the isTransient flag (after the version-gated improvedMinMax block and commitLogIntervals) — promoting it from RepairField::Unparsed to Decoded — and assert it byte-for-byte against the `IsTransient: true` reference line. Re-parked to open tracker #1508._
- `cass.schema_evolution.dropped_column.empty_index_block_reverse_scan` (partial): A wide dropped-column empty-index-block reverse-scan fixture is not yet generated. → _Generate a wide dropped-column fixture that yields an empty index block under reverse scan and assert parity; file a follow-up issue. Re-parked to open tracker #1511._
- `cass.sstable_format.SSTableIdGenerationTest.mixed_id_scheme_readable` (planned): No dedicated fixture mirrors a mixed sequential/UUID id-scheme generation set. → _Commission a fixture mixing sequential and UUID SSTable ids and assert all generations are discovered/readable. Re-parked to open tracker #1511._
- `cass.sstable_format.VersionSupportedFeaturesTest.version_feature_matrix` (planned): No dedicated fixture mirrors the complete version/supported-features matrix. → _Commission fixtures spanning the version matrix and assert feature-support resolution goldens. Re-parked to open tracker #1511._
- `cass.statistics_db.SSTableMetadataTest.max_local_deletion_time` (planned): STATS-section max timestamp / max local-deletion-time not yet decoded. → _Decode the STATS MetadataType component and assert max timestamp / max local-deletion-time against the reference dump. Re-parked to open tracker #1507._
- `cass.statistics_db.clustering_key_bounds` (planned): Covered-clustering min/max bounds not yet decoded from the STATS component. → _Decode the STATS-section clustering bounds and compare against the "Covered clusterings" reference line. Re-parked to open tracker #1507._
- `cass.statistics_db.histograms_and_estimates` (planned): STATS-section histograms and partition/row estimates not yet decoded. → _Decode the STATS-section EstimatedHistograms and count estimates and compare bucket boundaries against the reference dump. Re-parked to open tracker #1507._
- `cass.summary_db.IndexSummaryRedistributionTest.downsampled_summary_entries` (planned): No downsampled (sampling_level < 128) Summary.db fixture exists. → _Publish a redistributed Summary.db fixture and extend the strict suite to assert downsampled offset tables and size_at_full_sampling > entry count. Re-parked to open tracker #1507._
- `cass.tombstone_ttl.RangeTombstoneBurnTest.range_partition_column_deletes_readable` (planned): No dedicated burn-scale fixture mirrors the RangeTombstoneBurn cases. → _Re-parked to open tracker #2303 (Commission the parity fixture library; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.tombstone_ttl.RepairedDataTombstonesTest.repair_metadata_gates_purge` (partial): Repair-aware tombstone purge gating is unimplemented (and out of scope for #1021); only the parse/preserve/reject-mixed prerequisite exists. No real repaired fixture exists in the corpus (the published Cassandra 5.0 datasets are all unrepaired; a repaired SSTable requires a live cluster + nodetool repair / anticompaction). → _File a follow-up to (1) commission a live-cluster repaired+unrepaired fixture with a cross-repair-boundary shadowing tombstone (nodetool repair / sstablerepairedset + sstabledump/sstablemetadata goldens), and (2) implement + test repair-aware tombstone purge gating against it. Re-parked to open tracker #1508._
- `cass.tombstone_ttl.SSTableExpiredBlockersTest.fully_expired_drop_blockers` (planned): Fully-expired-SSTable drop (and the overlapping-SSTable blocker check that gates it) is not implemented; expired data is filtered on read but not dropped. → _The fully-expired-SSTable drop feature shipped (#1388 closed COMPLETED, PR #1740); the residual is to commission the overlapping-SSTable expired-blocker fixture and wire the parity test that asserts a fully-expired SSTable shadowing an EXCLUDED overlap is NOT dropped. Re-parked to open tracker #2303 (Commission the parity fixture library)._
- `cass.tombstone_ttl.ViewComplexTombstoneTest.cell_tombstone_not_purged` (planned): No dedicated fixture mirrors the view complex cell-tombstone cases; MV maintenance is coordinator-side. → _Re-parked to open tracker #2303 (Commission the parity fixture library; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.tombstone_ttl.range_tombstone_boundaries` (partial): test_deltas dataset asset not published/enforced in CI (#701). → _Publish/commit the test_deltas delta-generation corpus and enforce scan_delta parity in CI. Re-parked to open tracker #1512 (successor to closed #701; predecessor #1387 closed COMPLETED, PR #1429)._
- `cass.tombstone_ttl.repaired_unrepaired_purge_gate` (partial): repairedAt / pendingRepair parsing is not implemented (gated on #968/#988), so the repaired-vs-unrepaired purge gate is only partially exercised. → _Parse repairedAt / pendingRepair from Statistics.db and gate purge on repair status (#968/#988). Re-parked to open tracker #1508._
- `cass.write_load_path.FlushingTest.flush_no_phantom_rows` (planned): No dedicated fixture mirrors the flush phantom-row / tombstone-placement case. → _Commission a flush fixture and assert no phantom rows and correct tombstone placement vs Cassandra output. Re-parked to open tracker #1510._
- `cass.write_load_path.MemtableQuickTest.flush_living_cells_survive` (planned): No dedicated fixture mirrors the living-cells-survive-flush case. → _Commission a flush fixture asserting living cells survive and tombstones do not over-purge. Re-parked to open tracker #1510._
- `cass.write_load_path.SSTableWriterTransactionTest.writer_output_components` (planned): No dedicated fixture mirrors committed writer-output component integrity for this case. → _Commission a writer fixture and assert committed output components match Cassandra (transaction-log abort lifecycle remains out of scope). Re-parked to open tracker #1510._
- `cass.write_load_path.flush.tombstone_and_ttl_artifacts` (partial): The static-row writer gap (row-level HAS_TIMESTAMP / PK liveness wrongly set on a static-only UPDATE) is now RESOLVED (issue #1196): CQLite emits the static block byte-identical to Cassandra (flags 0xa0, static cell carries its own explicit timestamp), verified file-vs-file against the committed test_writeparity.static_clustering_shape reference. Whole-artifact byte parity for the broader tombstone/TTL shape REMAINS blocked by wall-clock-derived localDeletionTime on TTL/DELETE cells (nowInSeconds, not reproducible across independent writers). → _The remaining blocker is intrinsic: TTL/DELETE cells carry a wall-clock-derived localDeletionTime (nowInSeconds) that cannot byte-match an independent writer, so they stay semantic-only by nature. A future upgrade to byte_for_byte would require a non-TTL static-tombstone reference whose deletion time is deterministic (explicit localDeletionTime) plus a strict Data.db byte diff. Re-parked to open tracker #1510._
...
- `cass.compaction.accord_journal_compaction_out_of_scope` — Accord transactional journal compaction
  - Safe wording: CQLite reads and compacts Cassandra 5.0 base-table SSTables; it does not reproduce Accord journal compaction.
...
- `cass.compaction.node_compaction_orchestration_out_of_scope` — Node-side compaction scheduling / strategy managers (LCS/UCS)
  - Safe wording: CQLite compacts base-table SSTables with STCS on explicit invocation, producing output equal to Cassandra's per the compaction_merge scenarios; it does not reproduce the node's compaction scheduler or strategy managers.
- `cass.compaction.secondary_structure_and_guardrail_compaction_out_of_scope` — Compaction of 2i/MV + write guardrails + only-purge-repaired GC
  - Safe wording: CQLite reads and compacts base-table SSTables honoring their repaired metadata; it does not reproduce 2i/MV rebuild, write guardrails, or the only-purge-repaired GC option.
- `cass.compression.codec_impl_and_cql_ddl_out_of_scope` — Zstd codec implementation + CQL compression DDL
  - Safe wording: CQLite reads compressed SSTables produced with any supported codec and their compression parameters; it does not reproduce the Java codec implementation or CQL compression DDL.
- `cass.sstable_format.UpgradeSSTablesTest.upgrade_rewrite_out_of_scope` — nodetool upgradesstables / recompress rewrite
  - Safe wording: CQLite reads and compacts SSTables that upgradesstables produced (nb/oa/da); it does not reproduce the upgrade/recompress rewrite itself.
- `cass.sstable_io.reader_construction_out_of_scope` — Cassandra-internal SSTableReader / BTI loading builder
  - Safe wording: CQLite reads the same Data.db/BTI bytes Cassandra's reader loads and validates the decoded rows; it does not reproduce the Java reader-construction plumbing.
...
- `cass.tombstone_ttl.streaming_tombstone_histogram_builder_out_of_scope` — Streaming tombstone drop-time histogram builder
  - Safe wording: CQLite reads the serialized tombstone histogram Cassandra wrote into Statistics.db; it does not reproduce the streaming histogram builder.
- `cass.tombstone_ttl.tombstone_warnings_and_guardrails_out_of_scope` — Tombstone-count warnings + partition-tombstone guardrails
  - Safe wording: CQLite correctly reads and applies tombstones from SSTables; it does not implement query-time tombstone warning/guardrail thresholds.
...
- `cass.compaction.sai_segment_flush_out_of_scope` — SAI on-disk index segment flushing (fault injection)
  - Safe wording: CQLite reads Cassandra 5.0 base-table SSTables; it does not reproduce SAI segment flushing or its failure handling.
...
- `cass.compression.zstd_dictionary_feature_out_of_scope` — Zstd compression-dictionary training / cache / CLI
  - Safe wording: CQLite reads Cassandra 5.0 compressed SSTables using the standard codecs; it does not reproduce the zstd dictionary training/cache/CLI feature.
...
| `cass.bti_big_version_matrix.bti_da_write_read` | nightly_docker | .github/workflows/nightly-docker-parity.yml |
...
| `cass.compaction.CompactionDeleteAndPurgeRowTest.row_delete_purge` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.compaction.CompactionDeletePKTest.partition_delete_preserved` | manual_debug | — |
| `cass.compaction.CompactionDeleteRowRangeTest.range_tombstone_merge` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
...
| `cass.compaction.ForceCompactionTest.major_compaction_tombstone_purge` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.compaction.GcCompactionTest.static_and_complex_columns_survive_gc` | required_parity | .github/workflows/parity-regen-matrix.yml |
...
| `cass.compaction.SSTableRewriterTest.output_component_integrity` | nightly_docker | .github/workflows/nightly-docker-parity.yml |
...
| `cass.compaction.SizeTieredCompactionStrategyTest.bucket_selection` | fast_pr | — |
| `cass.compaction.TimeWindowCompactionStrategyTest.ttl_window_expiry_purge` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
...
| `cass.compaction.accord_journal_compaction_out_of_scope` | manual_debug | — |
...
| `cass.compaction.issue_899_per_element_collection_compaction` | required_parity | .github/workflows/parity-regen-matrix.yml |
...
| `cass.compaction.node_compaction_orchestration_out_of_scope` | manual_debug | — |
| `cass.compaction.sai_segment_flush_out_of_scope` | manual_debug | — |
| `cass.compaction.secondary_structure_and_guardrail_compaction_out_of_scope` | manual_debug | — |
| `cass.compaction.udt_collections_with_udts` | required_parity | .github/workflows/live-cell-compaction-parity.yml |
| `cass.compaction.udt_frozen_person` | required_parity | .github/workflows/live-cell-compaction-parity.yml |
| `cass.compaction.udt_nested` | required_parity | .github/workflows/live-cell-compaction-parity.yml |
...
| `cass.compaction_merge.partial_source_retains_tombstones` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.compaction_merge.partition_delete_shadowing_across_skipped_sources` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.compaction_merge.resurrection_safety.overlapping_sources` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.compaction_merge.static_row.survives_tombstone_gc` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.compaction_merge.tombstone_ttl_shadowing` | manual_debug | — |
| `cass.compaction_parity.statistics.repaired_metadata_preserve` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.compression.CompressionUpgradeTest.codec_compat_across_upgrade` | manual_debug | — |
| `cass.compression.codec_impl_and_cql_ddl_out_of_scope` | manual_debug | — |
| `cass.compression.fixture_matrix.deflate` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.compression.fixture_matrix.incompressible_uncompressed_chunk` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.compression.fixture_matrix.lz4` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.compression.fixture_matrix.short_final_chunk` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.compression.fixture_matrix.snappy` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.compression.fixture_matrix.uncompressed_table` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.compression.fixture_matrix.zstd_no_dictionary` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
...
| `cass.compression.zstd_dictionary_feature_out_of_scope` | manual_debug | — |
...
| `cass.compression_info.fields.algorithm_name` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.compression_info.fields.chunk_length` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.compression_info.fields.chunk_offsets` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.compression_info.fields.data_length` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.compression_info.fields.max_compressed_length` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.compression_info.fields.options` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.compression_info.layout.no_crc_fields` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
...
| `cass.corruption.bti_partitions_footer_bit_flip` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.corruption.bti_rows_truncation` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.corruption.compression_info.bad_offset` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.corruption.data_db.bit_flip` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.corruption.data_db.truncation` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.corruption.digest_crc32_mismatch` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.corruption.filter_db.bit_flip_big` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.corruption.index_db.bit_flip_big` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.corruption.statistics_db.header_damage` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.corruption.summary_db_truncation` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.corruption.toc_missing_component` | exhaustive_regeneration | .github/workflows/parity-regen-matrix.yml |
| `cass.corruption_verify.component_corruption_detection` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.cql_types.boundaries.absent_vs_null_regular_columns` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.cql_types.boundaries.empty_collections` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.cql_types.boundaries.length_prefix_edges` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.cql_types.boundaries.null_empty_text_blob` | required_parity | .github/workflows/parity-regen-matrix.yml |
...
| `cass.cql_types.complex.frozen_udt_value` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.cql_types.complex.legacy_dropped_tuple_udt_fields` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.cql_types.complex.multicell_udt_collection_paths` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.cql_types.complex.nested_frozen_collections` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.cql_types.complex.tuple_field_order` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.cql_types.complex.udt_field_order_null_empty` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.cql_types.counters.canonical_jsonl_value` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.cql_types.counters.compacted_final_value` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.cql_types.counters.deleted_counter_shadowing` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.cql_types.counters.multi_sstable_increment_decrement_merge` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.cql_types.counters.single_sstable_context_decode` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
...
| `cass.data_db.inline_crc.bad_trailer_rejected` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.data_db.inline_crc.incompressible_uncompressed_chunk` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.data_db.inline_crc.offset_delta_minus_crc_length` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.data_db.inline_crc.short_final_chunk` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.data_db.inline_crc.valid_trailer` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
...
| `cass.filter_db.statistical_false_positive_rate` | nightly_docker | .github/workflows/nightly-docker-parity.yml |
...
| `cass.index_summary.column_index.range_tombstone_boundary_big_bti` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
...
| `cass.repair.PendingAntiCompactionTest.pending_repair_metadata` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.repair.RepairJobTest.merkle_tree_sync` | manual_debug | — |
| `cass.repair.RepairedDataInfoTest.repaired_metadata_roundtrip` | required_parity | .github/workflows/parity-regen-matrix.yml |
...
| `cass.schema_evolution.dropped_column.empty_index_block_reverse_scan` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.schema_evolution.dropped_column.per_cell_purge` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.schema_evolution.issue_847_dropped_column_filter` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.schema_evolution.issue_850_static_presence` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.schema_evolution.serialization_header.altered_column_type` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.schema_evolution.serialization_header.altered_then_dropped_column` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.schema_evolution.serialization_header.dropped_column_same_type` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.schema_evolution.serialization_header.frozen_multicell_collection_mismatch` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.schema_evolution.serialization_header.no_schema_change` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.schema_evolution.serialization_header.static_regular_kind_mismatch` | required_parity | .github/workflows/parity-regen-matrix.yml |
...
| `cass.serialization.SerializationHeaderTest.static_and_dropped_columns` | required_parity | .github/workflows/parity-regen-matrix.yml |
...
| `cass.serialization.SerializationMirrorTest.schema_evolution_ordering` | required_parity | .github/workflows/parity-regen-matrix.yml |
...
| `cass.sstable_format.UpgradeSSTablesTest.upgrade_rewrite_out_of_scope` | manual_debug | — |
...
| `cass.sstable_io.reader.tombstone_only_partition` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.sstable_io.reader_construction_out_of_scope` | manual_debug | — |
| `cass.sstable_io.scanner.tombstone_only_partition_ranges` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.sstable_scan.wide_partition.forward_reverse_bounds` | fast_pr | — |
...
| `cass.statistics_metadata.max_local_deletion_time.tombstones_ttl` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.statistics_metadata.serialization_header` | required_parity | .github/workflows/sstabledump-parity-gate.yml |
| `cass.statistics_metadata.tombstone_histogram.deletion_times` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
...
| `cass.tombstone_ttl.RepairedDataTombstonesTest.repair_metadata_gates_purge` | required_parity | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.SSTableExpiredBlockersTest.fully_expired_drop_blockers` | manual_debug | — |
...
| `cass.tombstone_ttl.deletion_markers.cell_delete` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.deletion_markers.partition_delete` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.deletion_markers.range_delete_bounds` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.deletion_markers.range_tombstone_boundary` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.deletion_markers.row_delete` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.gc_grace.partition_row_cell` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.never_purge.cell_row_partition` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.range_tombstone.closed_last_block` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.range_tombstone.index_block_first_marker` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.range_tombstone.index_block_last_marker` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.range_tombstone.open_ended_middle_block` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.range_tombstone_boundaries` | manual_debug | — |
| `cass.tombstone_ttl.repaired_unrepaired_purge_gate` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.skipped_sstable.partition_delete_reincluded` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.skipped_sstable.partition_delete_shadows_older_rows` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.static_row.dropped_static_header_preserved` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.static_row.with_row_cell_range_tombstones` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.streaming_tombstone_histogram_builder_out_of_scope` | manual_debug | — |
| `cass.tombstone_ttl.tombstone_warnings_and_guardrails_out_of_scope` | manual_debug | — |
| `cass.tombstone_ttl.ttl_and_local_deletion_time` | required_parity | .github/workflows/sstabledump-parity-gate.yml |
| `cass.tombstone_ttl.ttl_cells.local_deletion_time` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.ttl_cells.mixed_expiring_and_live` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tombstone_ttl.ttl_expiry.gc_before_boundary` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.tools_cli.StandaloneUpgraderOnSStablesTest.standalone_upgrade` | manual_debug | — |
| `cass.verify.component_presence` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.verify.compression_info_parse` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.verify.digest_crc32_match` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.verify.full_row_scan` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.verify.healthy_compressed_sstable` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.verify.healthy_uncompressed_sstable` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.verify.inline_crc_validation` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
| `cass.verify.no_silent_empty_result_on_corruption` | nightly_docker | .github/workflows/parity-regen-matrix.yml |
...
| `cass.write_load_path.live_readback.semantic_only` | nightly_docker | .github/workflows/nightly-docker-parity.yml |
...
| `cass.compaction.SSTableRewriterTest.output_component_integrity` | nb | compaction-parity/build/parity-artifacts-byteParity/<scenario>/cassandra-output/<br>_fail:_ artifact.nightly_docker.live_logs |
...
| `cass.compaction.SizeTieredCompactionStrategyTest.bucket_selection` | nb, oa, da | test-data/parity/stcs_bucket_selection_vectors.jsonl |
...
| `cass.compaction.accord_journal_compaction_out_of_scope` | — | — |
...
| `cass.compaction.node_compaction_orchestration_out_of_scope` | — | — |
| `cass.compaction.sai_segment_flush_out_of_scope` | — | — |
| `cass.compaction.secondary_structure_and_guardrail_compaction_out_of_scope` | — | — |
| `cass.compaction.udt_collections_with_udts` | nb | test-data/datasets/sstables/test_compactionparityudt/udt_collections-3de4104073fe11f1ba13ff2af5955bf3/nb-3-big-Data.db<br>test-data/datasets/sstables/test_compactionparityudt/udt_collections-3de4104073fe11f1ba13ff2af5955bf3/nb-3-big-Data.db.jsonl<br>test-data/datasets/sstables/test_compactionparityudt/udt_collections-3de4104073fe11f1ba13ff2af5955bf3/nb-3-big-Index.db<br>test-data/datasets/sstables/test_compactionparityudt/udt_collections-3de4104073fe11f1ba13ff2af5955bf3/nb-3-big-Summary.db<br>test-data/datasets/sstables/test_compactionparityudt/udt_collections-3de4104073fe11f1ba13ff2af5955bf3/nb-3-big-Digest.crc32<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compaction.udt_frozen_person` | nb | test-data/datasets/sstables/test_compactionparityudt/udt_frozen_person-3dcdc92073fe11f1ba13ff2af5955bf3/nb-3-big-Data.db<br>test-data/datasets/sstables/test_compactionparityudt/udt_frozen_person-3dcdc92073fe11f1ba13ff2af5955bf3/nb-3-big-Data.db.jsonl<br>test-data/datasets/sstables/test_compactionparityudt/udt_frozen_person-3dcdc92073fe11f1ba13ff2af5955bf3/nb-3-big-Index.db<br>test-data/datasets/sstables/test_compactionparityudt/udt_frozen_person-3dcdc92073fe11f1ba13ff2af5955bf3/nb-3-big-Summary.db<br>test-data/datasets/sstables/test_compactionparityudt/udt_frozen_person-3dcdc92073fe11f1ba13ff2af5955bf3/nb-3-big-Digest.crc32<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compaction.udt_nested` | nb | test-data/datasets/sstables/test_compactionparityudt/udt_nested-3dda9a6073fe11f1ba13ff2af5955bf3/nb-3-big-Data.db<br>test-data/datasets/sstables/test_compactionparityudt/udt_nested-3dda9a6073fe11f1ba13ff2af5955bf3/nb-3-big-Data.db.jsonl<br>test-data/datasets/sstables/test_compactionparityudt/udt_nested-3dda9a6073fe11f1ba13ff2af5955bf3/nb-3-big-Index.db<br>test-data/datasets/sstables/test_compactionparityudt/udt_nested-3dda9a6073fe11f1ba13ff2af5955bf3/nb-3-big-Summary.db<br>test-data/datasets/sstables/test_compactionparityudt/udt_nested-3dda9a6073fe11f1ba13ff2af5955bf3/nb-3-big-Digest.crc32<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.compression.fixture_matrix.uncompressed_table` | nb | test-data/datasets/sstables/test_comp/uncompressed_table-25a5ca7071a911f19b3225f9984c6a77/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.exhaustive_regeneration.audit_report |
...
| `cass.compression.zstd_dictionary_feature_out_of_scope` | — | — |
| `cass.compression_checksum.checksum_trailer_detection` | da | test-data/datasets/sstables/test_da/simple_table-de1be8b064e711f19ad401a8c8227b11/da-2-bti-Digest.crc32<br>_fail:_ artifact.fast_pr.checksum_diff |
| `cass.compression_checksum.chunk_offsets_and_crc` | nb | test-data/datasets/sstables/test_basic/simple_table-6aa08200a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>test-data/datasets/sstables/test_collections/collection_table-6b8c8fb0a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl |
| `cass.compression_info.CompressedInputStreamTest.truncated_chunk_detection` | nb | test-data/datasets/sstables/test_basic/compression_test_table-6ad6ad30a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.CompressedInputStreamTest.truncated_chunk_detection.strict` | nb, oa, da | test-data/datasets/sstables/test_basic/composite_key_table-6ab56990a25111f0a3fef1a551383fb9/nb-1-big-TOC.txt<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.CompressedRandomAccessReaderTest.chunk_offsets` | nb | test-data/datasets/sstables/test_basic/compression_test_table-6ad6ad30a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.CompressedRandomAccessReaderTest.chunk_offsets.strict` | nb, oa, da | test-data/datasets/sstables/test_basic/composite_key_table-6ab56990a25111f0a3fef1a551383fb9/nb-1-big-TOC.txt<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.CompressedSequentialWriterTest.chunk_boundaries` | nb | test-data/datasets/sstables/test_basic/compression_test_table-6ad6ad30a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.CompressedSequentialWriterTest.chunk_boundaries.strict` | nb, oa, da | test-data/datasets/sstables/test_basic/compression_test_table-6ad6ad30a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.CompressionMetadataTest.metadata_serialization` | nb | test-data/datasets/sstables/test_basic/compression_test_table-6ad6ad30a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.CompressionMetadataTest.metadata_serialization.strict` | nb, oa, da | test-data/datasets/sstables/test_basic/composite_key_table-6ab56990a25111f0a3fef1a551383fb9/nb-1-big-TOC.txt<br>test-data/datasets/sstables/test_oa/collection_table-4b892c6064e711f1bd3ac7dbf655c673/oa-2-big-TOC.txt<br>test-data/datasets/sstables/test_da/collection_table-de2c155064e711f19ad401a8c8227b11/da-2-bti-TOC.txt<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.DirectCompressedChunkReaderTest.inline_crc_validation` | nb | test-data/datasets/sstables/test_basic/compression_test_table-6ad6ad30a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.DirectCompressedChunkReaderTest.inline_crc_validation.strict` | nb, oa, da | test-data/datasets/sstables/test_da/collection_table-de2c155064e711f19ad401a8c8227b11/da-2-bti-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.StandardCompressedChunkReaderTest.round_trip_chunk_bytes` | nb | test-data/datasets/sstables/test_basic/compression_test_table-6ad6ad30a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.deflate.real_fixture_chunks.strict` | — | — |
| `cass.compression_info.fields.algorithm_name` | nb | test-data/datasets/sstables/test_comp/lz4_table-25801a0071a911f19b3225f9984c6a77/nb-1-big-CompressionInfo.db.txt<br>test-data/datasets/sstables/test_comp/snappy_table-2588f3a071a911f19b3225f9984c6a77/nb-1-big-CompressionInfo.db.txt<br>test-data/datasets/sstables/test_comp/deflate_table-2592698071a911f19b3225f9984c6a77/nb-1-big-CompressionInfo.db.txt<br>test-data/datasets/sstables/test_comp/zstd_table-259ca2b071a911f19b3225f9984c6a77/nb-1-big-CompressionInfo.db.txt<br>_fail:_ artifact.nightly_docker.live_logs |
| `cass.compression_info.fields.chunk_length` | nb | test-data/datasets/sstables/test_comp/lz4_table-25801a0071a911f19b3225f9984c6a77/nb-1-big-CompressionInfo.db.txt<br>_fail:_ artifact.nightly_docker.live_logs |
| `cass.compression_info.fields.chunk_offsets` | nb | test-data/datasets/sstables/test_comp/lz4_table-25801a0071a911f19b3225f9984c6a77/nb-1-big-CompressionInfo.db.txt<br>test-data/datasets/sstables/test_comp/snappy_table-2588f3a071a911f19b3225f9984c6a77/nb-1-big-CompressionInfo.db.txt<br>_fail:_ artifact.nightly_docker.live_logs |
| `cass.compression_info.fields.data_length` | nb | test-data/datasets/sstables/test_comp/lz4_table-25801a0071a911f19b3225f9984c6a77/nb-1-big-CompressionInfo.db.txt<br>_fail:_ artifact.nightly_docker.live_logs |
| `cass.compression_info.fields.max_compressed_length` | nb | test-data/datasets/sstables/test_comp/lz4_table-25801a0071a911f19b3225f9984c6a77/nb-1-big-CompressionInfo.db.txt<br>_fail:_ artifact.nightly_docker.live_logs |
...
| `cass.compression_info.layout.no_crc_fields` | nb | test-data/datasets/sstables/test_comp/lz4_table-25801a0071a911f19b3225f9984c6a77/nb-1-big-CompressionInfo.db.txt<br>_fail:_ artifact.nightly_docker.live_logs |
| `cass.compression_info.lz4.real_fixture_chunks` | nb | test-data/datasets/sstables/test_basic/compression_test_table-6ad6ad30a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.lz4.real_fixture_chunks.strict` | nb, oa, da | test-data/datasets/sstables/test_basic/composite_key_table-6ab56990a25111f0a3fef1a551383fb9/nb-1-big-TOC.txt<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.snappy.real_fixture_chunks` | nb | test-data/datasets/sstables/test_basic/simple_table-6aa08200a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.snappy.real_fixture_chunks.strict` | nb, oa, da | test-data/datasets/sstables/test_basic/composite_key_table-6ab56990a25111f0a3fef1a551383fb9/nb-1-big-TOC.txt<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.compression_info.zstd.real_fixture_chunks.strict` | — | — |
| `cass.corruption.bti_partitions_footer_bit_flip` | da, bti | test-data/datasets/corruption/test_comp_corrupt/corruption-manifest.yml<br>_fail:_ artifact.exhaustive_regeneration.audit_report |
| `cass.corruption.bti_rows_truncation` | da, bti | test-data/datasets/corruption/test_comp_corrupt/corruption-manifest.yml<br>_fail:_ artifact.exhaustive_regeneration.audit_report |
| `cass.corruption.compression_info.bad_offset` | nb | test-data/datasets/corruption/test_comp_corrupt/corruption-manifest.yml<br>test-data/datasets/corruption/test_comp_corrupt/corruption-sha256.txt<br>_fail:_ artifact.exhaustive_regeneration.audit_report |
...
| `cass.corruption.toc_missing_component` | nb | test-data/datasets/corruption/test_comp_corrupt/corruption-manifest.yml<br>test-data/datasets/corruption/test_comp_corrupt/corruption-sha256.txt<br>_fail:_ artifact.exhaustive_regeneration.audit_report |
| `cass.corruption_verify.component_corruption_detection` | nb, da | test-data/datasets/corruption/test_comp_corrupt/corruption-manifest.yml<br>test-data/datasets/sstables/test_comp/lz4_table-25801a0071a911f19b3225f9984c6a77/nb-1-big-Data.db.jsonl<br>test-data/datasets/corruption/test_comp_corrupt/corruption-sha256.txt<br>_fail:_ artifact.nightly_docker.live_logs |
...
| `cass.cql_types.boundaries.length_prefix_edges` | nb | test-data/datasets/sstables/test_types/nb_length_prefix_edges-4fba4770706211f197e20b846582ecc8/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.cql_types.collections.map_key_lookup_offsets` | nb | test-data/codec-vectors/collections.json<br>_fail:_ artifact.fast_pr.byte_diff |
| `cass.cql_types.collections.map_key_range_offsets` | nb | test-data/codec-vectors/collections.json<br>_fail:_ artifact.fast_pr.byte_diff |
| `cass.cql_types.collections.set_lookup_offsets` | nb | test-data/codec-vectors/collections.json<br>_fail:_ artifact.fast_pr.byte_diff |
| `cass.cql_types.collections.set_range_offsets` | nb | test-data/codec-vectors/collections.json<br>_fail:_ artifact.fast_pr.byte_diff |
| `cass.cql_types.collections.single_cell_multicell_equivalence` | nb | test-data/codec-vectors/collections.json<br>_fail:_ artifact.fast_pr.byte_diff |
| `cass.cql_types.collections.vint_element_count_boundaries` | nb | test-data/codec-vectors/collections.json<br>_fail:_ artifact.fast_pr.byte_diff |
...
| `cass.cql_types.primitives.fixed_width_vectors` | nb | test-data/codec-vectors/primitives.json<br>_fail:_ artifact.fast_pr.byte_diff |
| `cass.cql_types.primitives.invalid_length_rejection` | nb | test-data/codec-vectors/primitives.json<br>_fail:_ artifact.fast_pr.byte_diff |
| `cass.cql_types.primitives.temporal_vectors` | nb | test-data/codec-vectors/primitives.json<br>_fail:_ artifact.fast_pr.byte_diff |
| `cass.cql_types.primitives.text_blob_ascii_vectors` | nb | test-data/codec-vectors/primitives.json<br>_fail:_ artifact.fast_pr.byte_diff |
| `cass.cql_types.primitives.uuid_inet_vectors` | nb | test-data/codec-vectors/primitives.json<br>_fail:_ artifact.fast_pr.byte_diff |
| `cass.cql_types.primitives.varint_decimal_duration_vectors` | nb | test-data/codec-vectors/primitives.json<br>_fail:_ artifact.fast_pr.byte_diff |
| `cass.data_db.inline_crc.bad_trailer_rejected` | nb | test-data/datasets/sstables/test_comp/lz4_table-25801a0071a911f19b3225f9984c6a77/nb-1-big-CompressionInfo.db.txt<br>test-data/datasets/sstables/test_comp/lz4_table-25801a0071a911f19b3225f9984c6a77/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.nightly_docker.live_logs |
| `cass.data_db.inline_crc.incompressible_uncompressed_chunk` | nb | test-data/datasets/sstables/test_comp/incompressible_uncompressed_chunk-25b8dd4071a911f19b3225f9984c6a77/nb-1-big-CompressionInfo.db.txt<br>test-data/datasets/sstables/test_comp/incompressible_uncompressed_chunk-25b8dd4071a911f19b3225f9984c6a77/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.nightly_docker.live_logs |
| `cass.data_db.inline_crc.offset_delta_minus_crc_length` | nb | test-data/datasets/sstables/test_comp/lz4_table-25801a0071a911f19b3225f9984c6a77/nb-1-big-CompressionInfo.db.txt<br>test-data/datasets/sstables/test_comp/lz4_table-25801a0071a911f19b3225f9984c6a77/nb-1-big-Data.db.jsonl<br>test-data/datasets/sstables/test_comp/snappy_table-2588f3a071a911f19b3225f9984c6a77/nb-1-big-CompressionInfo.db.txt<br>_fail:_ artifact.nightly_docker.live_logs |
...
| `cass.data_db_decode.clustering_bounds.desc_order` | nb, oa | test-data/datasets/sstables/test_basic/composite_key_table-6ab56990a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.clustering_bounds.multi_column_prefix` | nb, oa | test-data/datasets/sstables/test_wide_rows/wide_partition_table-6d6d0f80a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.clustering_bounds.null_vs_empty` | nb, oa | test-data/datasets/sstables/test_basic/static_columns_table-6b0425d0a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.range_tombstone.bound_markers` | nb, oa | test-data/datasets/sstables/test_deltas/range_tombstones-298894f0701f11f1b5d1d98b0640ec05/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.range_tombstone.boundary_markers` | nb, oa | test-data/datasets/sstables/test_deltas/adjacent_ranges-29bdd5c0701f11f1b5d1d98b0640ec05/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.data_db_decode.serialization_mirror.multi_clustering_column_order` | nb, oa | test-data/datasets/sstables/test_basic/composite_key_table-6ab56990a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.static_rows.static_only_partition` | nb, oa | test-data/datasets/sstables/test_basic/static_columns_table-6b0425d0a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>test-data/datasets/sstables/test_deltas/static_with_rows-29a4cf80701f11f1b5d1d98b0640ec05/nb-1-big-Data.db<br>test-data/datasets/sstables/test_deltas/static_with_rows-29a4cf80701f11f1b5d1d98b0640ec05/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.static_rows.static_with_clustering_rows` | nb, oa | test-data/datasets/sstables/test_tomb/static_with_tombstones-4cdb9780702011f1b8f419c9a388d558/nb-1-big-Data.db<br>test-data/datasets/sstables/test_tomb/static_with_tombstones-4cdb9780702011f1b8f419c9a388d558/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.tombstone.cell_deletion_time` | nb, oa | test-data/datasets/sstables/test_deltas/cell_tombstones-29733830701f11f1b5d1d98b0640ec05/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.tombstone.partition_deletion_time` | nb, oa | test-data/datasets/sstables/test_deltas/partition_tombstones-299258f0701f11f1b5d1d98b0640ec05/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.tombstone.row_deletion_time` | nb, oa | test-data/datasets/sstables/test_deltas/row_tombstones-297f1f10701f11f1b5d1d98b0640ec05/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.ttl.local_deletion_time_delta` | nb, oa | test-data/datasets/sstables/test_deltas/ttl_cells-299c9220701f11f1b5d1d98b0640ec05/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.unfiltered_serializer.row_and_cell_flags` | nb, oa | test-data/datasets/sstables/test_basic/uncompressed_table-6aedb7a0a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.unfiltered_serializer.row_size_vints` | nb, oa | test-data/datasets/sstables/test_basic/uncompressed_table-6aedb7a0a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.data_db_decode.wide_partition.row_boundaries` | nb | test-data/datasets/sstables/test_big/wide_partition-ffe2ee50733111f19e8f6d08b8e7a294/nb-2-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.index_db.CorruptPrimaryIndexTest.big_primary_index_corruption` | nb, oa | test-data/datasets/sstables/test_basic/simple_table-6aa08200a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.index_db.RowIndexEntryTest.partition_offsets` | nb, oa | test-data/datasets/sstables/test_basic/simple_table-6aa08200a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.index_db.RowIndexEntryTest.promoted_index_entries` | nb | test-data/datasets/sstables/test_big/wide_partition-ffe2ee50733111f19e8f6d08b8e7a294/nb-2-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.index_db.RowIndexTest.row_index_sort_order` | nb, oa, big | — |
| `cass.index_db.SSTableReaderTest.point_lookup_offsets` | nb, oa | test-data/datasets/sstables/test_basic/simple_table-6aa08200a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.index_db.SSTableScannerTest.range_boundaries` | nb, oa | test-data/datasets/sstables/test_basic/simple_table-6aa08200a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.index_db.big.wide_partition_promoted_entries` | nb | test-data/datasets/sstables/test_big/wide_partition-ffe2ee50733111f19e8f6d08b8e7a294/nb-2-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.index_db.bti.index_component_discovery` | da | test-data/datasets/sstables/test_da/simple_table-de1be8b064e711f19ad401a8c8227b11/da-2-bti-TOC.txt<br>_fail:_ artifact.manual_debug.reproduction_bundle |
| `cass.index_db.promoted_index.clustering_bounds` | nb | test-data/datasets/sstables/test_big/wide_partition-ffe2ee50733111f19e8f6d08b8e7a294/nb-2-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.index_db.promoted_index.index_info_offsets` | nb | test-data/datasets/sstables/test_big/wide_partition-ffe2ee50733111f19e8f6d08b8e7a294/nb-2-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.index_db.promoted_index.range_tombstone_boundary_at_block_edge` | nb | test-data/datasets/sstables/test_big/wide_partition-ffe2ee50733111f19e8f6d08b8e7a294/nb-2-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.index_summary.summary_boundaries` | nb, oa, big | test-data/datasets/sstables/test_timeseries/app_metrics-6c87b890a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.sstable_format.CQLSSTableWriterTest.frozen_udt_roundtrip` | nb | test-data/datasets/sstables/test_compactionparityudt/udt_frozen_person-3dcdc92073fe11f1ba13ff2af5955bf3/nb-3-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.sstable_format.LegacySSTableTest.complex_udt_frozen_non_frozen` | nb | test-data/datasets/sstables/test_compactionparityudt/udt_collections-3de4104073fe11f1ba13ff2af5955bf3/nb-3-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.sstable_format.SSTableIdGenerationTest.mixed_id_scheme_readable` | nb, oa, big | — |
| `cass.sstable_format.UpgradeSSTablesTest.upgrade_rewrite_out_of_scope` | — | — |
...
| `cass.sstable_io.reader_construction_out_of_scope` | — | — |
...
| `cass.statistics_db.SSTableMetadataTrackingTest.timestamp_and_ttl_metadata` | nb, oa, da | test-data/datasets/sstables/test_basic/ttl_test_table-6af66a30a25111f0a3fef1a551383fb9/nb-1-big-Statistics.db.txt<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.statistics_db.SerializationHeaderTest.schema_evolution_header` | nb, oa, da | test-data/datasets/sstables/test_basic/static_columns_table-6b0425d0a25111f0a3fef1a551383fb9/nb-1-big-Statistics.db.txt<br>test-data/datasets/sstables/test_oa/udt_table-4b9f738064e711f1bd3ac7dbf655c673/oa-2-big-Statistics.db.txt<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.statistics_db.SerializationMirrorTest.column_ordering_metadata` | nb, oa, da | test-data/datasets/sstables/test_basic/composite_key_table-6ab56990a25111f0a3fef1a551383fb9/nb-1-big-Statistics.db.txt<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.statistics_db.clustering_key_bounds` | — | — |
| `cass.statistics_db.core_metadata_checksums` | nb, oa, da | test-data/datasets/sstables/test_da/simple_table-de1be8b064e711f19ad401a8c8227b11/da-2-bti-Statistics.db.txt<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.statistics_db.histograms_and_estimates` | — | — |
| `cass.statistics_metadata.max_local_deletion_time.tombstones_ttl` | nb | test-data/datasets/sstables/test_basic/ttl_test_table-6af66a30a25111f0a3fef1a551383fb9/nb-1-big-Statistics.db.txt<br>_fail:_ artifact.nightly_docker.live_logs |
...
| `cass.statistics_metadata.tombstone_histogram.deletion_times` | nb | test-data/datasets/sstables/test_tomb/tombstone_histogram-4ca1e9e0702011f1b8f419c9a388d558/nb-1-big-Statistics.db.txt<br>_fail:_ artifact.nightly_docker.live_logs |
...
| `cass.summary_db.IndexSummaryTest.first_last_key_boundaries` | nb, oa, big | test-data/datasets/sstables/test_timeseries/app_metrics-6c87b890a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.summary_db.IndexSummaryTest.offset_table_entries` | nb, oa, big | test-data/datasets/sstables/test_timeseries/app_metrics-6c87b890a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.summary_db.IndexSummaryTest.serialization_round_trip` | nb, oa, big | test-data/datasets/sstables/test_timeseries/app_metrics-6c87b890a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.summary_db.big.index_offset_references` | nb, oa, big | test-data/datasets/sstables/test_timeseries/app_metrics-6c87b890a25111f0a3fef1a551383fb9/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.summary_db.bti.summary_discovery_classification` | nb, oa, da, big, bti | test-data/datasets/sstables/test_da/simple_table-de1be8b064e711f19ad401a8c8227b11/da-2-bti-TOC.txt<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.tombstone_ttl.RangeTombstoneTest.marker_merge_and_persistence` | nb, oa | test-data/datasets/sstables/test_deltas/range_tombstones-298894f0701f11f1b5d1d98b0640ec05/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
| `cass.tombstone_ttl.RepairedDataTombstonesTest.repair_metadata_gates_purge` | nb | — |
| `cass.tombstone_ttl.SSTableExpiredBlockersTest.fully_expired_drop_blockers` | nb | — |
| `cass.tombstone_ttl.TTLExpiryTest.gc_boundary` | nb, oa | test-data/datasets/sstables/test_deltas/ttl_cells-299c9220701f11f1b5d1d98b0640ec05/nb-1-big-Data.db.jsonl<br>_fail:_ artifact.required_parity.byte_diff |
...
| `cass.tombstone_ttl.static_row.dropped_static_header_preserved` | nb | test-data/datasets/sstables/test_tomb/dropped_static_col-4cd18560702011f1b8f419c9a388d558/nb-1-big-Statistics.db.txt<br>_fail:_ artifact.nightly_docker.live_logs |
...
| `cass.tombstone_ttl.streaming_tombstone_histogram_builder_out_of_scope` | — | — |
| `cass.tombstone_ttl.tombstone_warnings_and_guardrails_out_of_scope` | — | — |
