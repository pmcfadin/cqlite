...
// ===== TEMPORARY MEASUREMENT (#3742) =====
#[test]
fn tmp_3742_measure_accumulator_zero_column() {
    let columns: Vec<ColumnInfo> = Vec::new();
    let rows: Vec<QueryRow> = (0..3)
        .map(|_| row(vec![("a", crate::types::Value::Integer(1))]))
        .collect();
    let mut acc = ArrowRowAccumulator::new(&columns);
    for r in &rows {
        acc.stage(r.clone());
    ...
    let b = acc.to_record_batch();
    eprintln!(
        "MEASURE acc.len()={} acc.to_record_batch ok={} num_rows={:?} err={:?}",
        acc.len(),
        b.is_ok(),
        b.as_ref().map(|x| x.num_rows()).ok(),
        b.as_ref().err().map(|e| e.to_string())
    );
    let r2 = rows_to_record_batch(&columns, &rows);
    eprintln!(
        "MEASURE rows_to_record_batch ok={} err={:?}",
        r2.is_ok(),
        r2.as_ref().err().map(|e| e.to_string())
    );
    // Zero rows too
    let empty: Vec<QueryRow> = Vec::new();
    let acc2 = ArrowRowAccumulator::new(&columns);
    eprintln!(
        "MEASURE zero-col zero-row: acc.len()={} to_record_batch err={:?} rows_to_record_batch err={:?}",
        acc2.len(),
        acc2.to_record_batch().err().map(|e| e.to_string()),
        rows_to_record_batch(&columns, &empty).err().map(|e| e.to_string())
    );
}
