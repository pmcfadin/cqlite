...
fn demo_node_types() {
    ...
    println!(
        ...
        single_node.find_transition(b'a').is_some()
    );
    ...
    println!(
        "  Sparse: {:?} -> {} transitions",
        sparse_node.node_type(),
        sparse_node.get_transitions().len()
    );
...
fn demo_byte_comparable_encoding() {
    ...
    for i in 1..encoded_texts.len() {
        assert!(
            encoded_texts[i - 1].1 < encoded_texts[i].1,
    ...
    for i in 1..encoded_ints.len() {
        assert!(
            encoded_ints[i - 1].1 < encoded_ints[i].1,
