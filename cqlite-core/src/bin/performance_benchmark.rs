...
impl PerformanceBenchmarkRunner {
    ...
    fn benchmark_vint_performance(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let test_values = vec![
            ...
            (
                vec![16384, 2097151, 1048576, 65535],
                "Large values (3+ bytes)",
            ),
        ];
        ...
        for (values, description) in test_values {
            ...
            let encoding_speed =
                (total_bytes as f64) / encoding_duration.as_secs_f64() / 1_000_000.0;
            ...
            let decoding_speed =
                (total_bytes as f64) / decoding_duration.as_secs_f64() / 1_000_000.0;

            println!(
                "  {} - Encoding: {:.1} MB/s, Decoding: {:.1} MB/s",
        ...
        println!(
            "  📈 Average Encoding Speed: {:.1} MB/s",
            avg_encoding_speed
        );
        println!(
            "  📈 Average Decoding Speed: {:.1} MB/s",
            avg_decoding_speed
        );
    ...
    fn generate_summary_report(&self) {
        ...
        for (_, result) in &self.results {
            ...
            println!(
                "  {} {}: {:.1} MB/s",
                status, result.name, result.throughput_mb_per_sec
            );
