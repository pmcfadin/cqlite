...
async fn test_compression_algorithm_selection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_cases = vec![
        ...
        (
            "Text data",
            b"Hello world! This is a sample text with some repetition.".repeat(50),
        ),
        ("Mixed data", {
    ...
    for (name, data) in test_cases {
        ...
        if data.len() > 100 && data.iter().all(|&b| b == data[0]) {
            ...
            assert_ne!(
                ratio_choice,
                cqlite_core::storage::sstable::compression::CompressionAlgorithm::None
            );
        }
    }
    ...
    while let Some(entry) = dir_entries.next_entry().await? {
        ...
        if path.is_dir() {
            ...
            if data_file.exists() {
                ...
                let reader =
                    StreamingSSTableReader::open(&data_file, config, platform.clone()).await?;
...
async fn test_memory_usage_monitoring(
    ...
    println!(
        "    📊 Buffer pool configured: {} MB",
        streaming_config.buffer_pool_size_mb
    );
    println!(
        "    📊 Memory limit: {} MB",
        streaming_config.max_memory_usage_mb
    );
...
async fn test_large_file_handling(
    ...
    if let Some((largest_file_path, file_size)) = largest_file {
        println!(
            "    🔍 Testing with largest file: {} ({:.2} MB)",
            largest_file_path.display(),
        ...
        let stats = reader.get_streaming_stats().await?;
        println!(
            "    📊 Memory usage: {:.2} MB (should be < 128 MB)",
        ...
        let table_name = largest_file_path
            .parent()
...
async fn test_real_data_integration(
    ...
    if results.is_empty() {
        ...
        println!(
            "  ✅ Integration test completed with {} benchmark results",
            results.len()
        );

        // Verify that streaming reader performs comparably
        let streaming_results: Vec<_> = results
            .iter()
            .filter(|r| r.reader_type.contains("Streaming"))
            .collect();

        if !streaming_results.is_empty() {
            let avg_ops_per_sec: f64 = streaming_results
                .iter()
