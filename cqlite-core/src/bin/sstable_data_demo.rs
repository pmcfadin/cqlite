...
async fn main() -> Result<()> {
    ...
    println!("{}", "🚀 SSTable Data Loading & Caching System Demo".cyan().bold());
    println!("===============================================\n");

    // Check if data directory exists
    if !args.data_dir.exists() {
        eprintln!("{} Data directory does not exist: {:?}", "Error:".red().bold(), args.data_dir);
        std::process::exit(1);
    }
...
impl SstableDataDemo {
    async fn new(args: &Args) -> Result<Self> {
        ...
        let schema_manager = Arc::new(
            SchemaManager::new(&temp_dir).await
        ...
        let data_manager = Arc::new(
            ...
            ).await.context("Failed to create data manager")?
        );
    ...
    async fn demo_discovery(&self) -> Result<()> {
        ...
        let discovery = self.data_manager.discover_tables(&self.args.data_dir).await
            .context("Failed to discover tables")?;

        println!("⏱️  Discovery completed in: {:.2}s", start_time.elapsed().as_secs_f64());
        ...
        for keyspace in &discovery.keyspaces {
            if !keyspace.tables.is_empty() {
                println!("\n  🔹 {} ({} tables):", keyspace.name.green(), keyspace.tables.len());
    ...
    async fn demo_data_loading(&self) -> Result<()> {
        ...
        let _discovery = self.data_api.initialize(&self.args.data_dir).await
        ...
        let target_keyspace = if let Some(ref ks) = self.args.keyspace {
            if keyspaces.contains(ks) {
                ...
                println!("⚠️  Specified keyspace '{}' not found, using first available", ks);
        ...
        println!("📊 Tables in {}: {}", target_keyspace, table_listing.tables.len());
        ...
        if let Some(table_info) = table_listing.tables.first() {
            ...
            let start_time = Instant::now();
            let result1 = self.data_api.select(
                table_name,
                None,
                None,
                Some(self.args.limit),
            ).await.context("First query failed")?;

            println!("  ⏱️  First query: {:.3}s (cache miss)", start_time.elapsed().as_secs_f64());
            ...
            let start_time = Instant::now();
            let result2 = self.data_api.select(
                table_name,
                None,
                None,
                Some(self.args.limit),
            ).await.context("Second query failed")?;

            println!("  ⏱️  Second query: {:.3}s (cache hit)", start_time.elapsed().as_secs_f64());
    ...
    async fn demo_schema_inference(&self) -> Result<()> {
        println!("{}", "🔍 Step 3: Schema Discovery & Inference".blue().bold());
        println!("=======================================\n");
        ...
        if let Some(keyspace) = keyspaces.first() {
            ...
            if let Some(table_info) = table_listing.tables.first() {
                let table_name = &table_info.name;
                println!("🔬 Analyzing table: {}.{}", keyspace.green(), table_name.cyan());
    ...
    async fn demo_query_performance(&self) -> Result<()> {
        ...
        println!("  🎯 Cache hit rate: {:.1}%", 
            ...
            (system_info.cache_stats.cache_hits + system_info.cache_stats.cache_misses).max(1) as f64 * 100.0
        );
        println!("  📁 Cache entries: {}", system_info.cache_stats.cache_entries);
        println!("  ⚡ Avg access time: {}μs", system_info.cache_stats.avg_access_time_micros);

        if self.args.metrics {
            println!("\n📈 Detailed Cache Statistics:");
            println!("  Cache hits: {}", system_info.cache_stats.cache_hits);
            println!("  Cache misses: {}", system_info.cache_stats.cache_misses);
            println!("  Current cache size: {}", format_bytes(system_info.cache_stats.current_cache_size_bytes as u64));
            println!("  Cache evictions: {}", system_info.cache_stats.evictions);
            println!("  Background operations: {}", system_info.cache_stats.background_operations);
        }
    ...
    async fn demo_system_stats(&self) -> Result<()> {
        ...
        println!("\n🔍 Discovery Status:");
        println!("  In progress: {}", if discovery_in_progress { "Yes" } else { "No" });
        ...
        println!("  Cache efficiency: {:.1}%", 
            if system_info.cache_stats.cache_hits + system_info.cache_stats.cache_misses > 0 {
                system_info.cache_stats.cache_hits as f64 / 
                (system_info.cache_stats.cache_hits + system_info.cache_stats.cache_misses) as f64 * 100.0
            } else {
                0.0
            }
        );
