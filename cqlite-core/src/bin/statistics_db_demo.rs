    ...
    match parse_enhanced_statistics_file(&file_data) {
        Ok((_, statistics)) => {
            ...
            println!(
                ...
                if statistics.header.version == 4 {
                    "Cassandra 5.0 'nb' format"
                } else {
                    "Other format"
                }
            );
            println!(
                "  Statistics Kind: 0x{:08X}",
                statistics.header.statistics_kind
            );
            ...
            println!(
                ...
                if statistics.row_stats.total_rows > 0 {
                    (statistics.row_stats.live_rows as f64 / statistics.row_stats.total_rows as f64)
                        * 100.0
                } else {
                    0.0
                }
            ...
            println!(
                "  Avg Rows/Partition: {:.1}",
                statistics.row_stats.avg_rows_per_partition
            );

            println!("\n💾 Table Statistics:");
            println!(
                "  Disk Size: {:.2} KB",
                statistics.table_stats.disk_size as f64 / 1024.0
            );
            println!(
                "  Uncompressed Size: {:.2} KB",
                statistics.table_stats.uncompressed_size as f64 / 1024.0
            );
            println!(
                "  Compression Ratio: {:.1}%",
                statistics.table_stats.compression_ratio * 100.0
            );
            println!("  Block Count: {}", statistics.table_stats.block_count);
            println!(
                "  Index Size: {:.2} KB",
                statistics.table_stats.index_size as f64 / 1024.0
            );
            println!(
                "  Bloom Filter Size: {:.2} KB",
                statistics.table_stats.bloom_filter_size as f64 / 1024.0
            );

            println!("\n🗂️ Partition Statistics:");
            println!(
                "  Avg Partition Size: {:.1} bytes",
                statistics.partition_stats.avg_partition_size
            );
            println!(
                "  Min Partition Size: {} bytes",
                statistics.partition_stats.min_partition_size
            );
            println!(
                "  Max Partition Size: {:.1} KB",
                statistics.partition_stats.max_partition_size as f64 / 1024.0
            );
            println!(
                "  Large Partitions: {:.1}%",
                statistics.partition_stats.large_partition_percentage
            );
            ...
            println!(
                "  Original Size: {:.2} KB",
                statistics.compression_stats.original_size as f64 / 1024.0
            );
            println!(
                "  Compressed Size: {:.2} KB",
                statistics.compression_stats.compressed_size as f64 / 1024.0
            );
            println!(
                "  Compression Speed: {:.1} MB/s",
                statistics.compression_stats.compression_speed
            );
            println!(
                "  Decompression Speed: {:.1} MB/s",
                statistics.compression_stats.decompression_speed
            );
            ...
            println!(
                ...
                analysis.compression_efficiency
            );
            println!(
