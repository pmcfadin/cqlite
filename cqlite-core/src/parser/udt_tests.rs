...
mod tests {
    ...
    fn test_udt_registry_dependency_validation() {
        ...
        let person_udt = registry
            .resolve_udt_with_dependencies("system", "person")
            .unwrap();
    ...
    fn test_udt_registry_circular_dependency_detection() {
        ...
        let self_referencing_udt = UdtTypeDef::new("test".to_string(), "circular".to_string())
            .with_field(
                "self_ref".to_string(),
                CqlType::Udt("circular".to_string(), vec![]),
                true,
            );
    ...
    fn test_udt_registry_dependent_udts() {
        ...
        let contact_info_depends_on_person =
            dependents.iter().any(|udt| udt.name == "contact_info");
    ...
    fn test_udt_parsing_with_registry() {
        ...
        let (remaining, parsed_value) =
            parse_udt_with_registry(&serialized, "person", "system", &registry).unwrap();
    ...
    fn test_frozen_udt_parsing() {
        ...
        let (remaining, parsed_value) =
            parse_frozen_udt_with_registry(&serialized, address_def, &registry).unwrap();
    ...
    fn test_complex_nested_structure() {
        ...
        assert!(matches!(
            emergency_contacts_field.field_type,
            CqlType::List(_)
        ));
    }
}
