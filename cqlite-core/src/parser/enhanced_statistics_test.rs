...
mod tests {
        ...
        for test_file_path in &test_files {
            ...
            match fs::read(path).await {
                Ok(file_data) => {
                    ...
                    for (i, chunk) in file_data[..std::cmp::min(64, file_data.len())]
                        .chunks(16)
                        .enumerate()
                    {
                        print!("    {:08x}  ", i * 16);
    ...
    fn validate_enhanced_statistics(stats: &SSTableStatistics, file_path: &str) {
        ...
        assert!(
            stats.header.data_length > 0,
            "Data length should be positive"
        );

        // Row statistics validation
        assert!(
            stats.row_stats.total_rows >= stats.row_stats.live_rows,
            "Total rows should be >= live rows"
        );
        assert!(
            stats.row_stats.partition_count > 0,
    ...
    fn test_enhanced_parser_components() {
        ...
        let result = parse_nb_format_statistics_data(&header, &insufficient_data, None);
        assert!(
            result.is_err(),
