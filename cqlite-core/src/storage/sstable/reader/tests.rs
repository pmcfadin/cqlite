...
mod tests {
    ...
    /// `PrefetchMode::Auto` MUST NOT map to `MADV_SEQUENTIAL` on the mmap
    /// backend, and (since #2824) DOES map to `MADV_WILLNEED`.
    ///
    /// The invariant #1143 needs is "`Auto` never yields `Sequential`", not
    /// "`Auto` yields nothing". `MADV_SEQUENTIAL`'s harm is **drop-behind**:
    /// pages are evicted as a scan passes them, so under concurrent write load
    /// the evicted pages are gone when an overlapping scan re-reads them,
    /// causing synchronous major page faults on the tokio worker and a ~2x
    /// read-side p99 tail regression. `MADV_WILLNEED` queues **asynchronous**
    /// read-ahead and has **no** drop-behind semantics, so it does not carry
    /// that mechanism — which is why #2824 could turn it on for the default
    /// path without reintroducing #1143. The old assert
    /// (`mmap_advice_for(Auto) == None`) stated the implementation of the day;
    /// this one states the invariant and is never to be deleted.
    ///
    /// This is deterministic and host-independent (it asserts the policy
    /// mapping, not timing), so it reliably fails if drop-behind is
    /// reintroduced — unlike a wall-clock tail guard, which cannot force
    /// page-cache reclaim on the tiny vendored fixtures. The integration guard
    /// `tests/issue_1143_mmap_prefetch_tail_guard.rs` stays observational only
    /// (it asserts nothing on timing), so THIS is the load-bearing #1143 pin.
    /// `Off` still issues no advice; explicit `Sequential`/`WillNeed` remain the
    /// caller's opt-in to those hints.
    #[cfg(unix)]
    #[test]
    fn test_mmap_advice_for_auto_is_willneed_never_sequential() {
        ...
        // (drop-behind). Asserted independently of what Auto DOES emit, so a
        // future policy change cannot quietly drop this protection.
        assert_ne!(
    ...
    /// Issue #2824: the DEFAULT [`StorageConfig`] resolves to `MADV_WILLNEED`
    /// advice on the mmap scan mapping.
    ///
    ...
    /// still selects `PrefetchMode::Auto`, so the behavioural change to the
    /// default read path is pinned where a reader looking at defaults will find
    /// it. A future default flip to `Off` would silently un-ship #2824 without
    /// touching `mmap_advice_for` at all.
    #[cfg(unix)]
    #[test]
    fn test_default_storage_config_advises_willneed() {
        ...
        assert_eq!(
            ...
            Some(memmap2::Advice::WillNeed),
            "issue #2824: the DEFAULT configuration must advise MADV_WILLNEED on \
             the scan mapping"
        );
    }
