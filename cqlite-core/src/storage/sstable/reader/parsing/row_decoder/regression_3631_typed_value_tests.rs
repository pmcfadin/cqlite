...
/// (`UserType extends TupleType`, cassandra-5.0.8).
...
/// A one-field `inner { a int }` UDT value carrying `a = 1`.
fn inner_a_1() -> Vec<u8> {
...
fn a_zero_count_collection_with_trailing_payload_is_refused_not_emptied() {
    ...
    let text = err.to_string();
    // The refusal is the UNIFIED consumption class — one `require_fully_consumed`
    // shared with the type-STRING side and with `cell_path_key.rs` (#3631's
    // one-implementation ruling, recorded on `main` beside the function #3811
    // shipped). So the message names how far the decode got and how far it had to
    // get: 4 of 32 bytes here, the 4-byte zero count and nothing else.
    assert!(
        text.contains("decoded only 4 of 32 byte(s)"),
        "the refusal must name the consumed/available counts in the shared \
         consumption class; got: {text}"
    );
}
...
// ════════════════════════════════════════════════════════════════════════════
// THE UDT BOUNDARY'S BYTE ACCOUNTING — COVERED ON `main`, DELIBERATELY NOT
// DUPLICATED HERE.
//
// An earlier revision of this file carried six cases over the exact/legally-short/
// trailing-garbage/partial-prefix matrix for both UDT decoders. Issue #3811 has
// since landed that enforcement INSIDE `parse_nested_udt_from_registry` and
// `parse_inline_udt_value` themselves, with its own matrix over four routes
// (`raw_value/issue_3811_consumption_demo_tests.rs`: `nested_udt_*`,
// `inline_udt_*`, `structural_nested_udt_*`, `frozen_udt_header_path_*`, plus
// `issue_3811_cell_site_tests.rs`'s cell-entry-point variants). #3631 delegates to
// those very functions, so re-asserting the same property here would be a second
// copy of someone else's oracle — exactly the duplication this file's own subject
// is about.
//
// What #3631 DOES own on this path is one thing main cannot see: that a nested UDT
// reached from the NEW `CqlType`-driven entry point is byte-checked AND
// depth-bounded, i.e. that the guard is on a route this issue actually takes. That
// is the last case in this file's column-level section.
// ════════════════════════════════════════════════════════════════════════════
...
// The three consumption cases that used to sit here — exact / trailing-garbage /
// partial-prefix on a top-level frozen-UDT column — are covered on `main` by
// #3811's `frozen_udt_header_path_*` matrix in
// `raw_value/issue_3811_consumption_demo_tests.rs` and are not duplicated. What is
// #3631's own is the case below: the NESTED decode reached through the new
// `CqlType` entry point, which is both byte-checked and depth-bounded.
