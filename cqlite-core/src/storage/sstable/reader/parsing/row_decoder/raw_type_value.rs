            ...
            normalized if Self::is_udt_type(normalized) => {
                ...
                for field_def in &udt_def.fields {
                    ...
                    let field_value = if field_len == -1 {
                        ...
                        // ONE per-field entry (issue #3631, roborev round 3). This was
                        // the THIRD and FOURTH copy of the same dispatch — a
                        // registry-present match and a no-registry match, each with its
                        // own nested-UDT resolution, its own frozen wrapping and its own
                        // `Value::Blob` fallback. `parse_simple_udt_field_value_at`
                        // expresses all of it once, threads `depth`, routes through the
                        // single exhaustion assert, and returns an explicit `Error`
                        // naming a UDT it cannot resolve instead of silently degrading
                        // (#3631 criterion 5). Registry-then-inline preference now lives
                        // in `typed_value.rs::parse_typed_udt` alone, so the
                        // registry/no-registry split here is redundant: it consulted the
                        // very `self.udt_registry` the delegate consults.
                    ...
                    if let Some(udt_def) = registry.get_udt_qualified(&self.keyspace, type_str) {
                        ...
                        for field_def in &udt_def.fields {
                            ...
                            let field_value = if field_len == -1 {
                                ...
                                // ONE per-field entry — the FIFTH and last copy of this
                                // dispatch (issue #3631, roborev round 3). See the
                                // sibling comment above: every property (depth,
                                // consumption, no silent blob) now holds because there is
                                // ONE implementation instead of five, which is what three
                                // rounds of per-site fixes could not achieve.
                                let value = self.parse_simple_udt_field_value_at(
                                    field_data,
                                    &field_def.field_type,
                                    depth,
