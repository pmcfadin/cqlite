...
impl SchemaAwareReader {
    ...
    fn validate_schema_completeness(schema: &TableSchema) -> Result<()> {
        ...
        if !schema.clustering_keys.is_empty() {
            let mut positions: Vec<usize> = schema.clustering_keys.iter().map(|k| k.position).collect();
            positions.sort();
            for (expected, &actual) in positions.iter().enumerate() {
                if expected != actual {
                    return Err(Error::Schema(format!(
    ...
    pub async fn scan(
        ...
        let table_id = self.get_table_id();
        let raw_results = self.reader.scan(
            &table_id,
            start_key.as_ref(),
            end_key.as_ref(),
            limit,
    ...
    pub fn parse_row_key(&self, row_key: &RowKey) -> Result<(Vec<Value>, Vec<Value>)> {
        ...
        let clustering_values = if key_bytes.len() > partition_length {
            self.schema_parser.parse_clustering_keys(&key_bytes[partition_length..])?
        } else {
            Vec::new()
        };
    ...
    pub fn parse_row_value(&self, raw_value: &Value) -> Result<HashMap<String, Value>> {
        ...
        for column in &self.context.schema.columns {
            ...
            let column_value = self.schema_parser.parse_column_value(
                &column.name,
                &value_bytes[offset..],
            )?;
    ...
    pub fn table_name(&self) -> String {
        format!("{}.{}", self.context.schema.keyspace, self.context.schema.table)
    }
    ...
    fn create_row_key(&self, partition_key: &[Value], clustering_key: Option<&[Value]>) -> Result<RowKey> {
        // Serialize partition key
        let mut key_bytes = Vec::new();
        for (value, comparator) in partition_key.iter().zip(&self.context.partition_comparators) {
    ...
    fn serialize_value_with_comparator(&self, value: &Value, _comparator: &ComparatorType) -> Result<Vec<u8>> {
    ...
    fn calculate_partition_key_length(&self, partition_values: &[Value]) -> Result<usize> {
        ...
        for (value, comparator) in partition_values.iter().zip(&self.context.partition_comparators) {
            let serialized = self.serialize_value_with_comparator(value, comparator)?;
    ...
    fn get_table_id(&self) -> crate::types::TableId {
        ...
        crate::types::TableId::from(format!("{}.{}", 
            self.context.schema.keyspace,
            self.context.schema.table
        ))
    }
}
