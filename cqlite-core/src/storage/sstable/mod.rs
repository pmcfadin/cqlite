...
impl SSTableManager {
    ...
    pub async fn create_from_memtable(
        ...
        let mut writer = writer::SSTableWriter::create(
            &file_path,
            &self.config,
            self.platform.clone(),
        ).await?;
    ...
    pub async fn stats(&self) -> Result<SSTableStats> {
        ...
        Ok(SSTableStats {
            ...
            total_entries,
            total_tables,
            average_size: if sstable_count > 0 { total_size / sstable_count as u64 } else { 0 },
        })
    }
...
mod tests {
    ...
    async fn test_sstable_manager_creation() {
        ...
        let manager = SSTableManager::new(temp_dir.path(), &config, platform).await.unwrap();
