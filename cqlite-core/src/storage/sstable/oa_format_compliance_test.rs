...
mod tests {
    ...
    fn test_magic_number_exact_compliance() {
        ...
        assert!(result.is_ok(), "Valid 'oa' header should parse successfully");
    }
    ...
    fn test_invalid_magic_numbers() {
        ...
        for (magic_bytes, description) in invalid_magic_tests {
            ...
            let result = reader.parse_oa_header(&header_data);
            assert!(result.is_err(), "Should reject invalid magic: {}", description);
            
            match result.unwrap_err() {
                Error::InvalidFormat(msg) => {
                    assert!(msg.contains("magic number") || msg.contains("Invalid format"),
                           "Error should mention magic number: {}", msg);
                }
    ...
    fn test_version_validation() {
        ...
        for version_bytes in unsupported_versions {
            ...
            let result = reader.parse_oa_header(&header_data);
            assert!(result.is_err(), "Should reject unsupported version: {:?}", version_bytes);
        }
    }
    ...
    fn test_reserved_bytes_validation() {
        ...
        assert!(result.is_ok(), "Implementation should be tolerant of reserved bytes");
    }
    ...
    fn test_vint_decoding_spec_compliance() {
        ...
        for (bytes, expected_value, expected_length, description) in vint_test_cases {
            ...
            assert!(result.is_ok(), "VInt should decode successfully: {}", description);
            
            let (value, consumed) = result.unwrap();
            assert_eq!(value, expected_value, "Wrong value for {}: expected {}, got {}", 
                      description, expected_value, value);
            assert_eq!(consumed, expected_length, "Wrong length for {}: expected {}, got {}", 
    ...
    fn test_vint_error_handling() {
        ...
        for (bytes, description) in error_cases {
            let result = reader.read_vint(&bytes);
            assert!(result.is_err(), "Should fail for invalid VInt: {}", description);
        }
    }
    ...
    fn test_big_endian_compliance() {
        ...
        assert_eq!(result.format_version, 0x0001, 
    ...
    fn test_header_size_compliance() {
        ...
        assert!(result.is_ok(), "Should handle oversized input by reading first 32 bytes");
    }
    ...
    fn test_timestamp_format_compliance() {
        ...
        assert_eq!(min_timestamp, 1640995200000000i64, "Min timestamp should be in microseconds");
        assert_eq!(max_timestamp, 1640995260000000i64, "Max timestamp should be in microseconds");
        
        // Verify chronological order
        assert!(min_timestamp <= max_timestamp, "Min timestamp should be <= Max timestamp");
    }
    ...
    fn test_partition_data_structure() {
        ...
        assert_eq!(partition_count, 100, "Should correctly parse partition count");
    ...
    fn test_token_coverage_format() {
        ...
        assert_eq!(token_section[0], 0x02, "Should have 2 token ranges as specified");
        ...
        for range_idx in 0..2 {
            ...
            assert!(start_result.is_ok(), "Should parse start token for range {}", range_idx);
            ...
            assert!(end_result.is_ok(), "Should parse end token for range {}", range_idx);
    ...
    fn test_compression_offset_format() {
        ...
        assert_eq!(compression_offset, 1024, "Compression offset should be big-endian encoded");
    }
    ...
    fn test_spec_regression_prevention() {
        ...
        assert!(result.is_ok(), "Should parse real Cassandra 5.0 header pattern");
        ...
        for format_bytes in other_formats {
            ...
            assert!(result.is_err(), "Should reject non-'oa' format: {:?}", format_bytes);
        }
    }
