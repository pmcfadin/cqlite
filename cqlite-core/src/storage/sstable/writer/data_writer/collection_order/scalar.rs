//! Scalar leaf comparators for SET-element / MAP-key ordering (issue #1275).
//!
//! These mirror the per-type Cassandra `AbstractType.compare` for the SCALAR
//! element/key types whose order differs from raw unsigned serialized bytes:
//! signed integers, `Float.compare`/`Double.compare` (Java NaN-last total order),
//! signed `varint`, scale-aware `decimal`, and `UUIDType`. The composite
//! comparators (issue #1296) recurse back into the public dispatcher in `mod.rs`,
//! which bottoms out here at the leaves — these are the SINGLE source of the
//! signed/float/varint/decimal/uuid order and are never reimplemented elsewhere.
...
pub(super) fn compare_signed_varint(a: &[u8], b: &[u8]) -> Ordering {
...
pub(super) fn compare_uuid_cassandra(a: &[u8; 16], b: &[u8; 16]) -> Ordering {
...
/// time. All shifts are on `u64` to match Java `>>>` (unsigned) and `<<`.
...
#[cfg(test)]
mod tests {
    use super::super::compare_collection_elements;
    use super::*;
