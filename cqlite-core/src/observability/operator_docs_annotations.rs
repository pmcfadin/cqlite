...
pub(super) const ANNOTATIONS: &[MetricDoc] = &[
    ...
    MetricDoc {
        ...
        kind: MetricKind::Counter,
        unit: catalog::unit::DIMENSIONLESS,
        summary: "Accesses attributable to each repeat-access bucket over one closed probe window (#2827) — the sum of the repeat counts of the partitions in that bucket.",
        attributes: &[attr::REPEAT_BUCKET],
        interpretation: "accesses minus distinct_partitions is the number of reads a cache holding that bucket could have served; equal values mean every access was a first touch (nothing to cache).",
        round_item: "—",
    },
    MetricDoc {
