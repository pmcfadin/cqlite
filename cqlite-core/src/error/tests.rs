//! Unit tests for the [`crate::error`] module.
//!
//! Split out of `error.rs` under the campsite rule (epic #1116 / #1135): the
//! module was 913 lines against the 800-line source threshold, so any change to
//! it — such as issue #3723's new `FixedWidthLengthMismatch` variant — tripped
//! the gate's `file-size` ratchet.
