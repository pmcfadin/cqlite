...
impl CqlGenerator {
    ...
    fn build_complete_script(
        ...
        if self.config.include_comments {
            writeln!(script, "-- CQL Schema Generated by CQLite").unwrap();
            writeln!(script, "-- Target Version: {:?}", self.config.target_version).unwrap();
            writeln!(script, "-- Generated at: {:?}", std::time::SystemTime::now()).unwrap();
        ...
        if !udt_definitions.is_empty() {
            if self.config.include_comments {
                writeln!(script, "-- User-Defined Types").unwrap();
            }
            for udt in udt_definitions {
                writeln!(script, "{}", udt).unwrap();
            }
            writeln!(script).unwrap();
        }
        ...
        // Main table
        if self.config.include_comments {
            writeln!(script, "-- Main Table").unwrap();
        }
        writeln!(script, "{}", create_table).unwrap();

        // Indexes
        if !index_definitions.is_empty() {
            writeln!(script).unwrap();
            if self.config.include_comments {
                writeln!(script, "-- Secondary Indexes").unwrap();
            }
            for index in index_definitions {
                writeln!(script, "{}", index).unwrap();
            }
        }
