...
mod tests {
    ...
    fn test_parse_frozen_type() {
        ...
        let frozen_comparator = ComparatorType::Frozen(Box::new(
            ComparatorType::Set(Box::new(ComparatorType::Int)),
        ));
    ...
    fn test_incomplete_context_rejected() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Incomplete parsing context"));
    }
}