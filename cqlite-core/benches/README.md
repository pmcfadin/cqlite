...
`CQLITE_BENCH_ALLOW_LOAD=1 … --bench reconcile_overlap -- --test`. It carries the
shared `profiling::configure()` config, so `-- --profile-time <secs>` writes a pprof
flamegraph per arm (the decomposition of *where* k-cost goes). `-- --list` is free:
enumeration builds no fixture and runs no validity gate.

| Aspect | Contract |
|---|---|
| **Matrix** | `reconcile_overlap/<mix>/k<N>` for `mix` ∈ {`disjoint`, `lww_overwrite`, `tombstone`, `ttl_expiring`, `field_blend`} × `k` ∈ {1, 2, 5, 10, 20} — 25 arms — **plus the two producer-count control arms** `reconcile_overlap/producer_control/{p1,p2}` (27 total). p1 is ONE double-width disjoint generation, p2 is TWO standard-width ones: identical output rows, cells and collision count (`o = 1`), differing only in producer-stream count. They measure the mechanism behind the `k = 1` anchor deviation. |
...
| **Shape guard** | Before timing, every arm asserts a FULL census of the reconciled output — output rows/partitions, live cells, cell tombstones, row tombstones, coexisting row deletions (`MergeEntry::row_deletion`, issue #932) — against the census implied by its documented collision mix, and asserts that generation *g* contributes identically at every `k`. A silently-degenerate fixture therefore cannot emit numbers. This matters because the flush writer reconciles WITHIN a generation: a row tombstone stamped above its generation's live cells is collapsed to a cell-less row tombstone before the merge ever sees it, so the fixture stamps row tombstones BELOW the live cells (the issue-#932 coexistence shape). |
| **Load guard** | Two tiers, both ceilings **derived from the host's core count** and printed in the run header (`cores=… load_ceiling=… foreign_cpu_ceiling_cores=…`), so a small host is not handed 4× the relative slack a 16-core one gets. **(1) Run start:** the 1-minute load average is printed and the run **fails closed** above `0.125 × cores` (floor 0.5) — **2.00** on the 16-core reference box. **(2) Every arm:** the **foreign** (not-this-process) CPU consumed during that arm's TIMED region — `/proc/stat` `user…steal` busy (`guest`/`guest_nice` excluded: the kernel already counts them inside `user`/`nice`) minus this process's own `utime+stime` — must stay under `0.0625 × cores` cores (floor 0.25) — **1.00 core** on that box — else that arm is void and the run fails. Tier 2 is not a per-arm loadavg check on purpose: `KWayMerger` runs one producer thread per generation, so the run's OWN parallelism drives `loadavg` to ~5 by k = 20 — a per-arm loadavg gate would fail on the instrument itself. `loadavg` is still sampled per arm and reported (min/max/spread) as drift information. **Operator note:** because a run inflates its own 1-minute average, a back-to-back re-run must wait for it to decay below the printed `load_ceiling` (a few minutes) or tier 1 will — correctly — refuse. `CQLITE_BENCH_ALLOW_LOAD=1` opts out of both tiers visibly and marks the run as non-measurement. |
| **Platform** | Both probes read `/proc`, so the validity guard is **Linux-only**; an unreadable probe is treated as unknown and fails closed. On a non-Linux host this instrument can only produce a self-labelled `CQLITE_BENCH_ALLOW_LOAD=1` NON-measurement run. The record's numbers are Linux measurements by construction. |
| **Gate policy** | **ADVISORY ONLY.** All 27 ids appear in `perf-gate.json` under `advisory_benches` with **no** `benches` entry and no `threshold_pct`, and `perf-regression.yml` does not run this target — an instrument whose numbers move with fixture tuning must never block a merge. The ids are a **declarative classification**, not live config: `check_perf_regression.py` iterates `benches` only, so with no `benches` entry these ids are inert today; they are listed so that if this target is ever added to `perf-regression.yml` it starts out advisory rather than strict. |
| **Anchor** | The **saturated `disjoint` control** — the mean per-row cost over k ≥ 5 — is validated against the published ~2.0 µs/row narrow-disjoint-singleton figure (`docs/research/phase2-verify-stage2.md:226-232`) before any multiplier is derived; **an out-of-band SATURATED anchor voids the run.** (Amended 2026-07-26, owner decision, issue #2043: the anchor was originally `disjoint/k1`, which is a whole-drain wall time from a SINGLE producer stream — not the quantity the published figure reports. The k = 1 arm is still reported, and its deviation from the anchor is carried as an EXPLAINED deviation whose mechanism the `producer_control` arms measure.) |
