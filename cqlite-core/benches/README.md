...
| **Load guard** | Two tiers. **(1) Run start:** the 1-minute load average is printed and the run **fails closed** above **2.00**. **(2) Every arm:** the **foreign** (not-this-process) CPU consumed during that arm's TIMED region — `/proc/stat` busy minus this process's own `utime+stime` — must stay under **1.00 core**, else that arm is void and the run fails. Tier 2 is not a per-arm loadavg check on purpose: `KWayMerger` runs one producer thread per generation, so the run's OWN parallelism drives `loadavg` to ~5 by k = 20 — a per-arm loadavg gate would fail on the instrument itself. `loadavg` is still sampled per arm and reported (min/max/spread) as drift information. **Operator note:** because a run inflates its own 1-minute average, a back-to-back re-run must wait for it to decay below 2.00 (a few minutes) or tier 1 will — correctly — refuse. `CQLITE_BENCH_ALLOW_LOAD=1` opts out of both tiers visibly and marks the run as non-measurement. |
