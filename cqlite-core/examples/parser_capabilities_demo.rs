...
fn test_ast_functionality() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match statement {
        CqlStatement::CreateTable(ref ct) => {
            ...
            println!(
                "  Primary key: {:?}",
                ct.primary_key.partition_key[0].name()
            );
        }
        _ => unreachable!(),
    }
...
fn test_visitor_pattern() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✓ IdentifierCollector found {} identifiers:",
        identifiers.len()
    );
...
fn test_backend_info() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "    - Statements/sec: {}",
        nom_info.performance.statements_per_second
    );
    println!(
        "    - Memory/statement: {} bytes",
        nom_info.performance.memory_per_statement
    );
    println!(
        "    - Startup time: {}ms",
        nom_info.performance.startup_time_ms
    );
    println!(
        "    - Async support: {}",
        nom_info.performance.async_support
    );
