...
fn proof_ast_creation() {
    ...
    println!(
        "Created quoted identifier: \"{}\" (quoted: {})",
        quoted_id.name(),
        quoted_id.is_quoted()
    );
...
fn proof_parser_factory() {
    ...
    println!(
        "  Production → {:?}",
        recommend_backend(UseCase::Production)
    );
...
fn proof_configuration() {
    ...
    // Builder pattern
    let custom_config = ParserConfig::default().with_backend(ParserBackend::Nom);
    ...
    println!(
        ...
        strict_config.strict_validation
    );
...
fn proof_error_handling() {
    ...
    let syntax_error = ParserError::syntax("Expected ';'", SourcePosition::new(1, 5, 5, 0));
    ...
    println!(
        "  Syntax error - category: {:?}, severity: {:?}",
        syntax_error.category(),
        syntax_error.severity()
    );
    println!(
        "  Semantic error - category: {:?}, severity: {:?}",
        semantic_error.category(),
        semantic_error.severity()
    );
    println!(
        "  Backend error - recoverable: {}",
        backend_error.is_recoverable()
    );
