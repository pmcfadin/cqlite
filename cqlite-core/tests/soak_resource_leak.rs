...
fn analyze_detects_rss_growth() {
    ...
    let thr = LeakThresholds {
        fd_consecutive_increase: 100, // disable the fd rule for this test
        rss_growth_ceiling_bytes: 64 * 1024 * 1024,
    };
    assert!(
...
mod linux_soak {
    ...
    async fn run_healthy(test: &str, config: &cqlite_core::Config, thr: &LeakThresholds) {
        ...
        let (fd, rss, rows) = run_soak(&data_db, config, false).await;
    ...
    async fn sabotage_fd_leak_is_detected() {
        ...
        std::env::set_var("CQLITE_SOAK_ITERATIONS", "40");
        std::env::set_var("CQLITE_SOAK_SAMPLE_EVERY", "4");
        ...
        let (fd, rss, rows) = run_soak(&data_db, &config, true).await;
