...
use cqlite_core::testing::{resolve_table_to_sstable_path, load_metadata, list_tables};
use std::env;
...
"#;
    ...
    unsafe { env::set_var("CQLITE_DATASETS_ROOT", &datasets_root); }
    ...
    let users_path = resolve_table_to_sstable_path("test_basic", "users")
    ...
    let system_tables = list_tables(Some("system_test")).expect("Failed to list system_test tables");
    ...
    unsafe { env::remove_var("CQLITE_DATASETS_ROOT"); }
}
