...
async fn test_basic_cassandra_compatibility() -> Result<()> {
    ...
    let test_entries = vec![
        ...
        (
            TableId::new("orders"),
            RowKey::from("order:100"),
            Value::Integer(1500),
        ),
        (
            TableId::new("orders"),
            RowKey::from("order:101"),
            Value::Float(29.99),
        ),
    ];
...
async fn test_cassandra_compression_compatibility() -> Result<()> {
    ...
    let mut compressed_writer =
        SSTableWriter::create(&compressed_path, &compressed_config, platform.clone()).await?;
    ...
    for i in 0..100 {
        ...
        let value = Value::Text(format!(
            "This is test data entry number {} with some repetitive content to ensure compression works effectively",
            i
        ));
    ...
    assert!(
        ...
        "Compressed SSTable should be created"
    );
    ...
    assert!(
        metadata.len() < 8000,
        "Compressed file should be smaller than expected uncompressed size"
    );
...
fn test_read_real_cassandra_sstables() {
    ...
    if !Path::new(test_env_path).exists() {
        println!(
            "Skipping real SSTable test - test environment not found at {}",
            test_env_path
        );
        return;
    }
    ...
    let entries =
        std::fs::read_dir(test_env_path).expect("Should be able to read test-env directory");
    ...
    assert!(
        sstable_dirs > 0,
        "Should have at least one SSTable directory in test environment"
    );
}
