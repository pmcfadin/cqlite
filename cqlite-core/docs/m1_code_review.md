...
- `src/storage/sstable/reader.rs:826`: Guesses Cassandra 5 header size from string lengths; the data seek lands inside the header on real SSTables, corrupting every block read.

- `src/storage/sstable/summary_reader.rs:254`: Assumes a fictitious header/entry layout for Summary.db; token-range logic always consumes junk.

## High
- `src/storage/sstable/reader.rs:975`: Only searches for `nb-1-big-CompressionInfo.db`, `CompressionInfo.db`, or `Data-CompressionInfo.db`; misses real component filenames and silently skips compression metadata.
