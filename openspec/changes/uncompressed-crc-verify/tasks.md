...
- [x] Add a `CRC.db` reader parsing the `ChecksumWriter` layout (4-byte big-endian `i32`
...
- [x] Expose `crc_for_offset(offset)` using `chunk_index = offset / chunk_size`,
      `crc_file_pos = chunk_index * 4 + 4`; stream — read only header + the one CRC entry.
- [x] Unit test: parse the committed Cassandra-written `CRC.db`
...
- [x] Unit test: round-trip the #1197 writer output through the reader (multi-chunk).
- [x] Unit test: truncated/short `CRC.db` → typed error, no panic.

## 2. Wire verification into the uncompressed read path (surface: `Database.execute` / `SSTableReader.scan`)
- [x] In `read_uncompressed_data_block` (`reader/block_io.rs`), verify each returned chunk
      against `CRC.db` on `chunk_size`-aligned boundaries, default-on, unconditional.
- [x] On mismatch return typed `Error::Corruption` naming the failing chunk index + Data.db
      byte offset; never return corrupt bytes / wrong values / silent 0 rows.
- [x] Handle absent `CRC.db` per the pinned decision (design D4); leave the compressed
      path (`block_io.rs` ~412-440) untouched.
- [x] Wiring-evidence e2e test: plain `Database.execute` over the bit-flipped uncompressed
      fixture returns `Error::Corruption` naming chunk/offset; clean fixture returns
      correct rows.
- [x] e2e test: single flip in a non-first chunk is attributed to chunk *k* (boundary).

## 3. verify --mode full integration (surface: `cqlite verify --mode full`)
- [x] Add a stable, distinct `VerifyErrorClass` checksum-mismatch variant (uncompressed
...
- [x] Test: `verify --mode full` reports the checksum-mismatch finding naming the chunk on
      the corrupt fixture; reports none on the clean source.

## 4. Corruption corpus + oracle (surface: corpus generator + parity manifest)
- [x] Add `uncompressed_data_bit_flip` to `generate-corruption-corpus.sh` (clean source:
...
- [x] Run the full uncompressed parity suite (sstabledump JSONL goldens) with verification
      on; assert byte-identical results vs baseline.
- [x] Confirm no memory/perf-budget regression (O(chunk_size) working set; one CRC32 per
