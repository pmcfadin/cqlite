...
- [ ] 1.1 Add `.claude/agents/flow-lead.md` — orchestrator persona: orient from
...
- [ ] 1.2 Set `"agent": "flow-lead"` in `.claude/settings.json`.

## 2. Pipeline skills (thin wrappers)

- [ ] 2.1 `flow-groom` — idea → one scoped issue (one P-label, `status:ready`,
      testable criteria). Route oracle-driven bugs to issue+pinned-test (skip
      OpenSpec).
- [ ] 2.2 `flow-activate` — worktree+branch (1:1:1:1) + `opsx:propose` + design;
      render inline; STOP at Seam 1.
- [ ] 2.3 `flow-implement` — spawn the specialist team in the worktree, run
      `agent-gate.sh` → C (spec-auditor) → roborev, open the PR; do not merge by
      default.
- [ ] 2.4 `flow-address` — resolve PR review comments in the worktree, push,
      reply per thread.
- [ ] 2.5 `flow-finalize` — `opsx:archive` + remove worktree/branch + close
      issue; only after merge.
- [ ] 2.6 `flow-board` — status across in-flight issues (labels + PR/CI) and
      surface the single next item for the owner.

## 3. Roster + autonomy wiring

- [ ] 3.1 Map the pipeline's specialist roles to CQLite agents (sstable-developer,
...
- [ ] 3.2 Encode the pre-authorized merge-on-green autonomy model (D4) and
      reconcile CLAUDE.md "Product-manager behavior (lead)" with it (D6).

## 4. Doctrine & docs

- [ ] 4.1 Document the delivery pipeline + the two seams + autonomy model in
...
- [ ] 5.1 Confirm no code lanes regress (docs/agent-only: zero `*.rs`/`Cargo.*`/
