...
- [ ] 1.1 Define the `parity-failures.json` schema (`[{scenario_id, workflow, test_target, component_path,
      failure_class}]`); normalize `failure_class` to `VerifyErrorClass.code()` where applicable.
- [ ] 1.2 Emit it from the parity lanes on failure (start with `compression-corruption-parity.yml`,
      `cql-type-parity.yml`, `tombstone-ttl-parity.yml`, `exhaustive-regeneration.yml`); upload as an artifact
      `if: always()`/`failure()`.

## 2. The automation workflow

- [ ] 2.1 Add `.github/workflows/parity-failure-issue.yml` — `workflow_run` (completed) over the parity lanes
      + `workflow_dispatch`; `permissions: {issues: write, contents: read}`; board-sync token-guard idiom.
- [ ] 2.2 Act only when `conclusion == failure` AND origin event ∈ {schedule, push(main), workflow_dispatch};
      skip pull_request.
- [ ] 2.3 Download the failed run's `parity-failures.json` (degraded summary/log parse + surfaced notice if
      absent); compute the `v1|sha256(...)` fingerprint per failing scenario.
- [ ] 2.4 Dedup: `gh issue list --label parity-failure --state open` → substring-match `<!-- PARITY-FAIL:<fp> -->`;
...
- [ ] 2.5 Green-run path: post a resolution comment on a tracked fingerprint's open issue; do NOT auto-close.
- [ ] 2.6 Non-gating + fail-open: never change the parity result; token absent → notice + exit 0.

## 3. Fingerprint/dedup unit tests (smoke, wiring-evidence)

- [ ] 3.1 Factor the fingerprint + dedup logic into a testable unit (script/action) accepting injected inputs
...
- [ ] 4.1 Add the 5 `cass.cli_reporting.failure_issue_*` scenarios (mirrored/tooling_only/smoke) modeled on
...
- [ ] 4.2 Add the implementation reference to `docs/development/parity-ci-tiers.md` (the nightly_docker
...
- [ ] 5.4 actionlint clean on the new workflow + any edited lanes.
