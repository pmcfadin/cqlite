...
> **Seam-1 owner decisions still OPEN** (see `design.md` §"Owner decisions needed at Seam 1"):
...
- [ ] Add the conjunctive, fail-closed bypass predicate at the full-scan decision point
      ...
      inference (#28).
- [ ] Add the forced-path override (`CQLITE_FLIGHT_MERGE_PATH=bypass|merge`, unset = automatic),
      mirroring `CQLITE_READ_PATH`; read it once per request, not per row.
- [ ] Add the path-taken observation seam (merger-construction + reconcile-entry counters/probe) the
      AC #1 test asserts on — an explicit marker, never a timing inference.
- [ ] Unit tests: predicate is 1-source-only; 2 sources → merge arm; post-prune (not pre-prune) count
...
- [ ] Factor the drive loop's ROW SOURCE behind a small internal trait so `drive_merge_over`
      ...
      duplicate the loop.
- [ ] Implement the fast source over `SSTableReader::scan_stream_batched_admitted`
...
- [ ] Implement the `(RowKey, ScanRow) → QueryRow` adapter by REUSING
...
- [ ] Thread the request's `now_secs` into the fast path's shadow/TTL clock explicitly (the merge arm
...
- [ ] Assert the fast path's parser is built with `read_shadowing = true`
      (`scan_stream_windowed.rs:748-751`) rather than assuming it.
- [ ] Verify the fast path decodes with `want_cell_metadata == false` and constructs zero
      `CellWriteMetadata` maps (AC #2) — by not entering `CompactionPolicy::on_data_row`, not by
      tuning it.
- [ ] Confirm predicate pushdown, projection, and the UDT registry still apply on the fast arm
      (`filter.rs` application point unchanged).

## 3. Path-taken pins (surface: `cqlite-flight/tests/`)
- [ ] AC #1: an e2e `do_get` over a real single-SSTable fixture asserts merger-construction count == 0
      AND reconcile-entry count == 0 — the test FAILS if the merge path is taken. Not a throughput
      assertion.
- [ ] AC #4 (inverse): an e2e `do_get` over a ≥2-overlapping-SSTable fixture asserts the merger IS
      constructed and reconcile entries are non-zero.
- [ ] Assert emitted `QueryRow.cell_metadata` is still `None` on both arms (no consumer-visible change).

## 4. Correctness pins (surfaces: `cqlite-flight/tests/`, `cqlite-core/tests/`, `test-data/`)
- [ ] **Forced-path differential** (the primary proof for gap (b)): run the SAME single-SSTable
...
- [ ] **New ≥2-overlapping-SSTable Flight fixture + oracle case.** Every committed
...
- [ ] Run both semantic oracles green: gate components `query-semantics-oracle`
...
- [ ] Run `cqlite-core/tests/point_vs_full_differential.rs` (#1918) green.
- [ ] Run the #2988 multi-generation pins green and unchanged:
...
- [ ] No allocator work (#3028) and no `RowColumnResolution` hoist (#3047) in this diff — they are
      held for re-pricing against the post-change profile.
- [ ] No #3060 (mid-stream shutdown spin) and no #3061 (double mmap / RSS) in this diff.
- [ ] No Arrow-encode changes and no mpsc-handoff changes in this diff.
- [ ] Pre-roborev self-check: no-heuristics (the predicate is authoritative), no
...
## 7. Docs / doctrine
- [ ] Document the forced-path override alongside `CQLITE_READ_PATH` in
...
- [ ] `scripts/agent-gate.sh --lite` green each fix round (summary-file redirect; never raw stdout).
