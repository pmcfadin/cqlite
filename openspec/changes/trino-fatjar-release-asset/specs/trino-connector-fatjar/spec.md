# trino-connector-fatjar — delta for trino-fatjar-release-asset (issue #2869)

**Read this first.** Trino loads a plugin from a **directory**
(`io.trino.server.ServerPluginsProvider.loadPlugins` filters the plugin path with
`Files::isDirectory`), so "one jar" never means "no directory" — it means one jar *inside* the
plugin directory. Every requirement below is written against that fact. The artifact this capability
covers is published **only** as a GitHub Release asset; `in.mcfad:cqlite-trino` on Maven Central
remains the thin jar, and the last requirement exists to keep it that way.

**Acceptance-criterion → requirement map** (issue #2869):

| AC | Requirement |
|---|---|
| A shaded, self-contained connector jar is produced | ADDED *The build produces a self-contained shaded connector jar* |
| Its service files are merged, not overwritten | ADDED *The build produces a self-contained shaded connector jar* |
| It is fetchable by version from a stable URL, with an integrity sidecar | ADDED *The shaded jar is published as a GitHub Release asset at a stable, version-addressed URL* |
| A pre-release build is fetchable without minting a `v*` tag | ADDED *The shaded jar is published as a GitHub Release asset at a stable, version-addressed URL* |
| The Maven Central publication gains no shaded variant | ADDED *Publishing the shaded jar does not alter the Maven Central publication* |
| A consumer can install the jar and query through it | ADDED *The shaded jar installs as a Trino plugin from inside the plugin directory* |
| Documentation states the directory rule and the residuals | ADDED *Consumer documentation states the directory rule, the channels, and the declared residuals* |
