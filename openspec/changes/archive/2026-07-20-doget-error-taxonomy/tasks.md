...
## Implementation
- [x] Add `AbortReason` enum (closed set: `superseded_split`, `client_cancel`, `admission_shed`,
  `snapshot_retired`, `internal`, `ticket_invalid`) with `label() -> &'static str` in
  `cqlite-flight/src/obs.rs` (surface: `AbortReason`).
- [x] Add `AbortContext { ticket_id, snapshot_generation }` and a reason-carrying hook
...
- [x] Stamp the authoritative reason at each abort construction site (do NOT infer from code/message):
...
- [x] Extend the `flight.rpc` span (abort path only) with ticket/split identity + snapshot generation;
  keep the happy path method-name-only (span cardinality cap).
- [x] Route the abort log level off the reason (benign→debug, internal→error, ticket→warn),
  replacing the code-driven level for the do_get abort path.

## Wiring evidence (public Flight surface)
- [x] Extend `tests/do_get_transport_test.rs` (real tonic server) under `observability-testing`:
  client-disconnect asserts `cqlite.errors.total{abort_reason="client_cancel"}` += 1.
- [x] Add a superseded/retired-snapshot transport case asserting
  `abort_reason ∈ {superseded_split, snapshot_retired}` += 1.
- [x] Add an admission-shed transport case asserting `abort_reason="admission_shed"` += 1.
- [x] Add `cqlite.flight.abort_reason` to `BOUNDED_KEYS` in `tests/metrics_capture_test.rs`.

## Adjudication + docs
- [x] Record benign (excluded-from-SLI) vs genuine reasons in the change/proposal; note the
...
- [x] Update any Flight observability doc listing metric attributes to include
