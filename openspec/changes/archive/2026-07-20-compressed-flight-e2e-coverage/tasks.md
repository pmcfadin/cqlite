...
- [x] 1.1 `git add -f` the components the tests read for `test_comp`: the four codec tables
...
- [x] 1.2 Confirm nothing else in the corpus was swept in and the pin is untouched
      (`dataset-pin.env` unchanged; no regeneration — #2222/#1935 stand).

## 2. Fixture helper

- [x] 2.1 Add `table_dir_by_prefix(keyspace, table_prefix, component_prefix) -> Option<PathBuf>` to
...
- [x] 2.2 SKIP-aware gating with `CQLITE_REQUIRE_FIXTURES=1` → hard failure
      (pattern `chunk_cache_wiring_tests.rs:26,214`).

## 3. Transport-level per-codec coverage

- [x] 3.1 New `cqlite-flight/tests/compressed_do_get_transport_test.rs`, copying the transport harness
      from `bti_do_get_transport_test.rs:68-125` (`do_get_batches_over_transport` / `run_do_get`).
- [x] 3.2 Full-scan-vs-golden case per codec (LZ4, Snappy, Deflate, Zstd): row count AND
      `(pk, ck, body)` equality against `nb-1-big-Data.db.jsonl`; present-but-empty = FAILURE.
- [x] 3.3 Edge-table full scans: `short_final_chunk`, `incompressible_uncompressed_chunk`.
- [x] 3.4 LIMIT-k case bounding the result and matching a golden prefix.
- [x] 3.5 Midstream-drop case on LZ4 reusing `do_get_drop_after` + `await_in_flight_settled`
      (`do_get_transport_test.rs:595-670`, batch_size=1): in-flight level settles to baseline.

## 4. Routing evidence

- [x] 4.1 Bracket each compressed scan with `reset_decompress_calls()` /
      `decompress_call_count() >= 1` (`data_access/mod.rs:146,151`).
- [x] 4.2 Uncompressed control (`test_comp.uncompressed_table`): same scan shape, counter **== 0**.
- [x] 4.3 Serialize counter-reading tests against each other (`#[serial]` or single-test binary);
      header comment explaining the counter is process-global and why the control makes it evidence.

## 5. Docs

- [x] 5.1 Mark FMT-1 closed for the in-repo Flight axis in
      `docs/architecture/issue-2363-coverage-matrix-audit.md`; note the live-testbed half remains open
      as the separate follow-up.
- [x] 5.2 Test header notes the goldens/bytes are pinned to datasets-v3 v3.5 and must be refreshed
      together on any future regen (#2222).

## 6. Certification

- [x] 6.1 `scripts/agent-gate.sh --lite` each fix round (summary-file redirect, unique path);
      plus the new test target explicitly.
- [x] 6.2 **Verify against the COMMITTED tree**, not the dirty worktree — `git worktree add --detach HEAD`
