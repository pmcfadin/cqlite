# flight-concurrency-sizing-guidance — delta for concurrency-admission-defaults (issue #3225)

**Read this first.** This capability covers the **measurement and the published guidance** half of
issue #3225. It creates no runtime surface: everything here is a harness run, a committed report and
operator documentation. The runtime half (the derived default, precedence, startup logging) is in
`specs/flight-admission-control/spec.md`.

**Two honesty constraints govern every requirement below.**

1. **#3217's data cannot satisfy AC1 or AC5 by itself.** Its N ramp was `1,2,4,8,16`, so it holds no
   point at the shipped default of **64**, and its peaks at 4 and 6 physical cores sit at the top of
   the ramp — censored observations meaning "≥16, unknown", with server utilisation at S=6/N=16 only
   0.967 (not saturated). A new sweep is required; re-reading the old JSON is not a reproduction.
2. **No throughput number is asserted by a test.** CLAUDE.md bans wall-clock threshold asserts in the
   correctness test path; a deliberate perf assert must be `#[ignore]`d and marked
   `perf-gate-allow`. The measurement's output is a committed report plus artifacts, and the
   artifacts under `docs/reports/*-artifacts/` are **reviewed code**, not docs — a PR carrying them is
   not a docs-only change and must be roborev-certified.

**Requirement → issue-AC map** (issue #3225's six ACs; ACs 2 and 4 are carried by the
`flight-admission-control` delta):

| AC | Verdict | Requirement(s) |
|----|---------|----------------|
| 1 — the peak-N-by-core-count table is **reproduced** and extended to the widest configuration in scope, medians of ≥3 with dispersion, throughput **and** per-scan-latency cost of over-admission stated at each width | **SATISFIED, with the scope of "widest" declared.** Widest = **6 physical cores / 12 hardware threads**; 8 cores is out of reach on a single box because the client needs 2 exclusive physical cores and `sweep.sh` refuses an overlapping set. Declared, not silently narrowed. | ADDED *The peak-concurrency-by-width curve is re-measured with the shipped default inside the ramp*; ADDED *Over-admission cost is reported in throughput and in per-scan latency at every width* |
| 3 — operator documentation states the measured relationship, including the 16.4%-at-1-core cost and the 31 s → 302 s latency curve | **SATISFIED** (and required regardless of the AC2 decision) | ADDED *Operator documentation states the measured relationship and how to override it* |
| 5 — **no regression at the widest configuration**, measured, not assumed | **SATISFIED, and load-bearing.** Because the real default is 64 (not 16), "the default is already optimal at 4 and 6 cores" is unproven; the ramp must therefore include 24, 32 and 64. | ADDED *No throughput regression at the widest configuration in scope* |
| 6 — every throughput figure names its byte basis; fixture geometry recorded (rows, B/row, sha256) | **SATISFIED.** #3217's corpus binaries are gone from the measurement box and are gitignored, so the corpus is re-staged and its **new** sha and geometry are recorded — not copied from the old report. | ADDED *Every reported throughput figure names its byte basis and its fixture geometry* |
| — | Scope discipline | ADDED *The change records what it does not change* |
