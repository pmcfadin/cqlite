...
- [ ] 1.1 Export `AGENT_GATE_PARENT_RUN_ID="$RUN_ID"` alongside the existing
      `AGENT_GATE_SUMMARY_FILE` de-export, before any component runs.
- [ ] 1.2 In summary-path resolution: if `AGENT_GATE_PARENT_RUN_ID` is set and no explicit
...
- [ ] 1.3 Regression self-test (new, wired into `tooling-tests`): parent gate mid-run + nested
...
- [ ] 2.1 Component-boundary guard: `grep -q "run-id: $RUN_ID" "$SUMMARY_FILE"` (when the file
      exists); on mismatch write `summary-integrity: FAIL (foreign run-id detected mid-run;
      expected $RUN_ID)` + `RESULT: FAIL`, exit non-zero.
- [ ] 2.2 Self-test: overwrite the summary with a foreign-run-id block mid-run (fast selftest
      mode) → named FAIL line present, exit non-zero, never bare INCOMPLETE.

## 3. Hermetic self-test fixtures (surface: `scripts/tests/*.sh`)
- [ ] 3.1 `test_agent_gate_parity_report.sh`: per-run `mktemp` mutated-manifest fixture under
      `test-data/` (terminal `XXXXXX`); trap removes only this run's file. Verify no write to
      shared repo files (reads/diffs OK).
- [ ] 3.2 Sweep all gate self-tests for other fixed shared paths → per-run mktemp. (Slots dir
      `/tmp/cqlite-gate-slots` is the semaphore by design — out of scope.)
- [ ] 3.3 Structural self-check: FAIL `tooling-tests` on any fixed `.tmp-*` fixture name or
      non-terminal-`XXXXXX` mktemp template in `scripts/tests/*.sh`.

## 4. Concurrency self-test (surface: `tooling-tests` component)
- [ ] 4.1 Bounded test: two concurrent instances of the fast parity-report/summary self-tests in
      one checkout, both pass.
- [ ] 4.2 Record `tooling-tests` wall-clock before/after on the PR (±10% budget).

## 5. Docs (same change)
- [ ] 5.1 `docs/development/gate-ops.md`: replace box-exclusive/serialize-everything guidance with
