...
- [ ] 1.1 Author `docs/reports/delivery-telemetry.schema.json` (JSON Schema, draft 2020-12):
...
- [ ] 1.2 Create the live ledger `docs/reports/delivery-telemetry.jsonl` (empty or backfilled
...
- [ ] 2.1 `record` subcommand: pull GitHub timestamps live via `gh` (or inject via
...
- [ ] 2.2 `lint` / `validate` subcommand: schema-validate every ledger line; non-zero +
      offending line number on any malformed record; zero on a clean ledger. Surface = `lint`.
- [ ] 2.3 `retro` subcommand: rank failure categories by `Σ(count × documented fixed weight)`
...
- [ ] 3.1 `scripts/tests/test_delivery_telemetry.py` (stdlib `unittest`):
...
- [ ] 3.2 Add fixtures under `scripts/tests/fixtures/` (a small ledger + an open-issues JSON).

## 4. Wire into finalize + manager + gate

- [ ] 4.1 `flow-finalize` skill: add the ledger-stamp step (call `record` with the run's
      counters) after merge/cleanup. Document where the counters come from.
- [ ] 4.2 `manager` skill + `docs/development/pm-operating-loop.md`: add the recurring
      `retro` step (cadence + when to file).
- [ ] 4.3 `scripts/agent-gate.sh`: add a SKIP-aware `delivery-telemetry` component that runs
      `scripts/tests/test_delivery_telemetry.py`; add it to `COMPONENTS`. Verify
      `scripts/agent-gate.sh --only delivery-telemetry`.

## 5. Docs

- [ ] 5.1 CLAUDE.md: document the ledger path/schema + retro in the delivery-pipeline section.
- [ ] 5.2 Website `agents-developing/delivery-pipeline` page: add a telemetry/retro subsection
      (mirror CLAUDE.md).
