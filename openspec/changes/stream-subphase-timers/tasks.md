...
- [ ] Add `StreamSubPhaseTimings` (five `AtomicU64` nanos counters behind an `Arc`) + a RAII
...
- [ ] Add the thread-local `Option<Arc<StreamSubPhaseSink>>` seam in `cqlite-core::observability`
...
- [ ] Propagate the request's `Arc` sink to the three scan-thread spawn sites: capture `current()`
...
- [ ] Wrap the `stream_merge` scope around the reconcile/materialize `step_row`, `stream_encode` around
...
- [ ] Create the request's `Arc<StreamSubPhaseTimings>` in the `spawn_blocking` merge closure, install
...
- [ ] Add the un-entered-sub-phase assertion: an uncompressed fixture records no `stream_decompress`
