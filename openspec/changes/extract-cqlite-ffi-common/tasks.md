...
- [ ] Create `cqlite-ffi-common/` with `Cargo.toml`: workspace-inherited `version`/`edition`/`authors`/
      `license`/`repository`/`rust-version`, `publish = false`, `[lints] workspace = true`.
- [ ] Dependencies: `cqlite-core = { path = "../cqlite-core" }` and `num-bigint = "0.4"` (declared
      locally to match `cqlite-core/Cargo.toml:80`; **do not** create a `[workspace.dependencies]`
      entry — non-goal).
- [ ] Add `"cqlite-ffi-common",` to the root `Cargo.toml` `members` list. The two binding crates are
      **already** members (`Cargo.toml:2-13`) — no membership surgery there.
- [ ] `src/lib.rs`: crate docs stating the dependency direction, the no-FFI-framework rule and its
...
- [ ] Implement the `cargo metadata` closure measurement of `design.md` D6: resolve the graph rooted at
...
- [ ] Fail-closed on: `CARGO` unset, non-zero exit, unparseable JSON, missing own node. Each FAILs with
      a message naming what could not be measured. **No env opt-out.**
- [ ] **RED-verify the assert before believing it**: in a scratch copy of the crate, add a dependency
...
- [ ] **3a. Apply the OQ3 ruling.** Implement the single policy the owner chose, as named `pub const`
...
- [ ] Move Node's `decimal_to_string` body (`bindings/node/src/value.rs:364-430`) into the crate,
...
- [ ] Unit-test every case the spec enumerates: empty; `scale == 0`; positive `scale` shorter/equal/
...
- [ ] **3c. Record the behaviour delta.** Write down, with concrete inputs, every case whose observable
...
- [ ] `pub fn varint_to_bigint(&[u8]) -> num_bigint::BigInt` — empty ⇒ 0, big-endian two's complement,
      sign-extending at any length. This is the single semantic implementation.
- [ ] `pub fn varint_to_sign_and_le_words(&[u8]) -> (bool, Vec<u64>)` — **derived** from the `BigInt`
      via `to_u64_digits()`, never computed independently. This is napi's
      `create_bigint_from_words(sign_bit, words)` contract.
- [ ] Unit-test: empty; 1 positive byte; 1 negative byte; exactly 8 bytes; 9 and 17 bytes both signs;
...
- [ ] `git mv cqlite-core/src/ffi_error_contract.rs cqlite-ffi-common/src/error_contract.rs` and
...
- [ ] Delete `pub mod ffi_error_contract;` from `cqlite-core/src/lib.rs:17`. **Add no re-export**
      (design D5): one item, one path.
- [ ] **5b. OQ1 — only if the owner rules "rename in scope":** rename
...
- [ ] Re-point consumers: `bindings/python/src/error.rs:25`, `bindings/python/src/lib.rs:108`,
...
- [ ] `cargo test -p cqlite-ffi-common` — all 12 moved tests pass unmodified.
- [ ] Commit.

## 6. INET (surface: `cqlite-ffi-common/src/inet.rs`)
- [ ] `InetKind { V4, V6 }`, `InetError { len }` with the ONE `Display` spelling of
      `"Invalid inet address length: {n} (expected 4 or 16)"`, `inet_kind`, `inet_bytes_to_string`.
- [ ] Move the two inet unit tests from `bindings/python/src/value.rs:643-657` here and **delete** the
      `#[cfg(test)] fn inet_to_string` at `value.rs:599` — a third formatter whose hex fallback
      contradicts production behaviour.
- [ ] Unit-test 4-byte, 16-byte (including a compressible IPv6), 0-byte, 5-byte, 15-byte, 17-byte.
- [ ] Commit.

## 7. OTel keys (surface: `cqlite-ffi-common/src/otel_keys.rs`)
- [ ] `pub const KNOWN_OTEL_KEYS: &[&str]` with the 7 snake_case names currently at
...
- [ ] Export `DECIMAL_VECTORS`, `VARINT_VECTORS`, `INET_VECTORS` as plain `pub const` data (no feature
...
- [ ] Cover every edge case enumerated in tasks 3, 4 and 6, including at least one expected-error entry
      per table.
- [ ] Assert inside the shared crate that every vector's expectation matches the shared implementation,
...
- [ ] Add `cqlite-ffi-common = { path = "../../cqlite-ffi-common" }`.
- [ ] `decimal_to_pydecimal` becomes: call `decimal_to_string`, map `DecimalError` →
...
- [ ] `varint_to_pyint` routes through `varint_to_bigint` per design D2 (prefer enabling pyo3's
      ...
      from the shared fn.
- [ ] `inet_to_py` calls `inet_kind` for the dispatch and renders `InetError` via `Display` for the
      error message. No literal copy of the message text remains.
- [ ] `observability.rs` consumes `KNOWN_OTEL_KEYS`; behaviour (both `ValueError` paths and the message
      naming the recognised keys) unchanged.
- [ ] Add the internal `_ffi_common_vectors_*` test-support functions that render every vector through
      the production path, registered in `_cqlite` alongside the existing `_decimal_from_parts` /
      `_inet_from_bytes` (`lib.rs:131`).
- [ ] Add `bindings/python/tests/test_shared_vectors.py` asserting the full tables.
- [ ] Commit.

## 10. Wire the Node binding (surfaces: `bindings/node/src/value.rs`, `observability.rs`, `lib.rs`, `Cargo.toml`, `__test__/`)
- [ ] Add `cqlite-ffi-common = { path = "../../cqlite-ffi-common" }`.
- [ ] Delete local `decimal_to_string` + its three policy consts, `varint_to_bigint`'s body (keep only
...
- [ ] Add the `#[cfg(test)]` set-equality assertion between `OtelOptions`'s snake_case field names and
      `KNOWN_OTEL_KEYS`, failing in both directions.
- [ ] Add the `_ffiCommonVectors*` test-support surface mirroring Python's, re-exported from
      `lib/index.js` in the established `_errorContract*` style, and
      `__test__/shared-vectors.test.js` asserting the full tables.
- [ ] Keep `num-bigint` in the Node manifest only if still used directly; remove it if the shared crate
      is now the only consumer.
- [ ] Commit.

## 11. Cross-binding + regression verification
- [ ] `cargo test -p cqlite-ffi-common`.
- [ ] `cd bindings/node && npm run build && npm test` — all pre-existing suites green, plus the new
      vector suite.
- [ ] `cd bindings/python && maturin develop` then
...
- [ ] **Mutation-verify the wiring claim** (spec: "a binding's rendered output tracks a change made only
...
- [ ] Re-check `bindings/python/tests/test_decimal_guard.py` and `__test__/abort-safety.test.js` against
...
- [ ] `CHANGELOG.md`: the DECIMAL policy convergence (the only user-visible change) and the
      `cqlite-core::ffi_error_contract` removal.
- [ ] Binding READMEs: the DECIMAL rendering policy, stated once and identically in both.
- [ ] `bindings/node/README.md:284` re-pointed to the new path.
- [ ] Note in the PR body that the `pub-surface` gate component does **not** detect public-API removal
      (#1712/#3366), so the `cqlite-core` module removal is being surfaced in prose deliberately.

## 13. Review, gate, close
- [ ] `scripts/agent-gate.sh --lite` green on every fix round (summary-file redirect).
- [ ] `rust-reviewer` + roborev on the lite-green diff, BEFORE the full gate
...
- [ ] Open the PR; hand the endgame to `flow-closer`: ONE full `scripts/agent-gate.sh` (paste the
