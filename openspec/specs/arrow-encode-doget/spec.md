# arrow-encode-doget Specification

## Purpose
Pricing and acceptance rules for Arrow-encode work on the Flight `do_get` read path:
how a throughput claim SHALL be measured (same-session ratio to the bare scan, rows/s
AND cycles/row), how output invariance SHALL be proven (in-repo Arrow-buffer digest
oracle), and the binding kill criterion under which a correctly-reported negative
result is a satisfying outcome.

**Delivery status (#3096, PR #3254, squash `eb3dde0`) — TWO REQUIREMENTS BELOW ARE UNMET.**
This spec states the rules, NOT achieved capability. Do not read it as evidence that the
1.3x ratio or a clean-checkout-runnable rig exists:

| Requirement | Status | Re-anchored to |
|---|---|---|
| "Acceptance is a same-session RATIO to the bare scan" (AC1/R1) | **UNMET** — ratio measured at 1.553x vs the 1.3x target (275,223 rows/s); 16.3% short | **#3248** |
| "The corpus generator and measurement scripts are COMMITTED and runnable from a clean checkout" (R4) | **UNMET** — rig split out by owner order | **#3272** |

Both Arrow-encode levers measured at ZERO and NEITHER was retained (lever 4 reverted →
**#3281**; lever 6 measured twice at zero, second time with the mechanism genuinely live:
+0.30%, 95% CI [−1,661,+3,125] covering zero). What shipped with value is verification
machinery, incl. the IPC-framing attribution at 313.0 ns/row. The unabridged verdicts,
evidence and owner rulings are in
`openspec/changes/archive/2026-08-04-arrow-encode-doget/tasks.md` (archived verbatim) —
that file, not this one, is the delivery record.
