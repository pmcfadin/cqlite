    ...
    for test_path in &config.test_data_directories {
        if let Ok(entries) = std::fs::read_dir(test_path) {
            for entry in entries.flatten().take(3) {
                // Limit to 3 test cases per format
                let path = entry.path();
                if path.is_dir() {
                    tests.push(CompatibilityTestCase {
                        name: format!("{} Format Compatibility", format.to_uppercase()),
                        description: format!(
                            "Validate {} output format compatibility with standard tools",
                            format
                        ),
...
fn generate_cqlsh_compatibility_tests(config: &ValidationConfig) -> Vec<CompatibilityTestCase> {
    ...
    vec![
        ...
        CompatibilityTestCase {
            ...
            description: "Ensure data values are formatted like cqlsh (dates, UUIDs, etc.)"
                .to_string(),
            sstable_path: test_data_path.clone(),
            output_format: OutputFormat::Table,
...
pub async fn run_test(
    ...
    if metrics.format_compliance_score >= 0.95 {
        ...
        result.warnings.push(format!(
            "Format compliance below optimal: {:.1}%",
            metrics.format_compliance_score * 100.0
        ));
    } else {
        result.status = ValidationStatus::Failed;
        result.errors.push(format!(
            "Poor format compliance: {:.1}%",
            metrics.format_compliance_score * 100.0
        ));
    }
    ...
    result.details.insert(
        ...
        format!("{:?}", test_case.output_format),
    );
    result.details.insert(
        "compatibility_check".to_string(),
        format!("{:?}", test_case.compatibility_check),
    );
    result.details.insert(
        "violations_count".to_string(),
        metrics.format_violations.len().to_string(),
    );
    result.details.insert(
        "metrics".to_string(),
...
async fn get_cqlite_output(
    ...
    if output.status.success() {
        ...
        if matches!(
            test_case.compatibility_check,
            CompatibilityCheck::ErrorMessages
        ) {
...
async fn get_reference_output(
    _test_case: &CompatibilityTestCase,
    _config: &ValidationConfig,
) -> Option<String> {
