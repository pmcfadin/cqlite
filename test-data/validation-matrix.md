...
| test_da | 3 (+2 committed local-only, see below) | enforced | ✅ |
| test_oa | 6 | enforced | ✅ |
...
#### Committed local-only BTI (`da`) fixtures — NOT in the published dataset asset

Two `test_da` tables have their binaries COMMITTED to the repo (`git add -f`,
they are otherwise gitignored) rather than shipped in the dataset asset. They are
**dynamically enumerated**: `smoke-test-all-tables.sh` derives its table list from
disk, so both are discovered and RUN wherever the checkout is present — the
`Tables` column above counts only what the published asset carries.

| Fixture | Shape | Rows | Smoke run-mode | Dataset asset | `check-dataset-manifest.sh` |
|---------|-------|------|----------------|---------------|-----------------------------|
| `test_da/wide_table` (`da-2-bti`) | `PRIMARY KEY (pk, ck)` — one `int` clustering column, 3 partitions x 300 rows, LZ4 | 900 | **enforced** (discovered from disk; PASS) | ❌ not in `v3.5` | **skip-pending** — `test_da` is intentionally not enrolled |
| `test_da/multiclustering_table` (`da-2-bti`) | `PRIMARY KEY (pk, bucket, seq)` — COMPOUND clustering (`text` then `int`), 3 partitions of non-uniform shape (3x60, 5x32, 8x16), LZ4 | 468 | **enforced** (discovered from disk; PASS) | ❌ not in `v3.5` | **skip-pending** — `test_da` is intentionally not enrolled |

"Enforced" and "skip-pending" here are two DIFFERENT gates and both statuses above
are deliberate:

* **`smoke-test-all-tables.sh` — enforced.** `test_da` is in neither
  `SKIP_KEYSPACE_NAMES` nor `SKIP_PENDING_KEYSPACES`, so every `test_da` table
  found on disk is run and must PASS. Both fixtures do (900 and 468 entries
  against their committed JSONL goldens).
* **`test-data/scripts/check-dataset-manifest.sh` — skip-pending, deliberately
  NOT enrolled.** That script asserts the published dataset ASSET contains a
  `Data.db` for every table in its 39-table enforced list; `test_da` is excluded
  there by design (see its header comment). Enrolling either fixture would red CI
  on every checkout, because neither ships in the asset. `test-data/dataset-pin.env`
  therefore stays at `v3.5` and `scripts/ci/check-dataset-pin.sh` stays green —
  no republish is required for these fixtures (issue #3032, owner decision).

`multiclustering_table` is regenerated by
[`test-data/scripts/gen-multiclustering-bti.sh`](scripts/gen-multiclustering-bti.sh)
(cassandra:5.0.2, `storage_compatibility_mode: NONE`, `selected_format: bti`,
`column_index_size: 2KiB`). Its schema is
[`test-data/schemas/multiclustering-table-bti.cql`](schemas/multiclustering-table-bti.cql).
It exists because `wide_table`'s single `int` clustering column makes every
`Rows.db` row-index separator a fixed-width `40 80 00 00 <byte>`, so its trie
degenerates into a chain of 2-byte single-transition nodes and its three
partitions are structurally identical — a shape that cannot discriminate a
mis-based row-index root, a proper-PREFIX clustering bound, or the OSS50
variable-length (`text`) component encoding.
