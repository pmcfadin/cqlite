...
command -v docker >/dev/null 2>&1 || { echo "[verdicts] docker required" >&2; exit 1; }
[[ -d "$CORPUS" ]] || { echo "[verdicts] corruption corpus missing: $CORPUS (run generate-corruption-corpus.sh)" >&2; exit 1; }
...
[[ "$up" -eq 1 ]] || { echo "[verdicts] cassandra did not come up" >&2; docker rm -f "$CID" >/dev/null 2>&1; exit 1; }
...
CREATE TABLE IF NOT EXISTS test_da.wide_table (pk int, ck int, body text, PRIMARY KEY (pk, ck));
" >/dev/null 2>&1 || true
...
run_verify() {
  ...
  [[ -n "$livedir" ]] || { echo "[verdicts] no live dir $ks.$tbl" >&2; return; }
  docker exec "$CID" sh -c "rm -f $livedir/*-* 2>/dev/null || true"
  docker cp "$srcdir/." "$CID:$livedir/" 2>/dev/null
  docker exec "$CID" sh -c "rm -f $livedir/*.jsonl $livedir/*.db.txt $livedir/*.yml $livedir/*.md $livedir/*.txt.* 2>/dev/null || true"
  docker exec "$CID" chown -R cassandra:cassandra "$livedir" 2>/dev/null || true
...
CLEANSRC=$(ls -d "$DATASETS"/sstables/test_comp/lz4_table-* 2>/dev/null | head -1)
[[ -n "$CLEANSRC" ]] && run_verify "$CLEANSRC" test_comp lz4_table CLEAN_BASELINE_lz4

for fx in "$CORPUS"/*/; do
  fx="${fx%/}"; name="$(basename "$fx")"
  [[ -f "$fx"/*-Data.db ]] 2>/dev/null || { ls "$fx"/*-Data.db >/dev/null 2>&1 || continue; }
