...
FIXTURES+=("data_db_truncation|cass.corruption.data_db.truncation|$SRC_LZ4|nb-1-big-Data.db|truncate|4096||Data.db|UnexpectedEof/TruncatedChunk|Tail truncated mid-stream so the final chunk(s) referenced by CompressionInfo.db cannot be read.")
FIXTURES+=("compression_info_bad_offset|cass.corruption.compression_info.bad_offset|$SRC_LZ4|nb-1-big-CompressionInfo.db|setbyte|47|128|CompressionInfo.db|ChunkOffsetOutOfBounds|chunk[1] offset MSB set -> chunk offset points ~9.2e18 bytes past Data.db end.")
...
FIXTURES+=("bti_rows_truncation|cass.corruption.bti_rows_truncation|$ROWS_DIR_BTI|__BTI_ROWS__|truncate|256||Rows.db|UnexpectedEof/TruncatedTrie|Rows.db trie truncated mid-node so row-trie traversal hits EOF.")
...
FIXTURES+=("summary_db_truncation|cass.corruption.summary_db_truncation|$SRC_LZ4|nb-1-big-Summary.db|truncate|16||Summary.db|UnexpectedEof/TruncatedSummary|Summary.db truncated inside the index-samples block so the summary cannot be deserialized.")
