...
#   filter_db_bit_flip          Filter.db (nb/BIG) bit-array bit flip   fixed
...
#   * Filter.db (nb/BIG) layout is [hash_count: i32 BE][num_longs: i32 BE][bitset:
#     num_longs*8 bytes]. Byte 8 is the FIRST byte of the bit array (the header is
#     bytes 0..7 and is left VALID). For the lz4_table filter that byte is 0x10 (a
#     SET bit), so XOR 0x10 clears it to 0x00 — turning off a bit a present key
#     hashes to, which produces a Bloom-filter FALSE NEGATIVE for that live
#     partition (might_contain==false) while the header still decodes cleanly.
...
#   * uncompressed_table (nb/BIG, NO CompressionInfo.db, has CRC.db) Data.db byte
#     70000 sits inside uncompressed chunk 1 (the 64 KiB block [65536,131072) of a
#     ~190 KiB / 3-chunk Data.db); XOR 0x01 flips one bit there so chunk 1's CRC32
#     no longer matches the stored CRC.db entry -> read-path fails fast with a typed
#     Error::Corruption naming chunk 1, and `verify --mode full` reports
#     UncompressedChunkCrcMismatch (issue #1396). A LATER (non-first) chunk is chosen
#     deliberately so the fixture also proves correct cross-boundary chunk attribution.
...
FIXTURES+=("filter_db_bit_flip|cass.corruption.filter_db.bit_flip_big|$SRC_LZ4|nb-1-big-Filter.db|bitflip|8|16|Filter.db|FilterFalseNegative|Bit-array bit flip (byte 8, the first bitset byte past the valid 8-byte header): a SET bit is cleared 1->0 so a present partition key hashes to an unset bit -> Bloom-filter FALSE NEGATIVE (might_contain==false). The BIG point-lookup path then returns Ok(None) for a live partition (silent data invisibility); full scans and BTI lookups are unaffected. Filter.db has no checksum so the flip is not caught on load. CQLite adds a verify --mode full FilterFalseNegative detection Cassandra lacks.|clean|divergent|Cassandra 5.0.2 sstableverify -e verifies CLEAN (captured: 'Verify of BigTableReader:big(...nb-1-big-Data.db) succeeded. All 1 rows read successfully'): the standalone verifier does NOT read/validate Filter.db bit-array contents (Cassandra only rebuilds a filter it cannot deserialize; a parseable-but-wrong filter passes), so a flipped bit-array bit does NOT trip its verify. CQLite is intentionally STRICTER: verify --mode full re-probes every present Index.db key against the decoded filter and reports FilterFalseNegative. Recorded divergence + a detection tool Cassandra lacks; the read-side exposure (silent false negative on BIG point lookups) is Cassandra-equivalent (issue #1398).|5bad4443c287d42f9ccfcec1a151669549b4cf41bc4dc52e0bdb2fcf57dd2941|yes|88b3312ab3bf1455107b524b3341f42fc049b7114238bfaa7bf8411e4e910c98")
...
FIXTURES+=("uncompressed_data_bit_flip|cass.corruption.uncompressed_data.bit_flip|$SRC_UNC|nb-1-big-Data.db|bitflip|70000|1|Data.db|UncompressedChunkCrcMismatch|Single-bit flip at Data.db byte 70000 inside uncompressed chunk 1 (the 64 KiB block [65536,131072) of a ~190 KiB / 3-chunk uncompressed Data.db). The clean source is test_comp.uncompressed_table (NO CompressionInfo.db; ships a CRC.db). The flip changes chunk 1 so its CRC32 no longer matches the stored CRC.db entry: the read path fails fast with a typed Error::Corruption naming chunk 1 (never wrong values / never a silent 0 rows), and verify --mode full reports UncompressedChunkCrcMismatch for the chunk (issue #1396). A non-first chunk is chosen so the fixture also proves cross-boundary chunk attribution.|corrupt|equivalent|Apache Cassandra 5.0.2 sstableverify -e: 'WARNING: Corrupted file: integrity check (digest) failed ... 65963209 != 3867023516' -> 'Invalid SSTable ..., please force a full repair' (rc=1). The clean uncompressed_table source verifies clean ('Verify ... succeeded. All 1 rows read successfully'). corrupt/corrupt parity; CQLite reports the per-chunk UncompressedChunkCrcMismatch (CRC.db) in addition to the whole-file DigestMismatch.|8daa51d7fe8596eaf588eb69f53e4a5ae9103065f1975b8033d89f74ba8239d5|yes|c0b3766f0ee01d51ea680e0e066ffa3efd273e307d7aab8d06e6ab7bf4ade969")
...
captured against (issue #1294 Item 1)."
  ...
  if [[ "$dir_sha" != "$verdict_dir_sha" ]]; then
    if [[ "$VERIFY_ONLY" -eq 1 ]]; then
      ...
      # committed fixture. By the time execution reaches here, the mutated-component
      # sha is ALREADY GUARANTEED to match its captured sha — the mutated-component
      # check above (verdict_sha != corr_sha) fails hard in MODE 1 — so any full-dir
      # drift on this path is NECESSARILY a NON-mutated sibling that changed. FATAL
      # UNCONDITIONALLY (issue #1294 roborev Finding 1): there is no live Cassandra to
      # recapture from on this path. This mirrors the committed-fixture parity test
      # (sstable_parity_corruption_verify.rs::check_full_dir_binding), which fails
      # unconditionally against the byte-stable git tree.
