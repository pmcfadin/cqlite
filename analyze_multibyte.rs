fn main() {
    ...
    for (expected_value, bytes) in cases {
        println!("\nExpected: {}, Bytes: {:02X} {:02X}", expected_value, bytes[0], bytes[1]);
        ...
        // Maybe it's storing the two's complement directly
        ...
        let sign_extended = if unsigned_13 >= 0x800 { // Sign bit for 12 bits?
        ...
        // First byte 0xC0 just indicates "two-byte format"
        // Second byte is the actual value (signed)
