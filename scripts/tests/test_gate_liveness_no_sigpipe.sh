...
# the shared matcher library (scripts/tests/lib/sigpipe-matcher.sh) and in the run-time DECLARED
# SCOPE block, and pinned as cases 6,7,8,9,9e,9i.
...
# ---------------------------------------------------------------------------
# The matcher — SOURCED, not defined here (#4061).
#
# It used to live in this file. It now lives in scripts/tests/lib/sigpipe-matcher.sh because a
# SECOND consumer exists (scripts/tests/test_scripts_sigpipe_ratchet.sh, the class ratchet over
# every git-tracked scripts/**/*.sh), and CLAUDE.md is explicit that a canonical form implemented
# twice diverges silently. THE RULE, the six DECLARED false-positive classes and residuals a/b/c
# are stated in that library; the 33 cases below are what PIN it, for both consumers.
#
# Fail-closed: a missing or unsourceable library means no scan happened at all, which must never
# read as "0 violations".
# shellcheck source=scripts/tests/lib/sigpipe-matcher.sh
if ! . "$SCRIPT_DIR/lib/sigpipe-matcher.sh" 2>/dev/null || ! declare -F sigpipe_violations >/dev/null 2>&1; then
  printf 'FAIL 0 matcher library: %s is absent, unreadable or did not define sigpipe_violations\n' \
    "$SCRIPT_DIR/lib/sigpipe-matcher.sh"
  printf '     REMEDY: restore scripts/tests/lib/sigpipe-matcher.sh (#4061) — without it nothing was scanned\n'
  printf '\npassed=0 failed=1 cases=1\n'
  exit 1
fi
# The name this suite has always used, kept so every case below reads unchanged.
violations() { sigpipe_violations "$1"; }
...
_declared_fp() { # _declared_fp <n> <label> <line-of-bash>
  ...
  if [ "$got" -ge 1 ]; then
    ...
    bad "$n DECLARED false positive: $label" "expected the broad form to REPORT this shape and it did not — the matcher has been narrowed; see #3992 and update the declaration in scripts/tests/lib/sigpipe-matcher.sh"
  fi
}
