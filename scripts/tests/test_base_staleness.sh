...
if run 4 "the matched record's sha resolves to a commit behind the base" "$R_MOTIV"; then
  ...
  # THE REV-LISTS ARE CAPTURED, NEVER PIPED INTO `grep -q` (#3650 review R5).
  # Under this suite's `set -o pipefail`, `git rev-list … | grep -qx …` fails
  # NON-DETERMINISTICALLY: `grep -q` exits at the first match, git then takes
  # EPIPE (141), and pipefail propagates that as the pipeline's status — so the
  # case reds exactly when the needle is found EARLY. Observed once in ~5 runs of
  # this suite (this same assertion, on an unchanged fixture). A wall-clock/race
  # red in a correctness test is the #2642 defect class; capturing first removes
  # the pipeline entirely.
  m_behind=$'\n'$(g "$R_MOTIV" rev-list mainline 2>/dev/null)$'\n'
  m_subject=$'\n'$(g "$R_MOTIV" rev-list feature 2>/dev/null)$'\n'
  m_in_behind=0
  m_in_subject=0
  case "$m_behind" in *$'\n'"$m_full"$'\n'*) m_in_behind=1 ;; esac
  case "$m_subject" in *$'\n'"$m_full"$'\n'*) m_in_subject=1 ;; esac
  if [ -n "$m_full" ] && [ "$m_in_behind" -eq 1 ] && [ "$m_in_subject" -eq 0 ]; then
...
# PARTIAL/PROMISOR clone, plain object access fetches over the network and WRITES
# a packfile into the repository — so the shipped absolute "never fetches, never
# writes" was FALSE on that configuration, and the script's own comment admitted
# it four lines later. The advisory now decides between three cases before ANY
# object access; these cases pin all three, plus the boundary and the
# UNDETERMINED branch.
#
# THE FALSIFYING CONTRAST WAS MEASURED, not argued (git 2.43.0 host, a real
# `--filter=tree:0 --no-local` clone, the old-git shim below): the PRE-FIX script
# took the object store from 4 files to 8 — a packfile written into the
# repository — and reported `NO-STALENESS-RECOGNISED`; the fixed script leaves it
# at 4 and reports `UNMEASURED`. Case 19a asserts the object-store half directly,
# so a regression is caught by an artifact on disk and not only by a verdict.
#
# THE SHIM SIMULATES AN OLD GIT FAITHFULLY, WHICH TAKES BOTH HALVES: it reports
# the old version string AND `unset`s `GIT_NO_LAZY_FETCH` before delegating to the
# real git. Reporting the version alone would leave the real git still honouring
# the variable, so the object-store assertion would pass for the wrong reason —
# the case would prove nothing about lazy fetching.
REAL_GIT=$(command -v git 2>/dev/null) || REAL_GIT=""
if [ -z "$REAL_GIT" ]; then
  bad "lazy-fetch-guard: \`git\` is not on PATH — this whole suite cannot run"
fi

# make_git_shim <dir> <version-line|REAL> [config-exit] — a `git` first on PATH
# that reports <version-line> for `--version`, optionally fails every
# `git config` invocation with <config-exit>, unsets GIT_NO_LAZY_FETCH, and
# otherwise delegates to the real git.
make_git_shim() {
  local d="$1" ver="$2" cfgrc="${3:-}"
  mkdir -p "$d"
  {
    printf '#!/usr/bin/env bash\n'
    if [ "$ver" != REAL ]; then
    ...
    if [ -n "$cfgrc" ]; then
      printf 'if [ "$1" = config ]; then printf "shimmed config failure\\n" >&2; exit %s; fi\n' "$cfgrc"
    fi
    printf 'unset GIT_NO_LAZY_FETCH\n'
    printf 'exec %s "$@"\n' "$REAL_GIT"
  } >"$d/git"
  chmod +x "$d/git"
}

# run_shim <expected-exit> <desc> <repo> <shim-dir> — run() with a git shim
# first on PATH. Output is accumulated for the whole-suite anchor assertions.
run_shim() {
  local want="$1" desc="$2" repo="$3" shim="$4"
  OUT=$(cd "$repo" && PATH="$shim:$PATH" bash "${USE_SCRIPT:-$ADVISORY}" 2>&1)
  RC=$?
  record_out "$desc"
  if [ "$RC" -ne "$want" ]; then
    bad "$desc (exit $RC, wanted $want)"
...
SHIM_OLD="$T/shim-git-old"
SHIM_236="$T/shim-git-236"
SHIM_235="$T/shim-git-235"
SHIM_UNPARSEABLE="$T/shim-git-unparseable"
SHIM_CFGFAIL="$T/shim-git-cfgfail"
make_git_shim "$SHIM_OLD" "git version 2.20.1"
make_git_shim "$SHIM_236" "git version 2.36.0"
make_git_shim "$SHIM_235" "git version 2.35.9"
make_git_shim "$SHIM_UNPARSEABLE" "git version banana"
make_git_shim "$SHIM_CFGFAIL" "git version 2.20.1" 3

# NON-VACUITY for the shims themselves: a shim that did not really report the
# version it claims would make every case below pass for the wrong reason.
shim_vacuous=0
for pair in "$SHIM_OLD:2.20.1" "$SHIM_236:2.36.0" "$SHIM_235:2.35.9" \
  "$SHIM_UNPARSEABLE:banana"; do
  _sd=${pair%:*}
  _sv=${pair##*:}
  if [ "$(PATH="$_sd:$PATH" git --version 2>/dev/null)" = "git version $_sv" ]; then
    continue
  fi
  bad "lazy-fetch-guard fixture: the shim $_sd does not report 'git version $_sv'"
  shim_vacuous=1
done
if [ "$shim_vacuous" -eq 0 ]; then
  ok "lazy-fetch-guard fixture: every git shim really reports the version its case claims"
fi
if PATH="$SHIM_CFGFAIL:$PATH" git config --get user.name >/dev/null 2>&1; then
  bad "lazy-fetch-guard fixture: the config-failing shim still answers \`git config\` (the case would be vacuous)"
else
  ok "lazy-fetch-guard fixture: the config-failing shim really fails \`git config\`"
fi

# --- fixture: promisor by CONFIG, every object LOCAL -------------------------
# `remote.<name>.promisor=true` is what git 2.43 actually writes for a partial
# clone (measured: `extensions.partialclone` was NOT set), so this is the real
# marker and not an invented one. Every object is local, so a git that DOES
# honour the variable can still MEASURE this repo — which is what makes Case 19c
# a differential: the same repository, the version gate the only difference.
R_PROM_CFG=$(newrepo promisor-config)
commit_paths "$R_PROM_CFG" "the PR: sstable module" \
  "cqlite-core/src/storage/sstable/mod.rs"
advance_main "$R_PROM_CFG"
commit_paths "$R_PROM_CFG" "behind: a gate-global commit" ".config/nextest.toml"
publish_main "$R_PROM_CFG"
back_to_feature "$R_PROM_CFG"
g "$R_PROM_CFG" config remote.origin.promisor true
if [ "$(g "$R_PROM_CFG" config --bool --get remote.origin.promisor)" = true ]; then
  ok "lazy-fetch-guard fixture: the config-marked repo really carries remote.origin.promisor=true"
else
  bad "lazy-fetch-guard fixture: the config-marked repo is not marked promisor (the case would be vacuous)"
fi

# --- fixture: promisor by OBJECT-STORE MARKER, under the GIT COMMON DIR ------
# The marker a partial clone actually leaves is `objects/pack/<pack>.promisor`
# (measured on the real clone below: `objects/info/promisor` did NOT exist), so
# BOTH shapes are probed and both are fixtured. This one carries no promisor
# config at all, so it isolates source 3.
R_PROM_MARK=$(newrepo promisor-marker)
commit_paths "$R_PROM_MARK" "the PR" "cqlite-core/src/storage/sstable/mod.rs"
publish_main "$R_PROM_MARK"
back_to_feature "$R_PROM_MARK"
PM_COMMON=$(cd "$R_PROM_MARK" && git rev-parse --git-common-dir)
case "$PM_COMMON" in /*) ;; *) PM_COMMON="$R_PROM_MARK/$PM_COMMON" ;; esac
mkdir -p "$PM_COMMON/objects/pack"
: >"$PM_COMMON/objects/pack/pack-deadbeef.promisor"
if [ -e "$PM_COMMON/objects/pack/pack-deadbeef.promisor" ] &&
  [ -z "$(g "$R_PROM_MARK" config --get-regexp '^remote\..*\.promisor$')" ]; then
  ok "lazy-fetch-guard fixture: the marker repo has a pack .promisor marker and NO promisor config"
else
  bad "lazy-fetch-guard fixture: the marker repo does not have the shape its case claims"
fi

R_PROM_INFO=$(newrepo promisor-info-marker)
commit_paths "$R_PROM_INFO" "the PR" "cqlite-core/src/storage/sstable/mod.rs"
publish_main "$R_PROM_INFO"
back_to_feature "$R_PROM_INFO"
PI_COMMON=$(cd "$R_PROM_INFO" && git rev-parse --git-common-dir)
case "$PI_COMMON" in /*) ;; *) PI_COMMON="$R_PROM_INFO/$PI_COMMON" ;; esac
mkdir -p "$PI_COMMON/objects/info"
: >"$PI_COMMON/objects/info/promisor"

# --- fixture: a REAL partial clone ------------------------------------------
# `--filter=tree:0 --no-local` over a `file://` URL against a source with
# `uploadpack.allowFilter` — no network, and the resulting repo is a genuine
# promisor clone whose objects are NOT all local. This is the only fixture that
# can demonstrate the OBJECT-STORE half (a lazy fetch really writing a
# packfile), so Case 19a is the primary evidence for F1.
#
# It is SKIPPED (visibly, with a named reason) rather than failed if the
# transport-dependent clone cannot be built: the config-marked and marker
# fixtures above cover the same detection sources with no transport at all, so a
# host whose git cannot do a filtered file:// clone loses one case rather than
# reding a suite on correct code.
R_PARTIAL_SRC="$T/partial-src"
R_PARTIAL="$T/partial-clone"
partial_shape=0
mkdir -p "$R_PARTIAL_SRC"
git init -q -b mainline "$R_PARTIAL_SRC" >/dev/null 2>&1
g "$R_PARTIAL_SRC" config user.email t@t
g "$R_PARTIAL_SRC" config user.name t
commit_paths "$R_PARTIAL_SRC" "c0" "README.md" ".config/nextest.toml"
commit_paths "$R_PARTIAL_SRC" "c1 gate-global" ".config/nextest.toml"
g "$R_PARTIAL_SRC" config uploadpack.allowfilter true
g "$R_PARTIAL_SRC" config uploadpack.allowanysha1inwant true
if git clone -q --filter=tree:0 --no-local --no-checkout \
  "file://$R_PARTIAL_SRC" "$R_PARTIAL" >/dev/null 2>&1; then
  git -C "$R_PARTIAL" update-ref "$MAIN_REF" mainline >/dev/null 2>&1
  _pm=0
  for _f in "$R_PARTIAL"/.git/objects/pack/*.promisor; do
    [ -e "$_f" ] && _pm=1
  done
  if [ "$(git -C "$R_PARTIAL" config --bool --get remote.origin.promisor 2>/dev/null)" = true ] &&
    [ "$_pm" -eq 1 ]; then
    partial_shape=1
  fi
fi
if [ "$partial_shape" -eq 1 ]; then
  ok "lazy-fetch-guard fixture: the real --filter=tree:0 clone IS a promisor clone with a pack marker"
else
  echo "skip - lazy-fetch-guard: this host's git could not build a filtered file:// clone; the config-marked and marker fixtures still cover detection"
fi

# --- Case 19a: OLD GIT + a REAL partial clone -> UNMEASURED, nothing written --
if [ "$partial_shape" -eq 1 ]; then
  objs_before=$(find "$R_PARTIAL/.git/objects" -type f | wc -l | tr -d ' ')
  if run_shim 5 "old git in a REAL partial clone is UNMEASURED" "$R_PARTIAL" "$SHIM_OLD"; then
    has "lazy-fetch-guard: the verdict is UNMEASURED on an old git in a promisor clone" \
      "verdict UNMEASURED"
    has "lazy-fetch-guard: the cause names the git version field and that it is not honoured" \
      "git-version 2.20 honours-GIT_NO_LAZY_FETCH NO"
    has "lazy-fetch-guard: the cause names the promisor state and its evidence" \
      "promisor YES evidence remote.origin.promisor=true"
    has "lazy-fetch-guard: the guard line reports what was measured" \
      "lazy-fetch-guard git-version 2.20 honours-GIT_NO_LAZY_FETCH NO promisor YES"
    # NO OBJECT ACCESS HAPPENED: neither the subject nor the behind count is
    # reported, because the refusal precedes every object read.
    lacks "lazy-fetch-guard: no behind-count is reported (no object access happened)" \
      "BASE-STALENESS: behind "
    lacks "lazy-fetch-guard: no blast-radius count is reported" "blast-radius"
  fi
  objs_after=$(find "$R_PARTIAL/.git/objects" -type f | wc -l | tr -d ' ')
  if [ "$objs_before" = "$objs_after" ]; then
    ok "lazy-fetch-guard: the object store is UNCHANGED ($objs_before files) — nothing was fetched or written"
  else
    bad "lazy-fetch-guard: the object store grew $objs_before -> $objs_after — a lazy fetch WROTE into the repository (#3650 R5 F1)"
  fi
fi

# --- Case 19b: OLD GIT + a promisor-by-config repo -> UNMEASURED -------------
if run_shim 5 "old git in a config-marked promisor repo is UNMEASURED" \
  "$R_PROM_CFG" "$SHIM_OLD"; then
  has "lazy-fetch-guard(config): UNMEASURED" "verdict UNMEASURED"
  has "lazy-fetch-guard(config): the promisor evidence is the config key" \
    "promisor YES evidence remote.origin.promisor=true"
  lacks "lazy-fetch-guard(config): no measured verdict is emitted" "NO-STALENESS-RECOGNISED"
  lacks "lazy-fetch-guard(config): no STALE verdict is emitted either" "STALE-RECOGNISED"
fi

# --- Case 19c: THE DIFFERENTIAL — the same repo on a git that DOES honour it --
# Same repository, same promisor config, every object local: the ONLY difference
# is the git version. A supported git needs no promisor probe at any clone shape,
# so this run MEASURES and says `promisor NOT-PROBED`.
if run_shim 4 "a supported git measures the SAME promisor repo (promisor NOT-PROBED)" \
  "$R_PROM_CFG" "$SHIM_236"; then
  has "lazy-fetch-guard(differential): 2.36 is the floor and is honoured" \
    "lazy-fetch-guard git-version 2.36 honours-GIT_NO_LAZY_FETCH YES promisor NOT-PROBED"
  has "lazy-fetch-guard(differential): the scan really measured" "verdict STALE-RECOGNISED"
  lacks "lazy-fetch-guard(differential): no no-fetch refusal" "cannot be upheld"
fi
# The boundary is pinned from BELOW too: 2.35.9 must NOT be treated as honoured.
if run_shim 5 "2.35.9 is below the floor and refuses in a promisor repo" \
  "$R_PROM_CFG" "$SHIM_235"; then
  has "lazy-fetch-guard(boundary): 2.35.9 is NOT honoured" \
    "git-version 2.35 honours-GIT_NO_LAZY_FETCH NO"
fi

# --- Case 19d: the OBJECT-STORE marker sources, in isolation ----------------
if run_shim 5 "old git + a pack .promisor marker (no promisor config) is UNMEASURED" \
  "$R_PROM_MARK" "$SHIM_OLD"; then
  has "lazy-fetch-guard(marker): the pack marker is the named evidence" \
    "promisor YES evidence the promisor pack marker"
fi
if run_shim 5 "old git + an objects/info/promisor marker is UNMEASURED" \
  "$R_PROM_INFO" "$SHIM_OLD"; then
  has "lazy-fetch-guard(info-marker): the info marker is the named evidence" \
    "promisor YES evidence the promisor marker"
fi

# --- Case 19e: NO VERSION FLOOR ON THE COMMON CASE --------------------------
# An affirmatively NON-promisor repository has nothing to lazily fetch FROM, so
# an old git is unaffected and the scan proceeds. Burdening this case with a
# version floor would red the tool on correct input — the guard agents learn to
# waive.
if run_shim 4 "old git in a NON-promisor repo still measures (no version floor)" \
  "$R_MOTIV" "$SHIM_OLD"; then
  has "lazy-fetch-guard(non-promisor): the guard line records the affirmative NO" \
    "lazy-fetch-guard git-version 2.20 honours-GIT_NO_LAZY_FETCH NO promisor NO"
  has "lazy-fetch-guard(non-promisor): the scan really measured" "verdict STALE-RECOGNISED"
  lacks "lazy-fetch-guard(non-promisor): no no-fetch refusal" "cannot be upheld"
fi

# --- Case 19f: an UNPARSEABLE version is UNSUPPORTED, never assumed-good ----
if run_shim 5 "an unparseable git version refuses in a promisor repo" \
  "$R_PROM_CFG" "$SHIM_UNPARSEABLE"; then
  has "lazy-fetch-guard(unparseable): the version field reads UNPARSEABLE" \
    "git-version UNPARSEABLE honours-GIT_NO_LAZY_FETCH NO"
  has "lazy-fetch-guard(unparseable): the raw string it could not parse is reported" \
    "reported: git version banana"
fi
if run_shim 4 "an unparseable git version still measures a NON-promisor repo" \
  "$R_MOTIV" "$SHIM_UNPARSEABLE"; then
  has "lazy-fetch-guard(unparseable): a non-promisor repo is unaffected" \
    "lazy-fetch-guard git-version UNPARSEABLE honours-GIT_NO_LAZY_FETCH NO promisor NO"
fi

# --- Case 19g: an UNDETERMINED promisor state is UNMEASURED, not "absent" ----
# The probe is THREE-valued. A failing `git config` is "cannot tell", and
# collapsing that onto the permissive answer is the shape #3650 exists to refuse.
if run_shim 5 "a failing promisor probe is UNMEASURED, not read as non-promisor" \
  "$R_MOTIV" "$SHIM_CFGFAIL"; then
  has "lazy-fetch-guard(undetermined): the promisor state is UNKNOWN" "promisor UNKNOWN"
  has "lazy-fetch-guard(undetermined): the reason it is undetermined is named" \
    "undetermined git config --get extensions.partialclone exited 3"
  lacks "lazy-fetch-guard(undetermined): it is NOT reported as non-promisor" "promisor NO"
fi
