...
run_anc() {
  local want="$1" desc="$2"
  shift 2
  # `--c-verdict` is REQUIRED (#3751), so this runner goes through the SAME
  # injection `run` uses rather than carrying a second opinion about the flag:
  # every #3653 case is about ANCHOR ANCESTRY, not about the C verdict, so it
  # gets the explicit PASS file and keeps asserting exactly what it asserted.
  c_inject_into ${1+"$@"}
  set -- ${C_INJ[@]+"${C_INJ[@]}"}
  OUT=$(cd "$ANC_REPO" && PATH="$BIN:$PATH" MOCK_GH_FAIL=0 MOCK_GH_OUT="$R_CERT OPEN" \
    bash "$NEUTRAL_ASSERT" ${1+"$@"} 2>&1)
  RC=$?
  if [ "$RC" -ne "$want" ]; then
    bad "$desc (exit $RC, wanted $want)"
    printf '     output: %s\n' "$OUT"
...
if [ "$mut_ok" -eq 1 ]; then
  MUTOUT=$(cd "$ANC_REPO" && PATH="$BIN:$PATH" MOCK_GH_FAIL=0 MOCK_GH_OUT="$R_CERT OPEN" \
    bash "$MUTDIR/premerge-assert.sh" 2421 "$R_CERT" "$RFORFULL" "$RFORDELTA" --c-verdict "$C_PASS_FILE" 2>&1)
  MUTRC=$?
  if [ "$MUTRC" -eq 0 ]; then
...
if (cd "$NOREPO" && git rev-parse --git-dir >/dev/null 2>&1); then
  ...
  OUT=$(cd "$NOREPO" && PATH="$BIN:$PATH" MOCK_GH_FAIL=0 MOCK_GH_OUT="$R_CERT OPEN" \
    bash "$NEUTRAL_ASSERT" 2421 "$R_CERT" "$RANCFULL" "$RGOODDELTA" --c-verdict "$C_PASS_FILE" 2>&1)
  RC=$?
  if [ "$RC" -ne 3 ]; then
...
if [ "$nogit_ok" -ne 1 ]; then
  ...
  OUT=$(cd "$ANC_REPO" && PATH="$NOGIT" MOCK_GH_FAIL=0 MOCK_GH_OUT="$R_CERT OPEN" \
    "${BASH:-/bin/bash}" "$NEUTRAL_ASSERT" 2421 "$R_CERT" "$RANCFULL" "$RGOODDELTA" \
...
if [ "$shallow_shape" -eq 1 ]; then
  ...
  OUT=$(cd "$SH_REPO" && PATH="$BIN:$PATH" MOCK_GH_FAIL=0 MOCK_GH_OUT="$SH_C OPEN" \
    bash "$NEUTRAL_ASSERT" 2421 "$SH_C" "$SHFULL" "$SHDELTA" --c-verdict "$C_PASS_FILE" 2>&1)
  RC=$?
...
if [ "$broken_shape" -eq 1 ]; then
  ...
  OUT=$(cd "$BR_INTACT" && PATH="$BIN:$PATH" MOCK_GH_FAIL=0 MOCK_GH_OUT="$BR_C OPEN" \
    bash "$NEUTRAL_ASSERT" 2421 "$BR_C" "$BRFULL" "$BRDELTA" --c-verdict "$C_PASS_FILE" 2>&1)
  RC=$?
  if [ "$RC" -eq 0 ] && [ "${OUT#*anchor-ancestry: BOUND}" != "$OUT" ]; then
...
if [ "$graft_shape" -eq 1 ] && [ "$graft_attack" -eq 1 ]; then
  ...
  OUT=$(cd "$GRAFT_REPO" && PATH="$BIN:$PATH" MOCK_GH_FAIL=0 MOCK_GH_OUT="$R_CERT OPEN" \
    bash "$NEUTRAL_ASSERT" 2421 "$R_CERT" "$RFORFULL" "$RFORDELTA" --c-verdict "$C_PASS_FILE" 2>&1)
  RC=$?
  if [ "$RC" -ne 2 ]; then
...
to_run_arm() {
  ...
  out=$(cd "$ANC_REPO" && PATH="$shimdir:$BIN:$PATH" MOCK_GH_FAIL=0 MOCK_GH_OUT="$R_CERT OPEN" \
    bash "$TOFLOW/premerge-assert.sh" 2421 "$R_CERT" "$RANCFULL" "$RGOODDELTA" \
      --c-verdict "$C_PASS_FILE" 2>&1)
  rc=$?
  if [ "$rc" -ne 3 ]; then
