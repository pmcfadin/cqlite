...
#
# TWO SUBJECTS HAVE BEEN SPLIT OUT of this file, both on the campsite rule and both by seam:
# `test_ws0_binary_provenance.sh` (round 11) owns WHICH PROGRAMS produced the ratio, and
# `test_ws0_output_dir_exclusivity.sh` (round 13) owns whether the DIRECTORY holding the
# artifacts belongs to exactly one session. Neither is this file's subject, and the reason is the
# same in both cases: every check here is satisfiable by a session whose bytes and configuration
# are impeccably identified — and whose two arms were different BUILDS, or whose rep files came
# from two RUNS that shared a directory.
#
...
#
# RE-DERIVED at round 13's output-dir split (#3272). It read 100 against a real 103; the split
# moved 22 checks to `test_ws0_output_dir_exclusivity.sh`, leaving 81 MEASURED here — so 100 was
# a floor above the real count, i.e. it would have RED this suite. A floor left pointing at a
# count that no longer exists is the same defect in the other direction as one set too low: both
# are a number nobody re-measured. 78 is the current 81 less a small margin.
MIN_CHECKS=78
