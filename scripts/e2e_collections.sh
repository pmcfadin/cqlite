...
# Tests 16 collection-containing tables against Cassandra 5.
# Pipeline:
#   A. write mutations + flush portable SSTables
#   B. validate flushed artifact with Cassandra's sstabledump
#   C. package flushed SSTables for import tooling
#   D. import into Cassandra and verify through CQL queries
...
validate_flushed_artifact() {
  ...
  if [[ $NO_DOCKER -eq 1 ]]; then
    log_warning "Skipping flushed artifact validation for ${keyspace}.${table} (--no-docker flag set)"
    return 0
  fi
