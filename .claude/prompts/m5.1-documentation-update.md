...
> **HISTORICAL ARTIFACT — do not execute.** This is the completed M5.1 sprint prompt (issues
> #419–#424, all shipped), retained for provenance. Its file/line citations describe the tree as it
> stood at M5.1 and several no longer resolve — notably the row-decode parser, which was
> `parsing/v5_compressed_legacy.rs` then and is the `parsing/row_decoder/` directory now (epic #1116
> source splits). Do not use it as a current source map: read
> `docs/sstables-definitive-guide/` and the live tree instead.
...
1. Read implementation:
   ...
   - `cqlite-core/src/storage/sstable/reader/parsing/row_decoder/` (row/cell flag decode; at the
     time of this sprint this was `parsing/v5_compressed_legacy.rs`, since split out by epic #1116)
