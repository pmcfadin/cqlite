...
/// The compared case table — WHICH tables this lane covers — in its own file under
/// the campsite rule (CLAUDE.md, epic #1135). Imported here (NOT re-exported: a
/// `pub use` of a constant whose type is crate-private is a warning) so that
/// `super::CASES` keeps naming ONE declaration for the two comparison lanes and the
/// coverage census alike — a child module sees its ancestors' private imports.
#[path = "support/issue_1491_cases.rs"]
mod cases;
use cases::CASES;
