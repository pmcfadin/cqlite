...
// =======================================================================
// The container-map-key gap validates SHAPE while declining to pair KEYS
// (roborev job 302, Medium)
// =======================================================================

/// `ContainerMapKeyNotPairableByThisLane` must not become a blind spot for its
/// whole column.
///
/// An earlier version of this arm decided from the declared type ALONE and read no
/// values, on the reasoning that a lane limitation is not a claim about rendered
/// output. Declining to pair the ENTRIES is right; declining to look at the SHAPE
/// was not, because it made the gap suppress a null or a scalar where a map should
/// be — the "permanent blind spot for its whole column" the gap module was written
/// to remove.
///
/// So the property is a SPLIT: both sides must still be maps, and neither side's
/// contents are examined.
#[test]
fn the_container_map_key_gap_suppresses_only_a_well_formed_map_pair() {
    let container_key_map = CqlType::Map(
        Box::new(CqlType::Tuple(vec![
            CqlType::Text("text".into()),
            CqlType::Numeric("int".into()),
        ])),
        Box::new(CqlType::Numeric("int".into())),
    );
    let scalar_key_map = CqlType::Map(
        Box::new(CqlType::Numeric("int".into())),
        Box::new(CqlType::Numeric("int".into())),
    );
    let gap = Divergence::ContainerMapKeyNotPairableByThisLane;
    let ask = |golden: &Value, cli: &Value, ty: &CqlType| {
        gap.matched(
            golden,
            cli,
            ty,
    ...
    // A well-formed map pair whose KEY type is a container: this IS the gap. The
    // contents deliberately differ — pairing them is the thing this lane cannot do.
    let golden_map = json!({"alpha\\:1:1": 7});
    let cli_map = json!([{"key": [{"_type": "key_part"}, 1], "value": 7}]);
    assert!(
        ask(&golden_map, &cli_map, &container_key_map),
        "a well-formed map pair with a container key type is the declared gap"
    );

    // THE REGRESSIONS THE OLD FORM SWALLOWED. Each is a column that stopped being a
    // map; none is this gap, and each must be reported as an ordinary diff.
    for broken in [json!(null), json!(0), json!("0xdeadbeef"), json!({})] {
        assert!(
            !ask(&golden_map, &broken, &container_key_map),
            "a CLI rendering that is not a map array must NOT be suppressed: {broken:?}"
        );
    }
    for broken in [json!(null), json!(0), json!("scalar")] {
        assert!(
            !ask(&broken, &cli_map, &container_key_map),
            "a GOLDEN that is not a map object must NOT be suppressed: {broken:?}"
        );
    }

    // And the gap is scoped to a container KEY: a scalar-keyed map is compared
    // normally, so declaring this gap there suppresses nothing (and is reported
    // stale by the census).
    assert!(
