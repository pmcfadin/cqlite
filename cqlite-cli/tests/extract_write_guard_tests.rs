//! Issue #4199 (spec R-CLI-4.1) — `cqlite extract --out` inside the input is
...
//! `salvage` already uses — asserted end-to-end through the compiled binary,
//! not just the guard's own unit tests, per the wiring-evidence rule.
...
fn run_extract(input: &Path, out: &Path) -> Output {
