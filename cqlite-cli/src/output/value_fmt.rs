...
//!
//! This module provides stable, cqlsh-compatible formatting for all CQL value types.
...
impl ValueFormatter {
    ...
    fn format_tuple(fields: &[Value]) -> String {
        let formatted_fields: Vec<String> = fields.iter().map(|v| Self::format_value(v)).collect();
        format!("({})", formatted_fields.join(", "))
    }

    /// Format UDT as {field1: value1, field2: value2}
    fn format_udt(udt: &cqlite_core::UdtValue) -> String {
...
mod tests {
    use super::*;
    use cqlite_core::{UdtField, UdtValue};
    ...
    #[test]
    fn test_floats() {
        assert_eq!(ValueFormatter::format_value(&Value::Float32(3.14)), "3.14");
        assert_eq!(
            ValueFormatter::format_value(&Value::Float(2.71828)),
            "2.71828"
        );
