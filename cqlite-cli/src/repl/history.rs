...
impl HistoryManager {
    ...
    fn categorize_command(&self, command: &str) -> HistoryCategory {
        ...
        if trimmed.starts_with(':') || trimmed.starts_with('.') || trimmed.starts_with('\\') {
            ...
            } else if trimmed.contains("use") || trimmed.contains("tables") || 
                      trimmed.contains("keyspaces") || trimmed.contains("describe") {
                HistoryCategory::Navigation
            } else if trimmed.contains("clear") || trimmed.contains("history") || 
    ...
    pub fn search(&self, filter: &HistoryFilter) -> Vec<&HistoryEntry> {
        let mut results: Vec<&HistoryEntry> = self.history
            .iter()
    ...
    fn matches_filter(&self, entry: &HistoryEntry, filter: &HistoryFilter) -> bool {
        ...
        if let Some(ref pattern) = filter.pattern {
            if !entry.command.to_lowercase().contains(&pattern.to_lowercase()) {
                return false;
            }
        }
    ...
    fn update_stats(&mut self, entry: &HistoryEntry) {
        ...
        *self.stats.by_category.entry(entry.category.clone()).or_insert(0) += 1;
        
        // Update average duration
        if let Some(duration) = entry.duration {
            let duration_ms = duration.as_millis() as f64;
            let total_duration = self.stats.avg_duration_ms * (self.stats.total_commands - 1) as f64;
            self.stats.avg_duration_ms = (total_duration + duration_ms) / self.stats.total_commands as f64;
        }
    }
    ...
    pub fn export_text(&self) -> String {
        ...
        output.push_str(&format!("# CQLite Command History ({})\n", 
...
mod tests {
    ...
    fn test_categorization() {
        ...
        assert_eq!(history.categorize_command("SELECT * FROM users"), HistoryCategory::Query);
        assert_eq!(history.categorize_command(":help"), HistoryCategory::Meta);
        assert_eq!(history.categorize_command(":config show"), HistoryCategory::Config);
        assert_eq!(history.categorize_command(":tables"), HistoryCategory::Navigation);
        assert_eq!(history.categorize_command(":clear"), HistoryCategory::System);
    }
