...
use super::{TestResult, TestContainer};
...
impl CliTestRunner {
    ...
    pub fn test_info(&self) -> TestResult<()> {
        let sstable_fixture = self.container.environment().fixtures_dir.join("test.sstable");
        let schema_fixture = self.container.environment().fixtures_dir.join("test.schema");
...
impl CliTestScenarios {
    ...
    pub fn test_query_commands(runner: &CliTestRunner) -> TestResult<()> {
        ...
        std::fs::write(&schema_file, r#"{"tables": [{"name": "users", "columns": []}]}"#)?;
