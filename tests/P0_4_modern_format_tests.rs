...
async fn test_big_v5_rejects_header_heuristics() {
    ...
    assert!(
        result.is_ok(),
        "BIG v5 should use structured parsing, not heuristics"
    );

    // Verify the size is calculated structurally (should be reasonable, not heuristic-based)
    let size = result.unwrap();
    assert!(
        size > 0 && size <= problematic_header.len(),
        "Header size should be reasonable: got {}",
        size
    );
}
...
async fn test_bti_rejects_header_heuristics() {
    ...
    assert!(
        result.is_ok(),
        "BTI should use structured parsing, not heuristics"
    );

    let size = result.unwrap();
    assert!(
        size > 0 && size <= problematic_header.len(),
        "BTI header size should be reasonable: got {}",
        size
    );
}
...
async fn test_modern_formats_reject_blob_fallback_static_rows() {
    ...
    if let Err(Error::Schema(msg)) = result {
        ...
        assert!(
            ...
            "Error should mention the format: {}",
            msg
        );
    } else {
    ...
    if let Err(Error::Schema(msg)) = bti_result {
        assert!(
            msg.contains("Blob fallback not allowed"),
            "BTI error should mention blob fallback rejection: {}",
            msg
        );
        assert!(
            msg.contains("V5_0Bti"),
            "BTI error should mention the format: {}",
            msg
        );
    } else {
...
async fn test_legacy_format_allows_blob_fallback_with_feature() {
    ...
    assert!(
        result.is_ok(),
        "Legacy format should allow blob fallback with feature flag"
    );
}
...
async fn test_compression_heuristics_gated_behind_feature() {
    ...
    assert!(
        true,
        "Compression heuristics should be available with feature flag"
    );
}
