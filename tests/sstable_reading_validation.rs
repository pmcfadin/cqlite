...
async fn test_sstable_reader_exists() -> Result<()> {
    let harness = SSTableTestHarness::new().await?;
    let test_data = TestSSTableData::default();
    let sstable_path = harness
        .create_test_sstable("reader_test", test_data)
        .await?;
    ...
    let _attempt =
        SSTableReader::open(&sstable_path, &harness.config, harness.platform.clone()).await;
