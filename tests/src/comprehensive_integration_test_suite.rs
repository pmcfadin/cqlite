...
impl ComprehensiveIntegrationTestSuite {
            ...
            Ok(()) => {
                ...
                self.results.push(IntegrationTestResult {
                    ...
                    performance_metrics: Some(PerformanceMetrics {
                        throughput_mb_per_sec: (total_bytes as f64 / 1024.0 / 1024.0)
                            / test_start.elapsed().as_secs_f64(),
                        avg_parse_time_us: (test_start.elapsed().as_micros() as u64)
                            / total_files.max(1) as u64,
                ...
                println!(
                    "  ✅ Real SSTable tests completed: {} files, {} bytes",
    ...
    async fn run_feature_integration_tests(&mut self) -> Result<()> {
        ...
        match self.test_multi_generation_integration().await {
            Ok((bytes_processed, files_tested)) => {
                ...
                println!(
                    "  ✅ Multi-generation Integration: {} bytes, {} files",
                    bytes_processed, files_tested
                );
            }
            Err(e) => {
                self.results.push(IntegrationTestResult {
    ...
    async fn run_directory_tests(&mut self) -> Result<()> {
        ...
        self.results.push(IntegrationTestResult {
            ...
            error_message: if total_sstables == 0 {
                Some("No SSTables discovered".to_string())
            } else {
                None
            },
            performance_metrics: None,
        ...
        println!(
            "  📊 Summary: {} directories, {} SSTables, {} bytes",
            total_directories, total_sstables, total_bytes
        );
        println!();
        Ok(())
    }
    ...
    async fn run_collection_tests(&mut self) -> Result<()> {
        ...
        let collections_table_path = self
            .config
            .test_data_path
            .join("collections_table-462afd10673711f0b2cf19d64e7cbecb");
        ...
        if collections_table_path.exists() {
            match self.test_collections_parsing(&collections_table_path).await {
                Ok((bytes, features_tested)) => {
                    ...
                    println!(
                        "  ✅ Collection types: {} features tested, {} bytes",
                        features_tested, bytes
                    );
                }
                Err(e) => {
                    self.results.push(IntegrationTestResult {
        ...
        let all_types_path = self
            .config
            .test_data_path
            .join("all_types-46200090673711f0b2cf19d64e7cbecb");
    ...
    async fn run_tombstone_tests(&mut self) -> Result<()> {
        ...
        let time_series_path = self
            .config
            .test_data_path
            .join("time_series-464cb5e0673711f0b2cf19d64e7cbecb");
        if time_series_path.exists() {
            match self.test_deletion_markers(&time_series_path).await {
                Ok((bytes, markers)) => {
                    ...
                    println!(
                        "  ✅ Time series deletions: {} markers, {} bytes",
                        markers, bytes
                    );
                }
                Err(e) => {
        ...
        let large_table_path = self
            .config
            .test_data_path
            .join("large_table-465df3f0673711f0b2cf19d64e7cbecb");
        if large_table_path.exists() {
            match self.test_ttl_handling(&large_table_path).await {
                Ok((bytes, ttl_records)) => {
                    total_bytes += bytes;
                    println!(
                        "  ✅ TTL handling: {} records, {} bytes",
                        ttl_records, bytes
                    );
                }
                Err(e) => {
        ...
        println!(
            "  📊 Total deletion markers found: {}",
            deletion_markers_found
        );
        println!();
        Ok(())
    }
    ...
    async fn run_error_handling_tests(&mut self) -> Result<()> {
        ...
        println!(
            "  📊 Error handling: {}/{} cases handled gracefully",
            error_cases_handled, error_cases_tested
        );
        println!();
        Ok(())
    }
    ...
    async fn run_cli_tests(&mut self) -> Result<()> {
        ...
        if let Ok(entries) = fs::read_dir(&self.config.test_data_path) {
            for entry in entries.flatten().take(2) {
                // Test first 2 tables to save time
                if entry.path().is_dir() {
        ...
        println!(
            "  📊 CLI tests: {}/{} passed",
            cli_tests_passed, cli_tests_total
        );
        println!();
        Ok(())
    }
    ...
    async fn run_performance_tests(&mut self) -> Result<()> {
        ...
        match benchmarks.run_all_benchmarks().await {
            Ok(()) => {
                let avg_throughput: f64 = benchmarks
                    .get_results()
                    .iter()
                    .map(|r| r.operations_per_second)
                    .sum::<f64>()
                    / benchmarks.get_results().len() as f64;
    ...
    fn generate_comprehensive_results(&self) -> Result<IntegrationTestSuiteResults> {
        ...
        let total_files_tested: usize = self.results.iter().map(|r| r.files_tested).sum();
        ...
        let overall_performance_score: f64 = self
            .results
            .iter()
        ...
        if overall_compatibility_score < 0.8 {
            recommendations
                .push("Address compatibility issues before production deployment".to_string());
        }
    ...
    async fn test_deletion_markers(&self, table_path: &Path) -> Result<(usize, usize)> {
        ...
        if let Some(data_file) = self.find_data_file(table_path) {
            ...
            for window in data_content.windows(4) {
                if window == [0xFF, 0xFF, 0xFF, 0xFF] {
                    // Simplified deletion marker
                    markers_found += 1;
                }
            }
        }
    ...
    fn get_enabled_features(&self) -> String {
        ...
        if self.config.test_real_sstables {
            features.push("Real SSTables");
        }
        if self.config.test_feature_integration {
            features.push("Feature Integration");
        }
        if self.config.test_collection_types {
...
pub fn print_integration_test_results(results: &IntegrationTestSuiteResults) {
    ...
    for (category, tests) in categories {
        ...
        let total = tests.len();
        let avg_score: f64 =
            tests.iter().map(|t| t.compatibility_score).sum::<f64>() / total as f64;

        println!(
            "  🔹 {}: {}/{} passed, {:.3} avg score",
            category, passed, total, avg_score
        );
        for test in tests {
            let status = if test.passed { "✅" } else { "❌" };
            println!(
                "    {} {} ({:.3})",
                status, test.test_name, test.compatibility_score
            );
        }
    }
    ...
    async fn test_quick_integration_run() {
        ...
        match result {
            Ok(results) => {
                ...
                println!(
                    "Quick integration tests completed: {}/{} passed",
