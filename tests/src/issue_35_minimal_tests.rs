...
impl Issue35MinimalTests {
    ...
    pub async fn test_zero_tolerance_capability(&self) -> Result<()> {
        ...
        let tolerance = if cfg!(feature = "ci_zero_tolerance") { 0 } else { 64 };
        let within_tolerance = test_offset_diff <= tolerance;
        
        if cfg!(feature = "ci_zero_tolerance") {
            assert!(!within_tolerance, "32-byte diff should fail zero-tolerance validation");
            println!("  ✅ Zero-tolerance validation: 32-byte diff correctly rejected");
        } else {
            assert!(within_tolerance, "32-byte diff should pass development validation");
    ...
    async fn create_minimal_sstable(&self, path: &std::path::Path) -> Result<()> {
        ...
        let content = vec![
            ...
            0xCA, 0x55, 0x4E, 0xC1,
            // Version and basic header
            0x00, 0x05, 0x00, 0x00,
            // Minimal SSTable content for testing
            0x00, 0x00, 0x00, 0x01, // 1 partition
            0x00, 0x00, 0x00, 0x10, // 16 bytes data
            // Test partition key
            b't', b'e', b's', b't', b'_', b'k', b'e', b'y',
            // Test value data
...
async fn test_issue_35_minimal_compliance() {
    let test_suite = Issue35MinimalTests::new().await
