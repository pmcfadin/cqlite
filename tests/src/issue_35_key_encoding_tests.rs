...
impl Issue35KeyEncodingTests {
    ...
    pub async fn test_basic_partition_key_digest(&self) -> Result<()> {
        ...
        for (test_name, key_bytes) in test_cases {
            println!("  📝 Testing {} with key length: {}", test_name, key_bytes.len());
            ...
            if !key_bytes.is_empty() {
                ...
                assert_ne!(digest, different_digest, "Different keys should produce different digests");
            }
    ...
    pub async fn test_multi_component_partition_keys(&self) -> Result<()> {
        ...
        let test_cases = vec![
            ...
            (
                ...
                vec![ComparatorType::Text, ComparatorType::Text, ComparatorType::Text],
            ),
            (
                "binary_components",
        ...
        for (test_name, components, comparators) in test_cases {
            println!("  📝 Testing {} with {} components", test_name, components.len());
            ...
            for (i, component) in components.iter().enumerate() {
                ...
                match comparator {
                    ...
                    ComparatorType::Blob => {
                        ...
                        assert!(component.len() <= 1024, "Component should be reasonable size");
                    }
                    _ => {
            ...
            for component in &components {
                ...
                assert_ne!(composite_digest, individual_digest, 
                    "Composite key should have different digest than individual components");
            }
            
            println!("    ✅ {} passed - composite digest: {:?}", test_name, composite_digest);
        }
    ...
    pub async fn test_comparator_aware_key_encoding(&self) -> Result<()> {
        ...
        let test_cases = vec![
            ...
            (ComparatorType::Blob, Value::Blob(vec![0xDE, 0xAD, 0xBE, 0xEF])),
        ...
        for (comparator, value) in test_cases {
            println!("  📝 Testing {} comparator with value: {:?}", 
                comparator.type_name(), value);
            ...
            println!("    ✅ {} comparator passed - key bytes: {} B, digest: {:?}", 
                comparator.type_name(), key_bytes.len(), digest);
        }
        ...
        for (test_name, test_keys, description) in compatibility_tests {
            ...
            match test_name {
                ...
                _ => {
                    ...
                    assert_eq!(unique_digests.len(), digests.len(),
                        "All different keys should produce unique digests");
                }
            }
            
            println!("    ✅ {} passed - {} unique digests from {} keys", 
                test_name, digests.len(), digests.len());
        }
...
async fn test_issue_35_key_encoding_validation() {
    let test_suite = Issue35KeyEncodingTests::new().await
...
async fn test_key_digest_consistency_across_restarts() {
    ...
    assert_eq!(first_session_digests, second_session_digests,
...
async fn test_index_lookup_fails_with_raw_bytes() {
    ...
    for (i, raw_key) in test_keys.iter().enumerate() {
        ...
        assert_ne!(raw_key.clone(), correct_digest, 
        ...
        assert_eq!(correct_digest.len(), 8, 
            "Index.db digest should be 8 bytes, got {} bytes", correct_digest.len());
        ...
        if i > 0 {
            ...
            assert_ne!(correct_digest, prev_digest, 
                "Different keys should produce different digests");
        }
        
        println!("    ✅ Key {}: raw={} bytes, digest={} bytes, digest={:?}", 
            i, raw_key.len(), correct_digest.len(), correct_digest);
    }
    
    println!("✅ Index lookup digest validation test passed!");
    println!("💡 This test confirms that Index.db requires 8-byte digests, not raw partition key bytes");
}