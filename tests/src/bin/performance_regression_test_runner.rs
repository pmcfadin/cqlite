...
impl PerformanceRegressionTester {
    pub fn new(config_file: PathBuf, baseline_file: Option<PathBuf>, output_dir: PathBuf, verbose: bool) -> Result<Self> {
    ...
    fn load_benchmark_configs(config_file: &Path) -> Result<Vec<PerformanceBenchmarkConfig>> {
        ...
        let configs: Vec<PerformanceBenchmarkConfig> = serde_json::from_str(&content)
            .context("Failed to parse benchmark configuration")?;
    ...
    fn load_baseline_results(&self) -> Result<Option<Vec<BenchmarkResult>>> {
        if let Some(baseline_file) = &self.baseline_file {
            if baseline_file.exists() {
                ...
                let report: PerformanceRegressionReport = serde_json::from_str(&content)
    ...
    pub fn run_all_benchmarks(&self) -> Result<Vec<BenchmarkResult>> {
        ...
        for benchmark in &self.benchmarks {
            ...
            if self.verbose {
                ...
                println!("  Command: {} {}", benchmark.command, benchmark.args.join(" "));
            }
            ...
            match self.run_single_benchmark(benchmark) {
                Ok(result) => {
                    if result.success {
                        ...
                        println!("    Mean: {:.2}ms, P99: {:.2}ms", 
                                result.metrics.mean_duration_ms, result.metrics.p99_duration_ms);
                        println!("    Throughput: {:.2} ops/sec", result.metrics.throughput_ops_per_sec);
                    } else {
    ...
    fn run_single_benchmark(&self, config: &PerformanceBenchmarkConfig) -> Result<BenchmarkResult> {
        ...
        for i in 0..config.benchmark_iterations {
            if self.verbose && i % 10 == 0 {
                println!("      Benchmark iteration {}/{}", i + 1, config.benchmark_iterations);
            }
    ...
    fn execute_benchmark_command(&self, config: &PerformanceBenchmarkConfig) -> Result<f64> {
        ...
        cmd.stdout(if self.verbose { Stdio::inherit() } else { Stdio::null() });
        cmd.stderr(if self.verbose { Stdio::inherit() } else { Stdio::null() });
    ...
    pub fn analyze_regressions(&self, current_results: &[BenchmarkResult]) -> Result<PerformanceRegressionReport> {
        ...
        let summary = self.generate_performance_summary(current_results, &regressions, &improvements);
    ...
    fn analyze_single_benchmark_regression(
        ...
        let metrics_to_analyze = [
            ("mean_duration_ms", current.metrics.mean_duration_ms, baseline.metrics.mean_duration_ms),
            ("p95_duration_ms", current.metrics.p95_duration_ms, baseline.metrics.p95_duration_ms),
            ("p99_duration_ms", current.metrics.p99_duration_ms, baseline.metrics.p99_duration_ms),
            ("throughput_ops_per_sec", current.metrics.throughput_ops_per_sec, baseline.metrics.throughput_ops_per_sec),
        ];
    ...
    fn generate_performance_summary(
        ...
        let recommendation = if regressions.iter().any(|r| matches!(r.severity, RegressionSeverity::Critical)) {
            "❌ CRITICAL REGRESSIONS DETECTED - Do not merge this change".to_string()
        } else if regressions.iter().any(|r| matches!(r.severity, RegressionSeverity::Major)) {
    ...
    pub fn save_results(&self, report: &PerformanceRegressionReport) -> Result<PathBuf> {
        ...
        fs::write(&output_file, json)
            .context("Failed to write regression report")?;
...
<body>
    ...
    <table>
        ...
        report.baseline_file.as_ref().map(|p| p.display().to_string()).unwrap_or("None".to_string()),
    ...
    fn format_improvements_html(&self, improvements: &[ImprovementAnalysis]) -> String {
        ...
        improvements.iter()
            .map(|i| format!(
                ...
                i.benchmark_name, i.improvement_percent, i.metric,
                i.baseline_value, i.current_value
            ))
            .collect::<Vec<_>>()
            .join("\n")
    }
...
fn create_default_benchmark_config() -> Result<Vec<PerformanceBenchmarkConfig>> {
    Ok(vec![
        PerformanceBenchmarkConfig {
            ...
            args: vec!["run", "--release", "--package", "cqlite-cli", "--", "info", "test-data/generated/v4.1/sstables/simple_table/na-1-big-Data.db"].iter().map(|s| s.to_string()).collect(),
            timeout_seconds: 60,
            warmup_iterations: 5,
        ...
        PerformanceBenchmarkConfig {
            ...
            command: "cargo".to_string(),
            args: vec!["run", "--release", "--package", "cqlite-cli", "--", "info", "test-data/generated/v4.1/sstables/large_blob_table/na-1-big-Data.db"].iter().map(|s| s.to_string()).collect(),
            timeout_seconds: 120,
            warmup_iterations: 3,
        ...
        PerformanceBenchmarkConfig {
            ...
            command: "cargo".to_string(),
            args: vec!["run", "--release", "--package", "cqlite-cli", "--", "info", "test-data/generated/v4.1/sstables/collections_table/na-1-big-Data.db"].iter().map(|s| s.to_string()).collect(),
            timeout_seconds: 90,
            warmup_iterations: 5,
        ...
        PerformanceBenchmarkConfig {
            ...
            command: "cargo".to_string(),
            args: vec!["test", "--release", "--package", "tests"].iter().map(|s| s.to_string()).collect(),
            timeout_seconds: 300,
            warmup_iterations: 1,
...
fn main() -> Result<()> {
    ...
    println!("Overall change: {:.2}%", report.summary.overall_performance_change_percent);
    println!();
    ...
    let exit_code = if report.regressions.iter().any(|r| matches!(r.severity, RegressionSeverity::Critical)) {
        2 // Critical regressions
    } else if report.regressions.iter().any(|r| matches!(r.severity, RegressionSeverity::Major)) {
        1 // Major regressions
    } else {
