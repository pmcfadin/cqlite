...
async fn main() -> Result<()> {
                        ...
                        Ok(reader) => {
                            ...
                            match reader.get_health_metrics().await {
                                Ok(health) => {
                                    ...
                                    println!("   Memory usage: {} bytes", health.estimated_memory_usage);
                                    println!("   Compression: {} ({})", health.compression_enabled, health.compression_algorithm);
                            ...
                            match reader.perform_integrity_check().await {
                                Ok(integrity) => {
                                    ...
                                    println!("   Blocks checked: {}", integrity.total_blocks_checked);
                                    println!("   Total entries: {}", integrity.total_entries);
                                    println!("   Corrupted blocks: {}", integrity.corrupted_blocks.len());
                                    
                                    if integrity.corrupted_blocks.is_empty() && integrity.parsing_errors.is_empty() {
                                        test_results.passed += 1;
                                    } else {
                                        println!("   ⚠️  Found {} parsing errors", integrity.parsing_errors.len());
                                        test_results.failed += 1;
                                    }
                                }
                                Err(e) => {
    ...
    if test_results.critical == 0 && success_rate >= 0.8 {
        ...
        println!("   - {} critical failures prevent basic functionality", test_results.critical);
        println!("   - Success rate: {:.1}%", success_rate * 100.0);
