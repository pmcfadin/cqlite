...
use cqlite_core::storage::sstable::bti::{
    ...
    BtiConfig, BtiError, BtiLookupResult, BtiMetadata, BTI_MAGIC_NUMBER,
};
...
mod integration_tests {
    ...
    fn test_byte_comparable_encoder_integration() {
        ...
        let encoded = result.unwrap();
        assert!(!encoded.is_empty(), "Encoded composite key should not be empty");
    }
    ...
    fn test_byte_comparable_ordering() {
        ...
        let text_aa = encoder.encode_value(&Value::Text("aa".to_string())).unwrap();
        
        assert!(text_a < text_b, "Text ordering: 'a' should be less than 'b'");
        assert!(text_a < text_aa, "Text ordering: 'a' should be less than 'aa'");
        assert!(text_aa < text_b, "Text ordering: 'aa' should be less than 'b'");
        ...
        assert!(int_neg < int_zero, "Integer ordering: -100 should be less than 0");
        assert!(int_zero < int_pos, "Integer ordering: 0 should be less than 100");
    }
    ...
    fn test_node_type_parsing() {
        ...
        // Test payload flags
        assert_eq!(NodeType::from_header_byte(0x01).unwrap(), NodeType::PayloadOnly);
    ...
    fn test_bti_config_defaults() {
        ...
        assert!(config.page_aware_reading, "Page-aware reading should be enabled by default");
        assert_eq!(config.max_cached_nodes, 1024, "Default cache size should be 1024");
        assert!(config.pointer_compression, "Pointer compression should be enabled by default");
    }
    ...
    fn test_composite_key_separation() {
        ...
        assert_ne!(encoded1, encoded2, "Different composite keys should encode differently");
    }
    ...
    fn test_large_key_handling() {
        ...
        let large_composite: Vec<Value> = (0..100)
    ...
    fn test_special_values() {
        ...
        assert!(!empty_text.is_empty(), "Empty text should still produce encoded bytes");
        
        // Test zero values
        let zero_int = encoder.encode_value(&Value::Integer(0)).unwrap();
        let zero_bigint = encoder.encode_value(&Value::BigInt(0)).unwrap();
        
        assert!(!zero_int.is_empty(), "Zero integer should produce encoded bytes");
        assert!(!zero_bigint.is_empty(), "Zero bigint should produce encoded bytes");
    }
}
...
mod performance_tests {
    ...
    fn test_encoding_performance() {
        ...
        for i in 0..iterations {
            ...
            assert!(result.is_ok(), "Encoding should succeed for iteration {}", i);
        }
        ...
        assert!(ops_per_sec > 1000.0, "Encoding performance too slow: {:.0} ops/sec", ops_per_sec);
    }
...
mod compatibility_tests {
    ...
    fn test_cep25_type_hierarchy() {
        ...
        let null_val = encoder.encode_value(&Value::Null).unwrap_or_else(|_| vec![0x00]);
    ...
    fn test_cassandra_5_compatibility() {
        ...
        assert_eq!(BTI_MAGIC_NUMBER, 0x6461_0000, "BTI magic number should match Cassandra 5.0");
    ...
    fn test_ieee754_float_ordering() {
        ...
        assert!(neg_one < neg_zero, "Negative one should be less than negative zero");
        assert!(neg_zero <= pos_zero, "Negative zero should be <= positive zero");
        assert!(pos_zero < pos_one, "Positive zero should be less than positive one");
        assert!(pos_one < pos_inf, "Positive one should be less than positive infinity");
    }
}