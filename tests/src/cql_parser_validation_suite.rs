...
use cqlite_core::parser::{types::CqlTypeId, SSTableParser};
...
impl CqlParserValidationSuite {
    ...
    fn test_basic_cql_parsing(&mut self) -> Result<()> {
        ...
        if result.passed {
            println!(
                "✅ Basic CQL parsing tests passed ({} test cases)",
                test_cases.len()
            );
        } else {
            println!(
                "❌ Basic CQL parsing tests failed: {}",
    ...
    fn test_create_table_variations(&mut self) -> Result<()> {
        ...
        if result.passed {
            println!(
                "✅ CREATE TABLE variations tests passed ({} variations)",
                variations.len()
            );
        } else {
            println!(
                "❌ CREATE TABLE variations tests failed: {}",
                result.error_message.unwrap()
            );
        }
    ...
    fn test_table_name_matching(&mut self) -> Result<()> {
        ...
        let test_cases = vec![
            ...
            (
                "CREATE TABLE keyspace1.users (id UUID PRIMARY KEY);",
                "users",
                true,
            ),
            (
        ...
        if result.passed {
            println!(
                "✅ Table name matching tests passed ({} test cases)",
                test_cases.len()
            );
        } else {
            println!(
                "❌ Table name matching tests failed: {}",
                result.error_message.unwrap()
            );
        }
    ...
    fn test_type_conversions(&mut self) -> Result<()> {
                ...
                match Ok::<Vec<u8>, cqlite_core::Error>(vec![]) {
                    ...
                    Ok(_serialized) => {
                        ...
                        match Ok::<(cqlite_core::Value, i32), cqlite_core::Error>((
                            ...
                            // self.parser.parse_value(&serialized[1..], type_id) {
                            Ok((parsed_value, _)) => {
                            ...
                            Err(e) => {
                                failures
                                    .push(format!("Type '{}': Parse error: {}", cql_type_str, e));
                            }
                        }
                    }
                    Err(e) => {
        ...
        if result.passed {
            println!(
                "✅ Type conversion tests passed ({} types)",
                type_mappings.len()
            );
        } else {
            println!(
                "❌ Type conversion tests failed: {}",
                result.error_message.unwrap()
            );
        }
    ...
    fn test_complex_types(&mut self) -> Result<()> {
        ...
        let complex_types = vec![
            ...
            (
                "TUPLE<UUID, TIMESTAMP, TEXT>",
                "tuple with uuid, timestamp, text",
            ),
            // Complex nested structures
            (
                "MAP<TEXT, FROZEN<LIST<TUPLE<UUID, TEXT>>>>",
        ...
        if result.passed {
            println!(
                "✅ Complex type tests passed ({} types)",
                complex_types.len()
            );
        } else {
            println!(
                "❌ Complex type tests failed: {}",
                result.error_message.unwrap()
            );
        }
    ...
    fn test_error_handling(&mut self) -> Result<()> {
        ...
        let malformed_cql = vec![
            ...
            (
                "CREATE TABLE users (id UUID PRIMARY KEY, tags LIST<>);",
                "empty list type",
            ),
            // Invalid map syntax
            (
                "CREATE TABLE users (id UUID PRIMARY KEY, data MAP<TEXT>);",
                "incomplete map type",
            ),
            // Reserved keyword as column name without quotes
            (
                "CREATE TABLE users (select TEXT PRIMARY KEY);",
                "reserved keyword",
            ),
            // Invalid clustering order
            (
                "CREATE TABLE events (id UUID, ts TIMESTAMP, PRIMARY KEY (id, ts)) WITH CLUSTERING ORDER BY (ts INVALID);",
                "invalid clustering order",
            ),
        ];
        ...
        if result.passed {
            println!(
                "✅ Error handling tests passed ({} malformed cases)",
                malformed_cql.len()
            );
        } else {
            println!(
    ...
    fn test_json_vs_cql_schemas(&mut self) -> Result<()> {
        ...
        for (i, (cql, expected_json)) in test_cases.iter().enumerate() {
            ...
            match self.simulate_cql_to_schema_conversion(cql) {
                ...
                Err(e) => {
                    failures.push(format!("Test case {}: Failed to parse CQL: {}", i + 1, e));
                }
            }
        }
        ...
        if result.passed {
            println!(
                "✅ JSON vs CQL schema tests passed ({} comparisons)",
                test_cases.len()
            );
        } else {
            println!(
                "❌ JSON vs CQL schema tests failed: {}",
                result.error_message.unwrap()
            );
        }
    ...
    fn test_parser_performance(&mut self) -> Result<()> {
        ...
        let throughput_mbs =
            (content_size * iterations) as f64 / perf_duration.as_secs_f64() / 1_000_000.0;
        ...
        match cql_type {
            ...
            CqlType::Tuple(types) => {
                !types.is_empty()
                    && types
                        .iter()
                        .all(|t| self.validate_complex_type_structure(t))
            }
    ...
    fn create_test_cql_files(&self) -> Result<Vec<std::path::PathBuf>> {
        ...
        std::fs::create_dir_all(test_dir)
            .map_err(|e| Error::schema(format!("Failed to create test directory: {}", e)))?;
...
impl ValidationReport {
    ...
    pub fn print_report(&self) {
        ...
        for result in sorted_results {
            ...
            println!(
                "  {} - {} ({}ms, {} bytes)",
                result.test_name, status, result.execution_time_ms, result.bytes_processed
            );
        ...
        if self.failed_tests == 0 {
            ...
            println!(
                "⚠️  {} test(s) failed. Review errors above.",
                self.failed_tests
            );
        }
    }
