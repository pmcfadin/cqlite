...
impl CqlPerformanceBenchmarkSuite {
    ...
    fn benchmark_concurrent_parsing(&mut self) -> Result<()> {
        ...
        for handle in handles {
            handle
                .join()
                .map_err(|_| Error::schema("Thread join error".to_string()))?;
        }
        ...
        let result = BenchmarkResult {
            ...
            throughput_mb_per_sec: (success_count * test_cql.len()) as f64
                / total_duration.as_secs_f64()
                / 1_000_000.0,
            bytes_processed: success_count * test_cql.len(),
            memory_usage_kb: self.memory_tracker.current_memory_kb,
            passed_performance_targets: error_count == 0
                && success_count > total_iterations * 9 / 10, // 90% success rate
            error_message: if error_count > 0 {
                Some(format!(
                    "{} errors out of {} operations",
                    error_count, total_iterations
                ))
            } else {
                None
            },
        ...
        if result.passed_performance_targets {
            println!(
                "✅ {} benchmark passed ({:.1} ops/sec, {:.2}ms avg latency)",
    ...
    fn benchmark_memory_usage(&mut self) -> Result<()> {
        ...
        let memory_growth_rate = memory_measurements
            .iter()
        ...
        let result = BenchmarkResult {
            ...
            error_message: if passed {
                ...
                Some(format!(
                    "Memory usage {} KB exceeds target {} KB",
                    self.memory_tracker.peak_memory_kb, targets.max_memory_usage_kb
                ))
            },
        };
        ...
        if result.passed_performance_targets {
            println!(
                "✅ {} benchmark passed (peak memory: {} KB)",
    ...
    fn benchmark_schema_validation(&mut self) -> Result<()> {
        ...
        for (test_type, cql) in &validation_tests {
            ...
            for _ in 0..iterations {
                match Self::simulate_cql_parsing(cql) {
                    Ok(schema) => match Self::simulate_schema_validation(&schema) {
        ...
        if result.passed_performance_targets {
            println!(
                "✅ {} benchmark passed ({:.1} validations/sec, {:.2}ms avg latency)",
                benchmark_name, result.throughput_ops_per_sec, result.avg_latency_ms
            );
        } else {
            println!(
                ...
                result
                    .error_message
                    .unwrap_or("Performance targets not met".to_string())
            );
        }
    ...
    fn benchmark_type_conversions(&mut self) -> Result<()> {
        ...
        if result.passed_performance_targets {
            println!(
                "✅ {} benchmark passed ({:.1} conversions/sec, {:.3}ms avg latency)",
                benchmark_name, result.throughput_ops_per_sec, result.avg_latency_ms
            );
        } else {
            println!(
    ...
    fn benchmark_stress_testing(&mut self) -> Result<()> {
            ...
            for i in 0..iterations {
                match Self::simulate_cql_parsing(cql) {
                    Ok(schema) => match Self::simulate_schema_validation(&schema) {
                        ...
                        Err(e) => {
                            failures.push(format!("{} iteration {}: {}", test_name, i + 1, e))
                        }
                    },
        ...
        if result.passed_performance_targets {
            println!(
                "✅ {} benchmark passed ({:.2} ops/sec, {:.1}ms avg latency, {} KB peak memory)",
                benchmark_name,
    ...
    fn run_benchmark(
        ...
        let throughput_mb = (successful_iterations * test_cql.len()) as f64
            / total_duration.as_secs_f64()
            / 1_000_000.0;
        ...
        if result.passed_performance_targets {
            println!(
                "✅ {} benchmark passed ({:.1} ops/sec, {:.2}ms avg, {}ms p99)",
    ...
    fn generate_report(&self) -> BenchmarkReport {
        ...
        let passed_benchmarks = self
            .results
            .values()
            .filter(|r| r.passed_performance_targets)
            .count();
...
impl BenchmarkReport {
    ...
    pub fn print_report(&self) {
        ...
        for result in sorted_results {
            let status = if result.passed_performance_targets {
                "✅ PASS"
            } else {
                "❌ FAIL"
            };
            println!(
                "  {} - {} ({} iterations)",
                result.benchmark_name, status, result.iterations
            );
            println!(
                "    Throughput: {:.1} ops/sec, {:.2} MB/sec",
                result.throughput_ops_per_sec, result.throughput_mb_per_sec
            );
            println!(
                "    Latency: {:.2}ms avg, {}ms min, {}ms max",
        ...
        if self.failed_benchmarks == 0 {
            ...
            println!(
                "⚠️  {} benchmark(s) failed to meet performance targets.",
                self.failed_benchmarks
            );
        }
    }
...
mod tests {
    ...
    fn test_simulation_methods() {
        let result = CqlPerformanceBenchmarkSuite::simulate_cql_parsing(
