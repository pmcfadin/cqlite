...
impl ReplTestSuite {
    ...
    fn assess_quality_gates(&self) {
        ...
        let gate2_tests = ["help_system", "configuration_system", "data_exploration", "meta_commands"];
        ...
        let gate4_tests = ["exploration_workflow", "config_workflow", "help_workflow", "query_workflow"];
        ...
        self.print_gate_status("Gate 7: Session Management", gate7_passed, gate7_tests.len());
    }
    ...
    fn test_help_system(&mut self) -> Result<()> {
        ...
        let basic_help = output.contains("CQLite Interactive REPL") &&
                        output.contains("Meta Commands");
        ...
        self.record_result("help_system", passed, format!("{}\n---\n{}", output, topics_output), None);
        Ok(())
    }
    ...
    fn test_configuration_system(&mut self) -> Result<()> {
        ...
        self.record_result("configuration_system", passed, format!("{}\n---\n{}", output, timing_output), None);
        Ok(())
    }
    ...
    fn test_data_exploration_commands(&mut self) -> Result<()> {
        ...
        self.record_result("data_exploration", passed, format!("{}\n---\n{}", keyspaces_output, tables_output), None);
        Ok(())
    }
    ...
    fn test_meta_commands(&mut self) -> Result<()> {
        ...
        self.record_result("meta_commands", passed, format!("{}\n---\n{}", history_output, clear_output), None);
        Ok(())
    }
    ...
    fn test_basic_cql_queries(&mut self) -> Result<()> {
        ...
        let output = self.run_repl_command("SELECT * FROM system.keyspaces LIMIT 1;\n:quit", "basic_cql")?;
    ...
    fn test_query_timing_and_performance(&mut self) -> Result<()> {
        ...
        let output = self.run_repl_command(":timing\nSELECT * FROM system.keyspaces LIMIT 1;\n:quit", "timing")?;
    ...
    fn run_repl_command(&self, input: &str, test_name: &str) -> Result<String> {
        ...
        if self.config.verbose {
            println!("Test {}: {} chars output in {:?}", test_name, output.len(), start.elapsed());
        }
