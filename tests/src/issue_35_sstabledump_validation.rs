...
use cqlite_core::{Config, Error, Result, platform::Platform, storage::sstable::SSTableReader};
...
impl SSTableDumpParityValidator {
    ...
    pub async fn run_sstabledump(&self, sstable_path: &Path) -> Result<SSTableDumpOutput> {
        ...
        if use_real_sstabledump {
            ...
            match self
                .run_real_sstabledump_command(sstable_path, &output_file)
                .await
            {
                Ok(output) => {
                    println!(
                        "  ✓ Real SSTableDump output saved to {}",
                        output_file.display()
                    );
                    return Ok(output);
                }
        ...
        println!(
            "  ✓ Mock SSTableDump output saved to {}",
            output_file.display()
        );
    ...
    async fn run_real_sstabledump_command(
        &self,
        sstable_path: &Path,
        output_file: &Path,
        ...
        for sstabledump_cmd in &sstabledump_commands {
            ...
            match self
                .execute_sstabledump_command(sstabledump_cmd, sstable_path, output_file)
                .await
            {
                Ok(output) => {
                    println!(
                        "  ✅ Successfully executed sstabledump with: {}",
                        sstabledump_cmd
                    );
                    return Ok(output);
                }
        ...
        Err(Error::corruption(
            "All sstabledump command variants failed".to_string(),
        ))
    }

    /// Execute a specific sstabledump command
    async fn execute_sstabledump_command(
        &self,
        cmd: &str,
        sstable_path: &Path,
        output_file: &Path,
        ...
        let mut command = if cmd.starts_with("docker") {
            ...
            docker_cmd.args([
                ...
                "--rm",
                "-v",
                &format!("{}:/data", sstable_path.parent().unwrap().display()),
                "-w",
                "/data",
                "cassandra:5.0",
                "sstabledump",
    ...
    fn extract_json_from_sstabledump_output(&self, output: &str) -> Result<SSTableDumpOutput> {
        ...
        if let Some(start) = json_start {
            ...
            if let Some(end) = json_end {
                ...
                return serde_json::from_str::<SSTableDumpOutput>(&json_str).map_err(|e| {
                    Error::corruption(format!("Failed to parse extracted JSON: {}", e))
                });
            }
        }
    ...
    async fn generate_mock_sstabledump_output(
        &self,
        sstable_path: &Path,
    ) -> Result<SSTableDumpOutput> {
    ...
    pub async fn validate_index_parity(
        ...
        if let Some(sstabledump_index) = &sstabledump_output.index {
            ...
            for (i, sstabledump_partition) in sstabledump_index.partitions.iter().enumerate() {
                ...
                if let Ok(Some((our_data_offset, our_data_size))) =
                    reader.lookup_partition_with_index(partition_key).await
                {
                    ...
                    let offset_tolerance = if cfg!(feature = "ci_zero_tolerance") {
                        0
                    } else {
                        64
                    }; // Zero tolerance for CI
                    let offset_diff =
                        (our_data_offset as i64 - sstabledump_partition.offset as i64).abs();
                    ...
                    let size_tolerance = if cfg!(feature = "ci_zero_tolerance") {
                        0
                    } else {
                        (sstabledump_partition.size as f64 * 0.1) as u32
                    };
                    let size_diff =
                        (our_data_size as i32 - sstabledump_partition.size as i32).abs();
            ...
            println!(
                "  ✓ Index.db validation completed: {} differences",
            ...
            result
                .differences
                .push("Index.db reader or sstabledump index data not available".to_string());
        }
    ...
    pub async fn validate_summary_parity(
        ...
        if let Some(_sstabledump_summary) = &_sstabledump_output.summary {
            ...
            */
            ...
            println!(
                "  ✓ Summary.db validation completed: {} differences",
                result.differences.len()
            );
        } else {
            result.passed = false;
            result
                .differences
                .push("Summary.db reader or sstabledump summary data not available".to_string());
        }
    ...
    pub async fn validate_statistics_parity(
        ...
        result.details.insert(
            ...
            ValidationDetail {
                ...
                sstabledump_value: "pending".to_string(),
                tolerance_met: true,
                description: "Statistics.db validation placeholder - awaiting public API"
                    .to_string(),
            },
        );
    ...
    pub async fn validate_full_parity(
        &self,
        sstable_path: &Path,
    ) -> Result<Vec<ParityValidationResult>> {
        ...
        results.push(
            self.validate_index_parity(&reader, &sstabledump_output)
                .await?,
        );
        results.push(
            self.validate_summary_parity(&reader, &sstabledump_output)
                .await?,
        );
        results.push(
            self.validate_statistics_parity(&reader, &sstabledump_output)
                .await?,
        );
        ...
        if failed_components == 0 {
            ...
            println!(
                "❌ {} component(s) failed parity validation",
                failed_components
            );
