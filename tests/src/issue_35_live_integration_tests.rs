...
use cqlite_core::{
    Config, Result,
    platform::Platform,
    storage::sstable::{SSTableManager, SSTableReader},
...
impl Issue35LiveIntegrationTestSuite {
    ...
    pub async fn generate_wide_partition_data(
        &self,
        config: &WidePartitionTestConfig,
    ) -> Result<PathBuf> {
    ...
    pub async fn test_summary_reader_integration(&self) -> Result<()> {
        ...
        if let Some((min_token, max_token)) = token_coverage {
            ...
            println!(
                "    Token range iteration found {} entries",
                range_results.len()
            );
        } else {
...
async fn test_promoted_index_wide_partitions() {
    ...
    let config = WidePartitionTestConfig::promoted_index_guaranteed();
    let sstable_path = test_suite
        .generate_wide_partition_data(&config)
        .await
        .unwrap();
    ...
    assert!(
        ...
        "Should find wide partition via Index.db"
    );
    ...
    if let Some((offset, size)) = lookup_result {
        ...
        assert!(
            size >= 1024,
            "Wide partition should be at least 1KB: actual size={}",
            size
        );
    }
...
async fn test_no_promoted_index_small_partitions() {
    ...
    assert!(
        true,
        "Index.db reader test placeholder for small partitions"
    );
    ...
    if let Some((offset, size)) = lookup_result {
        ...
        assert!(
            size < 10240,
            "Small partition should be less than 10KB: actual size={}",
            size
        );
    }
