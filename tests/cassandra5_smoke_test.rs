...
fn test_cassandra5_smoke_read_simple_table() {
    ...
    assert!(
        data_path.exists(),
        "Cassandra 5 fixture not found at {:?}",
        data_path
    );
    
    // Create minimal reader
    let reader = MinimalSstableReader::new(data_path)
        .expect("Failed to create SSTable reader");
    ...
    // Extract single row
    let row = reader.extract_single_row()
...
fn test_all_fixture_components_readable() {
    ...
    for component in &components {
        ...
        let data = fs::read(&component_path)
            .unwrap_or_else(|e| panic!("Failed to read {}: {}", component, e));
        
        assert!(
            !data.is_empty() || component == &"Filter.db", // Filter can be empty
            "Component {} is unexpectedly empty",
            component
        );
        
        println!("✅ Successfully read component {}: {} bytes", component, data.len());
    }
}
...
fn test_toc_parsing() {
    ...
    let toc_content = fs::read_to_string(&toc_path)
        .expect("Failed to read TOC.txt");
    ...
    for expected_component in &expected {
        assert!(
            ...
            "TOC missing expected component: {}",
            expected_component
        );
    }
...
fn test_digest_crc32_format() {
    ...
    let digest_data = fs::read(&digest_path)
        .expect("Failed to read Digest.crc32");
    ...
    println!("✅ Digest.crc32 is valid: 0x{:08x}", crc_value);
}