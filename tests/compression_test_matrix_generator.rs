...
pub fn generate_compression_test_matrix() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for algorithm in &algorithms {
        for (i, chunk_size) in chunk_sizes.iter().enumerate() {
            ...
            println!("📦 Generating dataset: {} with {} chunks", algorithm, chunk_size);
...
fn generate_corruption_test(
    ...
    println!("  📝 Created corruption test: {}", corruption_path.display());
...
mod tests {
    ...
    fn test_generate_matrix() {
        ...
        let expected_dirs = vec![
            "lz4-4kb", "lz4-16kb", "lz4-64kb",
            "snappy-4kb", "snappy-16kb", "snappy-64kb",
            "zstd-4kb", "zstd-16kb", "zstd-64kb",
            "deflate-4kb", "deflate-16kb", "deflate-64kb",
        ];
        ...
        for dir_name in expected_dirs {
            ...
            assert!(corruption_file.exists(), "Missing corruption test in {}", dir_name);
        }
    }
