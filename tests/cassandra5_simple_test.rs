...
fn test_cassandra5_fixture_integrity() {
    ...
    let fixture_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("fixtures/cassandra5/minimal/simple_table");
    ...
    for file in &required_files {
        ...
        assert!(metadata.len() > 0 || file == &"Filter.db", "File {} is empty", file);
    ...
    assert!(components.len() >= 6, "TOC should list at least 6 components");
    assert!(components.contains(&"Data.db"), "TOC should list Data.db");
    assert!(components.contains(&"Statistics.db"), "TOC should list Statistics.db");
    
    println!("✅ TOC.txt validated - {} components listed", components.len());
    
    // Test total fixture size is reasonable
    let total_size: u64 = required_files.iter()
        .map(|file| fs::metadata(fixture_path.join(file)).unwrap().len())
        .sum();
    
    assert!(total_size < 1024, "Total fixture size should be < 1KB, got {} bytes", total_size);
    
    println!("✅ Total fixture size: {} bytes (meets minimal size requirement)", total_size);
