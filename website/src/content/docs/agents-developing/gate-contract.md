...
dep-duplicates:    PASS|SKIP (<Ns>)  [via check-dep-duplicates.sh (cargo tree -d --workspace --target all; a metadata probe, compiles nothing): feature set NOT observed]  {no census — the guard emits its own MEASURED/verdict lines, not an AGENT-GATE-CENSUS contract line}
