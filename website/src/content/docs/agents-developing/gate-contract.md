...
| `pub-surface` | `bash scripts/ci/check-pub-surface.sh` — cqlite-core public-API snapshot guard (issue #1712): diffs the default-feature surface — walked from rustdoc's module index graph, re-exports and glob expansions included, cross-checked against rustdoc's `all.html` — against `cqlite-core/pub-surface.snapshot` and asserts an unconditional crate-root `pub mod` is really in that surface. Granularity is item paths + associated-item names (methods, variants, fields, assoc consts) + re-export edges, never signatures; trait/blanket impl members are excluded by design. Regenerate with `--regenerate`; SKIP-aware (loud) only on an absent cqlite-core |
