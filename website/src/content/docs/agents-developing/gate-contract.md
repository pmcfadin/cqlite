...
| `pub-surface` | `bash scripts/ci/check-pub-surface.sh` — cqlite-core public-API snapshot guard (issue #1712): diffs the rustdoc-derived default-feature surface against `cqlite-core/pub-surface.snapshot` and asserts an unconditional crate-root `pub mod` is really in that surface. Granularity is item paths, not signatures. Regenerate with `--regenerate`; SKIP-aware (loud) only on an absent cqlite-core |
