...
> **Website-page cross-link rule (issue #2480).** Every new page MUST be
> cross-linked from an existing page's **body prose** (e.g. add it to the
> `user-docs/index.md` Topics list), not just the sidebar. The
> `starlight-llms-txt` plugin strips sidebar nav, so a page's URL only lands in
> `llms-full.txt` when some page's body links to it. A page reachable only via
> the sidebar fails `scripts/validate-agent-plumbing.mjs` — which reds the
> `Build docs site` check on the PR (advisory) and, if it slips onto `main`,
> files a docs-site failure tracking issue rather than silently skipping deploy
> (issue #2654).
