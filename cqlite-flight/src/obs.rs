...
/// `cqlite.error.category = "other"` bucket (see [`record_status_error`]), so a
/// benign abort (a split torn down under a streaming reader, a client that hung
/// up, an admission shed) was indistinguishable in-field from a genuine internal
/// fault. The gRPC `Code` is itself lossy — a client disconnect and a
...
/// knowable at the site that raises it. This enum is that authoritative,
/// closed-set classification, STAMPED at each abort construction site and NEVER
/// inferred from the gRPC code or the error message text (no-heuristics #28).
...
impl AbortReason {
    ...
    /// the categories [`record_status_error`]'s code mapping produced before this
    /// change (behavior-preserving for the coarse dimension).
    fn error_category(self) -> cqlite_core::observability::ErrorCategory {
...
/// (issue #2681).
///
/// This is the reason-carrying entry point that replaces [`record_status_error`]
/// on the `do_get` abort path. It:
///
...
/// [`record_status_error`]'s code→error mapping.
fn category_placeholder_error(
    category: cqlite_core::observability::ErrorCategory,
) -> cqlite_core::Error {
    use cqlite_core::observability::ErrorCategory;
    use cqlite_core::Error;
    match category {
